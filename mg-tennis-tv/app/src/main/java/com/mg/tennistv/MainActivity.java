...
public class MainActivity extends Activity implements SensorEventListener {
    ...
    private CameraMotionTracker vision;
    private volatile float visionX=0f, visionStrength=0f, visionY=0f;
    private volatile long visionTime=0L;
    ...
    @Override public void onCreate(Bundle b){
        ...
        vision=new CameraMotionTracker(this,(x,strength,vertical)->{visionX=x;visionStrength=strength;visionY=vertical;visionTime=System.currentTimeMillis();});
        askPermissions();
    }
    ...
    @Override protected void onPause(){sensorManager.unregisterListener(this);if(vision!=null)vision.stop();if(web!=null)web.onPause();super.onPause();}
    ...
    private void startVision(){if(checkSelfPermission(Manifest.permission.CAMERA)==PackageManager.PERMISSION_GRANTED&&vision!=null)vision.start();}
    private boolean visionFresh(){return System.currentTimeMillis()-visionTime<260&&visionStrength>.025f;}
    private float fuseDirection(float imu){return visionFresh()?clamp(imu*.70f+visionX*.30f,-1f,1f):imu;}
    private float fusePower(float p){return visionFresh()?clamp(p*(1f+Math.min(.28f,visionStrength*.90f)),.65f,2.75f):p;}
    private float fuseTilt(float t){return visionFresh()?clamp(t*.82f+(visionX*4.5f)*.18f,-6f,6f):t;}
    ...
    public void startAi(){controllerMode=false;externalRacketHost=false;if(vision!=null)vision.stop();stopRacketService();closeBt();sensorManager.unregisterListener(this);registerSensors();eval("window.setMode&&window.setMode('ai')");Toast.makeText(this,"Bu telefon: ekran + kendi raketi",Toast.LENGTH_SHORT).show();}
    ...
    private void readRemote(BluetoothSocket s,boolean asLocal)throws IOException{
        ...
        while((line=br.readLine())!=null){
            if(line.startsWith("SWING,")){String[] q=line.split(",");if(q.length>=3)try{float p=fusePower(Float.parseFloat(q[1])),d=fuseDirection(Float.parseFloat(q[2]));if(asLocal)localSwing(p,d);else remoteSwing(p,d);}catch(NumberFormatException ignored){}}
            else if(line.startsWith("RAISE_HIT,")){String[] q=line.split(",");if(q.length>=3)try{float p=clamp(fusePower(Float.parseFloat(q[1]))*1.10f,.95f,2.85f),d=fuseDirection(Float.parseFloat(q[2]));eval("document.getElementById('status').textContent='Yüksek raket • ileri sert vuruş • kamera+IMU';");if(asLocal)localSwing(p,d);else remoteSwing(p,d);}catch(NumberFormatException ignored){}}
            else if(line.startsWith("TILT,")){String[] q=line.split(",");if(q.length>=2)try{float t=fuseTilt(Float.parseFloat(q[1]));eval(String.format(Locale.US,"window.setPlayerTilt&&window.setPlayerTilt(%.4f)",t));}catch(NumberFormatException ignored){}}
        }
    }
    ...
    @Override public void onBackPressed(){if(controllerMode){controllerMode=false;stopRacketService();eval("window.setMode&&window.setMode('ai')");sensorManager.unregisterListener(this);registerSensors();return;}if(externalRacketHost){externalRacketHost=false;if(vision!=null)vision.stop();closeBt();eval("window.setMode&&window.setMode('ai')");sensorManager.unregisterListener(this);registerSensors();return;}super.onBackPressed();}
    @Override protected void onDestroy(){if(vision!=null)vision.stop();closeBt();if(web!=null){web.removeJavascriptInterface("Android");web.destroy();web=null;}super.onDestroy();}
}
