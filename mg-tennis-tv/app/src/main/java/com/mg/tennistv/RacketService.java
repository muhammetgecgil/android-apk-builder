...
public class RacketService extends Service implements SensorEventListener {
    ...
    private int displayRotation(){try{WindowManager wm=(WindowManager)getSystemService(WINDOW_SERVICE);Display d=wm==null?null:wm.getDefaultDisplay();return d==null?Surface.ROTATION_0:d.getRotation();}catch(Exception e){return Surface.ROTATION_0;}}
    ...
    @Override public void onSensorChanged(SensorEvent e){if(!running&&System.currentTimeMillis()>calibrateUntil)return;int type=e.sensor.getType();if(type==Sensor.TYPE_GYROSCOPE){gx=e.values[0];gy=e.values[1];gz=e.values[2];gyroMag=(float)Math.sqrt(gx*gx+gy*gy+gz*gz);return;}if(type==Sensor.TYPE_GAME_ROTATION_VECTOR||type==Sensor.TYPE_ROTATION_VECTOR){float[] rm=new float[9],rr=new float[9],ori=new float[3];SensorManager.getRotationMatrixFromVector(rm,e.values);SensorManager.remapCoordinateSystem(rm,SensorManager.AXIS_Y,SensorManager.AXIS_MINUS_X,rr);SensorManager.getOrientation(rr,ori);float roll=(float)Math.toDegrees(ori[2]);pitchDeg=(float)Math.toDegrees(ori[1]);long now=System.currentTimeMillis();if(now<calibrateUntil){calibPitchSum+=pitchDeg;calibCount++;if(calibCount>8)neutralPitch=calibPitchSum/calibCount;}else if(Float.isNaN(neutralPitch)){neutralPitch=pitchDeg;broadcast("CALIBRATED");}float dp=pitchDeg-neutralPitch;if(Math.abs(dp)>30f)raisedUntil=now+1150L;tilt=clamp(tilt*.80f+clamp(roll/8f,-6f,6f)*.20f,-6f,6f);long n=System.nanoTime();if(running&&n-lastTilt>33_000_000L){lastTilt=n;send(String.format(Locale.US,"TILT,%.4f",tilt));}return;}if(!running||e.values.length<3)return;
        float ax=e.values[0],ay=e.values[1],az=e.values[2];float a=(float)Math.sqrt(ax*ax+ay*ay+az*az);long now=System.currentTimeMillis();float threshold=type==Sensor.TYPE_LINEAR_ACCELERATION?6.4f:13.2f;
        float sx=screenX(ax,ay);float sgx=screenX(gx,gy);float accelDir=clamp(sx/Math.max(4.5f,a),-1f,1f);float gyroDir=clamp(sgx/Math.max(1.0f,gyroMag),-1f,1f);float direction=clamp(accelDir*.68f+gyroDir*.32f,-1f,1f);if(Math.abs(direction)<.08f)direction=0f;
        boolean raisedForward=now<raisedUntil&&a>threshold*1.08f&&gyroMag>.95f&&Math.abs(az)>threshold*.20f&&now-lastSwing>285;
        if(raisedForward){lastSwing=now;raisedUntil=0L;emitSwing(a,threshold,direction,true);return;}
        if(a>threshold&&gyroMag>.95f&&now-lastSwing>245){lastSwing=now;emitSwing(a,threshold,direction,false);}}
    private void emitSwing(float a,float threshold,float direction,boolean raised){float power=clamp((a/threshold)*(raised?.58f:.54f)+(gyroMag/4f)*(raised?.42f:.46f),raised?.95f:.68f,raised?2.65f:2.45f);String stroke=Math.abs(direction)<.12f?"CENTER":(direction>0?"FOREHAND":"BACKHAND");float screenGx=screenX(gx,gy),screenGy=screenY(gx,gy);float spin=clamp((Math.abs(gz)*.58f+Math.abs(screenGx)*.27f+Math.abs(screenGy)*.15f)*6f,0f,30f);String spinType=Math.abs(screenGx)>Math.abs(gz)*1.20f?"SLICE":"TOPSPIN";send(String.format(Locale.US,"%s,%.4f,%.4f,%s,%s,%.2f",raised?"RAISE_HIT":"SWING",power,direction,stroke,spinType,spin));vibrate(raised?34:22);}
