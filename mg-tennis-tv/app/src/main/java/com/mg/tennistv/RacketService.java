...
public class RacketService extends Service implements SensorEventListener {
    ...
    private long lastSwing=0L, lastTilt=0L;
    ...
    @Override public void onCreate(){
        ...
        if(rotation==null) rotation=sm.getDefaultSensor(Sensor.TYPE_ROTATION_VECTOR);
        PowerManager pm=(PowerManager)getSystemService(POWER_SERVICE);
        if(pm!=null){
            wakeLock=pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,"MGTennis:Racket");
    ...
    @Override public int onStartCommand(Intent intent,int flags,int startId){
        ...
        if(address==null||address.isEmpty()){ stopSelf(); return START_NOT_STICKY; }
        if(wakeLock!=null&&!wakeLock.isHeld()) wakeLock.acquire(8*60*60*1000L);
        registerSensors();
        connect(address);
    ...
    private void connect(String address){
        new Thread(()->{
            ...
            try{
                ...
                if(bt==null) throw new IOException("Bluetooth yok");
                if(Build.VERSION.SDK_INT>=31 && checkSelfPermission(Manifest.permission.BLUETOOTH_CONNECT)!=PackageManager.PERMISSION_GRANTED)
                    throw new SecurityException("Bluetooth izni yok");
                if(bt.isDiscovering()) bt.cancelDiscovery();
                BluetoothDevice dev=bt.getRemoteDevice(address);
                socket=dev.createRfcommSocketToServiceRecord(UUID_GAME);
                socket.connect();
                out=new PrintWriter(new OutputStreamWriter(socket.getOutputStream()),true);
                running=true;
                notifyState("Raket bağlı • ekran kapanabilir");
                while(running && socket!=null && socket.isConnected()) SystemClock.sleep(1000);
            }catch(Exception e){
                running=false;
                notifyState("Bağlantı koptu • uygulamayı açıp yeniden bağla");
            }
        },"MG-Racket-Bluetooth").start();
    }
    ...
    @Override public void onSensorChanged(SensorEvent e){
        ...
        if(type==Sensor.TYPE_GAME_ROTATION_VECTOR||type==Sensor.TYPE_ROTATION_VECTOR){
            float[] rm=new float[9], rr=new float[9], ori=new float[3];
            ...
            tilt=clamp(tilt*.82f+clamp(roll/8f,-6f,6f)*.18f,-6f,6f);
            long n=System.nanoTime();
            if(n-lastTilt>33_000_000L){ lastTilt=n; send(String.format(Locale.US,"TILT,%.4f",tilt)); }
            return;
        }
        ...
        if(a>threshold && gyroMag>1.35f && now-lastSwing>285){
            lastSwing=now;
            float power=clamp((a/threshold)*.54f+(gyroMag/4f)*.46f,.68f,2.45f);
            float direction=clamp(e.values[0]/Math.max(5f,a),-1f,1f);
            send(String.format(Locale.US,"SWING,%.4f,%.4f",power,direction));
            vibrate(22);
        }
    }
    ...
    private void vibrate(int ms){
        try{ Vibrator v=(Vibrator)getSystemService(VIBRATOR_SERVICE); if(v==null)return;
    ...
    private void createChannel(){
        if(Build.VERSION.SDK_INT>=26){
            NotificationChannel c=new NotificationChannel(CHANNEL,"MG Tennis Raket",NotificationManager.IMPORTANCE_LOW);
            c.setDescription("Raket telefonu sensör ve Bluetooth bağlantısı");
    ...
    private Notification notification(String text){
        ...
        Intent stop=new Intent(this,RacketService.class).setAction(ACTION_STOP);
        PendingIntent ps=PendingIntent.getService(this,1,stop,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);
        return new NotificationCompat.Builder(this,CHANNEL).setSmallIcon(android.R.drawable.ic_media_play)
                .setContentTitle("MG Tennis • Raket Modu").setContentText(text).setOngoing(true).setContentIntent(pi)
                .addAction(android.R.drawable.ic_menu_close_clear_cancel,"Durdur",ps).build();
    }
