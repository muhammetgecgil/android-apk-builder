...
'use strict';
const profiles={
  easy:{key:'easy',label:'KOLAY',shotTime:2.25,serveTime:1.72,reaction:720,runSpeed:3.2,accuracy:.46,errorRate:.18,power:.78},
...
let current=profiles.normal;
window.MGDifficulty={profiles,get current(){return current},set(key){current=profiles[key]||profiles.normal;update();return current}};
function update(){
  const e=document.getElementById('difficultySelect');if(e)e.value=current.key;
  const s=document.getElementById('status');if(s)s.textContent='Zorluk: '+current.label+' • rakip hız ve reaksiyon ayarlandı';
}
function mount(){
  ...
  sel.value=current.key;sel.addEventListener('change',()=>{current=profiles[sel.value]||profiles.normal;update()});
