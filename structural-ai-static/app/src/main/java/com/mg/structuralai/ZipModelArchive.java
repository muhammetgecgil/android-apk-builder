...
public final class ZipModelArchive {
    ...
    public static File extract(InputStream raw,String wanted,File cacheDir) throws IOException {
        ...
        try(ZipInputStream z=new ZipInputStream(new BufferedInputStream(raw));OutputStream none=OutputStream.nullOutputStream()){
            ZipEntry e;
            while((e=z.getNextEntry())!=null){
                ...
                String safe=safeName(e.getName());
                long entryBytes=0;
                OutputStream target=safe.equals(safeWanted)?new BufferedOutputStream(new FileOutputStream(out)):none;
                boolean closeTarget=target!=none;
                try{
