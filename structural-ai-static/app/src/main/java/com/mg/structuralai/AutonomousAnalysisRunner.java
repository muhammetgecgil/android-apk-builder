...
public final class AutonomousAnalysisRunner {
    ...
    public static Result run(MeshModel model){
        ...
        String capacityLine;
        if(idealReady&&p.materialConfidence<=0.0)capacityLine=String.format(Locale.US,"REFERENCE ONLY — NOT DESIGN ALLOWABLE — NOT CERTIFIED. Actual material is unknown, therefore real yield capacity is BLOCKED. Unit-load beam coefficient: sigma/1N=%.6g MPa/N. If, only as a normalization example, yield=%.6g MPa were assumed, the mathematical reference load would be %.6g N. Do not use this value for design or release.",ideal.maxBendingStressPa/1e6,p.material.yieldPa/1e6,p.material.yieldPa/ideal.maxBendingStressPa);
