...
public final class AssemblyOverlayRegressionGate {
    ...
    public static Result run(){
        try{
            ...
            boolean edgeOk=o.edges.size()>=2;boolean nearSeen=false,nearTransfers=false;
            for(AssemblyOverlayModel.Edge e:o.edges){if(e.style==AssemblyOverlayModel.EdgeStyle.INTERFERENCE)edgeOk=false;if(e.style==AssemblyOverlayModel.EdgeStyle.NEAR){nearSeen=true;nearTransfers|=e.loadTransfer;}}
            boolean nearBlockOk=!o.assemblyReady&&nearSeen&&!nearTransfers&&o.badgeText.contains("NEAR_CONTACT_REQUIRES_CLOSURE_EVIDENCE");
            ...
            boolean pass=bodyOk&&edgeOk&&nearBlockOk&&blockOk&&red;
            return new Result(pass,"ASSEMBLY OVERLAY REGRESSION "+(pass?"PASS":"FAIL")+" | bodies="+o.bodies.size()+" | edges="+o.edges.size()+" | nearBlocked="+nearBlockOk+" | interferenceBlock="+red);
