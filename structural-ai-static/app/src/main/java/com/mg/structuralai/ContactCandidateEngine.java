...
public final class ContactCandidateEngine {
    ...
    public static final class Pair {
        ...
        public final double bboxGap, sampledGap, normalOpposition, confidence;
        public final int sampledPairs;
        Pair(int a,int b,State s,double bg,double sg,double no,double c,int n){bodyA=a;bodyB=b;state=s;bboxGap=bg;sampledGap=sg;normalOpposition=no;confidence=c;sampledPairs=n;}
        public String summary(){return "body "+bodyA+" ↔ "+bodyB+" | "+state+" | bboxGap="+bboxGap+" | sampledGap="+sampledGap+" | opposedNormals="+normalOpposition+" | confidence="+confidence;}
    }
    ...
    public static Result analyze(MeshModel m,AssemblyBodyDecomposer.Result dec){
        ...
        for(int i=0;i<dec.bodies.size();i++)for(int j=i+1;j<dec.bodies.size();j++){
            ...
            double bg=bboxGap(a,b);
            if(bg>nearTol*3.0){out.add(new Pair(a.id,b.id,State.FAR,bg,bg,0,0.98,0));continue;}
            SampleResult sr=sample(m,a,b,96);
            State st;
            double conf;
            if(sr.minGap<=touchTol&&sr.opposition>=0.55){st=State.TOUCHING_OR_COINCIDENT;conf=clamp(0.70+0.25*sr.opposition);}
            else if(bg<=touchTol&&sr.minGap>nearTol){st=State.INTERFERENCE_SUSPECTED;conf=0.55;}
            ...
            out.add(new Pair(a.id,b.id,st,bg,sr.minGap,sr.opposition,conf,sr.count));
        }
