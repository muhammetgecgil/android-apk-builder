...
public final class ContactCandidateEngine {
    ...
    public static Result analyze(MeshModel m,AssemblyBodyDecomposer.Result dec){
        ...
        for(int i=0;i<dec.bodies.size();i++)for(int j=i+1;j<dec.bodies.size();j++){
            ...
            // A positive overlap on all three AABB axes above coincidence tolerance is conservative
            // interference evidence. A merely touching/separated face has zero AABB penetration on
            // at least one axis, so it must never be promoted to interference solely because a coarse
            // triangle sample missed the closest surface patch.
    ...
    private static SampleResult sample(MeshModel m,AssemblyBodyDecomposer.Body a,AssemblyBodyDecomposer.Body b,int maxPerBody){
        ...
        for(int ia:aa){Tri ta=tri(m,ia);for(int ib:bb){
            ...
            double eps=Math.max(1e-12,Math.max(d,r.minGap)*1e-7);
    ...
    /** Exact-enough Euclidean distance between two triangles: vertex-face and all edge-edge candidates. */
    private static double triangleDistance(Tri x,Tri y){
        double d=Double.POSITIVE_INFINITY;
        d=Math.min(d,pointTriangleDistance(x.a,y));d=Math.min(d,pointTriangleDistance(x.b,y));d=Math.min(d,pointTriangleDistance(x.c,y));
        d=Math.min(d,pointTriangleDistance(y.a,x));d=Math.min(d,pointTriangleDistance(y.b,x));d=Math.min(d,pointTriangleDistance(y.c,x));
        MeshModel.V3[][] ex={{x.a,x.b},{x.b,x.c},{x.c,x.a}},ey={{y.a,y.b},{y.b,y.c},{y.c,y.a}};
        for(MeshModel.V3[] a:ex)for(MeshModel.V3[] b:ey)d=Math.min(d,segmentSegmentDistance(a[0],a[1],b[0],b[1]));
        return d;
    }

    /** Real-Time Collision Detection closest-point-on-triangle formulation. */
    private static double pointTriangleDistance(MeshModel.V3 p,Tri t){
        MeshModel.V3 ab=sub(t.b,t.a),ac=sub(t.c,t.a),ap=sub(p,t.a);double d1=dot(ab,ap),d2=dot(ac,ap);
        if(d1<=0&&d2<=0)return dist(p,t.a);
        MeshModel.V3 bp=sub(p,t.b);double d3=dot(ab,bp),d4=dot(ac,bp);if(d3>=0&&d4<=d3)return dist(p,t.b);
        double vc=d1*d4-d3*d2;if(vc<=0&&d1>=0&&d3<=0){double v=d1/(d1-d3);return dist(p,add(t.a,scale(ab,v)));}
        MeshModel.V3 cp=sub(p,t.c);double d5=dot(ab,cp),d6=dot(ac,cp);if(d6>=0&&d5<=d6)return dist(p,t.c);
        double vb=d5*d2-d1*d6;if(vb<=0&&d2>=0&&d6<=0){double w=d2/(d2-d6);return dist(p,add(t.a,scale(ac,w)));}
        double va=d3*d6-d5*d4;if(va<=0&&(d4-d3)>=0&&(d5-d6)>=0){double w=(d4-d3)/((d4-d3)+(d5-d6));return dist(p,add(t.b,scale(sub(t.c,t.b),w)));}
        double denom=1.0/(va+vb+vc),v=vb*denom,w=vc*denom;return dist(p,add(t.a,add(scale(ab,v),scale(ac,w))));
    }

    /** Closest distance of two finite segments, robust for nearly parallel edges. */
    private static double segmentSegmentDistance(MeshModel.V3 p1,MeshModel.V3 q1,MeshModel.V3 p2,MeshModel.V3 q2){
        MeshModel.V3 d1=sub(q1,p1),d2=sub(q2,p2),r=sub(p1,p2);double a=dot(d1,d1),e=dot(d2,d2),f=dot(d2,r),s,t;
        if(a<=1e-30&&e<=1e-30)return dist(p1,p2);
        if(a<=1e-30){s=0;t=clamp01(f/e);}else{double c=dot(d1,r);if(e<=1e-30){t=0;s=clamp01(-c/a);}else{double b=dot(d1,d2),den=a*e-b*b;s=Math.abs(den)>1e-30?clamp01((b*f-c*e)/den):0;t=(b*s+f)/e;if(t<0){t=0;s=clamp01(-c/a);}else if(t>1){t=1;s=clamp01((b-c)/a);}}}
    ...
    private static MeshModel.V3 add(MeshModel.V3 a,MeshModel.V3 b){return new MeshModel.V3(a.x+b.x,a.y+b.y,a.z+b.z);}
    private static MeshModel.V3 scale(MeshModel.V3 a,double s){return new MeshModel.V3(a.x*s,a.y*s,a.z*s);}
    private static MeshModel.V3 cross(MeshModel.V3 a,MeshModel.V3 b){return new MeshModel.V3(a.y*b.z-a.z*b.y,a.z*b.x-a.x*b.z,a.x*b.y-a.y*b.x);}
