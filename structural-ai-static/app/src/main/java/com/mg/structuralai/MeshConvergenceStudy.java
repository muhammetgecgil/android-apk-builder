...
public final class MeshConvergenceStudy {
    ...
    public static Result run(MeshModel surface,double scale,LinearElasticMaterial mat,List<MeshModel.V3> supports,List<MeshModel.V3> loads,
        ...
        return run(surface,scale,mat,p,loads,fx,fy,fz,pressurePa,gravity,rho);
    }
    ...
    private static Level solveLevel(MeshModel surface,double scale,LinearElasticMaterial mat,Collection<AdvancedFemLoads.SupportPatch> supports,List<MeshModel.V3> loads,double fx,double fy,double fz,double pressurePa,boolean gravity,double rho,int cells){
