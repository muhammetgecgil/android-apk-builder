...
public final class CadImportGateway {
    ...
    public static ImportedModel read(String name,InputStream in) throws IOException {Format f=detect(name);if(f==Format.STL||f==Format.OBJ){MeshModel m=MeshParser.parse(name,in);return new ImportedModel(m,f,false,"single tessellated model","mesh format; no authoritative CAD assembly/material metadata");}if(f==Format.STEP||f==Format.IGES||f==Format.BREP){if(!NativeOcctBridge.isAvailable())throw new IllegalStateException("Exact "+f+" import requires the OCCT native CAD kernel; this APK build does not contain it yet.");throw new IllegalStateException("OCCT kernel is present but stream-to-native import adapter is not connected yet.");}throw new IllegalArgumentException("Unsupported CAD format: "+f+". P0: STEP, IGES, BREP, STL, OBJ.");}
}
