...
public final class AutoMainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        TextView sub=new TextView(this);sub.setText("v"+BuildConfig.VERSION_NAME+" • PRODUCTION MESH + ZIP CAD BROWSER • OCCT ARM64");sub.setTextColor(Color.LTGRAY);sub.setGravity(Gravity.CENTER);root.addView(sub);
        ...
        status=new TextView(this);status.setText("ZIP içinden STEP/STP/IGES/IGS/BREP/STL/OBJ + doğrudan CAD/mesh");status.setTextColor(Color.rgb(120,220,190));status.setPadding(4,8,4,5);root.addView(status);
    ...
    private void showZipEntryChooser(Uri uri,String zipName,List<String> entries){String[] items=entries.toArray(new String[0]);new AlertDialog.Builder(this).setTitle("ZIP içinden model seç").setMessage(zipName+" • "+entries.size()+" desteklenen model bulundu").setItems(items,(d,which)->{String entry=entries.get(which);status.setText("ZIP modeli açılıyor: "+entry);badge.setText("MODE: ZIP EXTRACT + CAD IMPORT");executor.submit(()->{try{extractAndImportZipEntry(uri,zipName,entry,false);}catch(Exception e){showImportBlocked(e);}});}).setNegativeButton("İptal",null).show();}
