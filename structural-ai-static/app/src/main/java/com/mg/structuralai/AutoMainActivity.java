...
public final class AutoMainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b);
        ...
        TextView sub=new TextView(this);sub.setText("v1.3.3 AUTO ALPHA • MODEL → AUTO ANALYZE");sub.setTextColor(Color.LTGRAY);sub.setGravity(Gravity.CENTER);root.addView(sub);
        ...
        ScrollView sc=new ScrollView(this);report=new TextView(this);report.setTextColor(Color.WHITE);report.setTextSize(12);report.setText("Otonom akış:\n1) Yalnız modeli seç\n2) AUTO ANALYZE\n3) Dahili cantilever regression\n4) Geometri QA\n5) Birim/malzeme belirsizliği yönetimi\n6) Mesnet/yük adayları\n7) Load-to-volume mapping QA\n8) 3-level FEM convergence\n9) Sıfır-cevap watchdog\n10) En güvenilir senaryo + confidence raporu\n\nRegression testi başarısız olursa kullanıcı modeli çözülmez; bozuk solver/mapping zinciriyle sonuç üretilmez.");sc.addView(report);root.addView(sc,new LinearLayout.LayoutParams(-1,0,1));setContentView(root);
    }
    ...
    private void runAuto(){if(model==null){status.setText("Önce model seç");return;}badge.setText("MODE: SELF TEST");badge.setBackgroundColor(Color.rgb(80,70,135));status.setText("Önce dahili FEM/load-mapping regression testi çalışıyor…");executor.submit(()->{try{
        ...
        runOnUiThread(()->{badge.setText("MODE: AUTONOMOUS ANALYSIS");badge.setBackgroundColor(Color.rgb(25,80,130));status.setText("Regression PASS → AI planlıyor → senaryoları çözüyor…");});
        AutonomousAnalysisRunner.Result r=AutonomousAnalysisRunner.run(model);MeshConvergenceStudy.Level f=r.scenario.convergence.fine;runOnUiThread(()->{viewport.setResult(f.mesh,f.fem);report.setText("IN-APP REGRESSION\n"+rg.summary+"\n\n"+r.report);status.setText(r.numericallyReady?"Otonom numerik analiz tamamlandı":"Otonom analiz QA tarafından bloke edildi");badge.setText(r.numericallyReady?"MODE: AUTO ANALYSIS COMPLETE":"MODE: AUTO QA BLOCKED");badge.setBackgroundColor(r.numericallyReady?Color.rgb(20,125,70):Color.rgb(145,45,45));});
