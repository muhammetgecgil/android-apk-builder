...
public final class AutoMainActivity extends Activity {
    ...
    private void runAuto(){if(model==null){status.setText("Önce model seç");return;}badge.setText("MODE: AUTONOMOUS ANALYSIS");badge.setBackgroundColor(Color.rgb(25,80,130));status.setText("AI planlıyor → senaryoları çözüyor → convergence kontrol ediyor…");executor.submit(()->{try{AutonomousAnalysisRunner.Result r=AutonomousAnalysisRunner.run(model);MeshConvergenceStudy.Level f=r.scenario.convergence.fine;runOnUiThread(()->{viewport.setResult(f.mesh,f.fem);report.setText(r.report);status.setText(r.numericallyReady?"Otonom numerik analiz tamamlandı":"Otonom analiz QA tarafından bloke edildi");badge.setText(r.numericallyReady?"MODE: AUTO ANALYSIS COMPLETE":"MODE: AUTO QA BLOCKED");badge.setBackgroundColor(r.numericallyReady?Color.rgb(20,125,70):Color.rgb(145,45,45));});}catch(Exception e){runOnUiThread(()->{badge.setText("MODE: AUTO ERROR");badge.setBackgroundColor(Color.rgb(145,45,45));status.setText("Otonom analiz durdu");report.setText("Hata: "+e.getMessage());});}});}
