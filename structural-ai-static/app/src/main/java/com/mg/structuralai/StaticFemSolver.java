...
/** Linear-static 3D FEM pipeline for TET4 meshes. SI units: m, N, Pa. Includes unilateral and Coulomb stick/slip contact. */
public final class StaticFemSolver {
    ...
    private static final class FrictionConstraint {
        final int a,b,normalOrdinal; final double nx,ny,nz,mu,factor;
        FrictionConstraint(int a,int b,MeshModel.V3 n,double mu,double factor,int normalOrdinal){
            this.a=a;this.b=b;double q=Math.sqrt(n.x*n.x+n.y*n.y+n.z*n.z);if(q<1e-20)throw new IllegalArgumentException("Friction normal is zero");nx=n.x/q;ny=n.y/q;nz=n.z/q;
            if(!Double.isFinite(mu)||mu<0||mu>2.0)throw new IllegalArgumentException("Friction coefficient outside [0,2]");this.mu=mu;this.factor=Math.max(10.0,Math.min(1e6,factor));this.normalOrdinal=normalOrdinal;
        }
    }
    private final TetMeshData mesh; private final LinearElasticMaterial material; private final boolean[] fixed; private final double[] force;
    private final List<Tie> bondedTies=new ArrayList<>(); private final List<NormalConstraint> normalConstraints=new ArrayList<>(); private final List<FrictionConstraint> frictionConstraints=new ArrayList<>();
    ...
    public Result solve(){
        ...
        for(int contactIt=0;contactIt<maxContactIter;contactIt++){
            ...
            boolean changed=false;int ui=0;for(NormalConstraint c:normalConstraints)if(c.unilateral){double du=relativeNormalDisplacement(c,u),totalGap=c.gapM+du;boolean next=active[ui];if(active[ui]&&du>openTol)next=false;else if(!active[ui]&&totalGap<-penetrationTol)next=true;if(next!=active[ui]){active[ui]=next;changed=true;}ui++;}
            for(int fi=0;fi<frictionConstraints.size();fi++){
                FrictionConstraint fc=frictionConstraints.get(fi);boolean normalActive=fc.normalOrdinal>=0&&fc.normalOrdinal<active.length&&active[fc.normalOrdinal];
                if(!normalActive){if(stick[fi]){stick[fi]=false;changed=true;}if(nonZero(slipForce[fi])){zero(slipForce[fi]);changed=true;}continue;}
                double[] rel=relativeVector(fc.a,fc.b,u);double un=fc.nx*rel[0]+fc.ny*rel[1]+fc.nz*rel[2];double tx=rel[0]-un*fc.nx,ty=rel[1]-un*fc.ny,tz=rel[2]-un*fc.nz;double ut=Math.sqrt(tx*tx+ty*ty+tz*tz);
    ...
    private void applyFrictionStick(SparsePcgSolver.Matrix K,double d,boolean[] active,boolean[] stick){for(int fi=0;fi<frictionConstraints.size();fi++){FrictionConstraint c=frictionConstraints.get(fi);if(!stick[fi]||c.normalOrdinal>=active.length||!active[c.normalOrdinal])continue;double kp=d*c.factor;double[][] P={{1-c.nx*c.nx,-c.nx*c.ny,-c.nx*c.nz},{-c.ny*c.nx,1-c.ny*c.ny,-c.ny*c.nz},{-c.nz*c.nx,-c.nz*c.ny,1-c.nz*c.nz}};for(int i=0;i<3;i++)for(int j=0;j<3;j++){double v=kp*P[i][j];int ai=3*c.a+i,aj=3*c.a+j,bi=3*c.b+i,bj=3*c.b+j;K.add(ai,aj,v);K.add(bi,bj,v);K.add(ai,bj,-v);K.add(bi,aj,-v);}}}
    private void applySlipForces(double[] rhs,boolean[] active,boolean[] stick,double[][] slipForce){for(int fi=0;fi<frictionConstraints.size();fi++){FrictionConstraint c=frictionConstraints.get(fi);if(stick[fi]||c.normalOrdinal>=active.length||!active[c.normalOrdinal])continue;double[] f=slipForce[fi];rhs[3*c.b]+=f[0];rhs[3*c.b+1]+=f[1];rhs[3*c.b+2]+=f[2];rhs[3*c.a]-=f[0];rhs[3*c.a+1]-=f[1];rhs[3*c.a+2]-=f[2];}}
    ...
    private static double[] relativeVector(int a,int b,double[] u){return new double[]{u[3*b]-u[3*a],u[3*b+1]-u[3*a+1],u[3*b+2]-u[3*a+2]};}
    private int unilateralCount(){int n=0;for(NormalConstraint c:normalConstraints)if(c.unilateral)n++;return n;}
    private double normalFactorForOrdinal(int ordinal){int ui=0;for(NormalConstraint c:normalConstraints)if(c.unilateral){if(ui==ordinal)return c.factor;ui++;}return 5e3;}
    ...
    private static boolean nonZero(double[] a){return Math.abs(a[0])+Math.abs(a[1])+Math.abs(a[2])>1e-20;}private static void zero(double[] a){a[0]=a[1]=a[2]=0;}
    private static double relativeChange(double[] a,double[] b){double da=Math.sqrt((a[0]-b[0])*(a[0]-b[0])+(a[1]-b[1])*(a[1]-b[1])+(a[2]-b[2])*(a[2]-b[2]));double nb=Math.max(Math.sqrt(b[0]*b[0]+b[1]*b[1]+b[2]*b[2]),1e-20);return da/nb;}
