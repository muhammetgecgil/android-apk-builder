...
public final class StaticFemSolver {
    ...
    public void addNodalForce(int node,double fx,double fy,double fz){
        addForce(3*node,fx); addForce(3*node+1,fy); addForce(3*node+2,fz);
    }
    ...
    public Result solve(){
        ...
        for(int n=0;n<mesh.nodes.size();n++){
            double ux=u[3*n],uy=u[3*n+1],uz=u[3*n+2];
            maxU=Math.max(maxU,Math.sqrt(ux*ux+uy*uy+uz*uz));
        }

        double extFx=0,extFy=0,extFz=0,rx=0,ry=0,rz=0;
        for(int n=0;n<mesh.nodes.size();n++){
            extFx+=force[3*n]; extFy+=force[3*n+1]; extFz+=force[3*n+2];
            if(fixed[3*n]||fixed[3*n+1]||fixed[3*n+2]){
                rx+=reactions[3*n]; ry+=reactions[3*n+1]; rz+=reactions[3*n+2];
            }
        }
        ...
        double eq=imbalance/applied;
        return new Result(u,reactions,vm,maxU,maxVm,eq,ls);
    }
    ...
    private SparsePcgSolver.Matrix assembleGlobal(){
        ...
        for(int[] t:mesh.tets){
            ...
            for(int a=0;a<4;a++) for(int ca=0;ca<3;ca++){
                ...
                for(int b=0;b<4;b++) for(int cb=0;cb<3;cb++){
                    int J=3*t[b]+cb, lj=3*b+cb;
                    K.add(I,J,er.stiffness[li][lj]);
                }
            }
        }
        return K;
    ...
    private static SparsePcgSolver.Matrix copy(SparsePcgSolver.Matrix a){
        SparsePcgSolver.Matrix b=new SparsePcgSolver.Matrix(a.n);
        for(int i=0;i<a.n;i++) for(int j=0;j<a.n;j++){ double v=a.get(i,j); if(Math.abs(v)>0) b.set(i,j,v); }
        return b;
    }
