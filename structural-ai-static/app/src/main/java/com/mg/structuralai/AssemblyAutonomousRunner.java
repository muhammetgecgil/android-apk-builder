...
public final class AssemblyAutonomousRunner {
    public static final class Result {
        public final TetMeshData mesh; public final StaticFemSolver.Result fem; public final LinearElasticMaterial material; public final boolean ready; public final String report;
        Result(TetMeshData m,StaticFemSolver.Result f,LinearElasticMaterial mat,boolean r,String s){mesh=m;fem=f;material=mat;ready=r;report=s;}
    }
    ...
    public static Result run(MeshModel source){
        ...
        for(MeshModel.V3 p:a.mesh.nodes){lo[0]=Math.min(lo[0],p.x);lo[1]=Math.min(lo[1],p.y);lo[2]=Math.min(lo[2],p.z);hi[0]=Math.max(hi[0],p.x);hi[1]=Math.max(hi[1],p.y);hi[2]=Math.max(hi[2],p.z);}
        ...
        for(int i=0;i<a.mesh.nodes.size();i++){MeshModel.V3 p=a.mesh.nodes.get(i);double q=axis==0?p.x:(axis==1?p.y:p.z);if(q<=lo[axis]+tol){solver.fixNode(i);fixed++;}if(q>=hi[axis]-tol)loaded++;}
        ...
        for(int i=0;i<a.mesh.nodes.size();i++){MeshModel.V3 p=a.mesh.nodes.get(i);double q=axis==0?p.x:(axis==1?p.y:p.z);if(q>=hi[axis]-tol){if(axis==2)solver.addNodalForce(i,0,each,0);else solver.addNodalForce(i,0,0,each);}}
