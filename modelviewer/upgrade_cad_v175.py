...
# Let Android picker show every file. We validate/route by extension ourselves.
c=c.replace('i.putExtra(Intent.EXTRA_MIME_TYPES,new String[]{"model/step","model/iges","model/obj","model/stl","application/octet-stream","text/plain","application/x-ply"});','// Keep */* so CAD files with vendor/generic MIME types are not hidden by Android.');
# Preserve exact extension as modelType for broad importer routing.
...
# Load assimpjs before the app code.
...
# Add generic Assimp JSON importer helpers before main().
...
async function loadViaAssimp(data){
  ...
  let rf=result.GetFile(0), txt=new TextDecoder().decode(rf.GetContent()), j=JSON.parse(txt);
...
# Route formats: OCCT for STEP/IGES/BREP, custom for OBJ/STL/PLY, Assimp for everything else.
oldseg="""const data=new Uint8Array(ab),params={linearUnit:'millimeter',linearDeflectionType:'bounding_box_ratio',linearDeflection:.0007,angularDeflection:.35};let result;if(fileType==='iges'||fileType==='igs')result=occt.ReadIgesFile(data,params);else if(fileType==='brep'||fileType==='brp')result=occt.ReadBrepFile(data,null);else result=occt.ReadStepFile(data,params);if(!result||!result.success)throw Error('CAD içe aktarma başarısız');(result.meshes||[]).forEach(addMesh);if(group.children.length===0)throw Error('Görüntülenecek yüzey bulunamadı');"""
newseg="""const data=new Uint8Array(ab),params={linearUnit:'millimeter',linearDeflectionType:'bounding_box_ratio',linearDeflection:.0007,angularDeflection:.35};let result;if(fileType==='iges'||fileType==='igs'){result=occt.ReadIgesFile(data,params);if(!result||!result.success)throw Error('IGES içe aktarma başarısız');(result.meshes||[]).forEach(addMesh);}else if(fileType==='brep'||fileType==='brp'){result=occt.ReadBrepFile(data,null);if(!result||!result.success)throw Error('BREP içe aktarma başarısız');(result.meshes||[]).forEach(addMesh);}else if(fileType==='step'||fileType==='stp'){result=occt.ReadStepFile(data,params);if(!result||!result.success)throw Error('STEP içe aktarma başarısız');(result.meshes||[]).forEach(addMesh);}else if(fileType==='obj'){parseObjUnified(new TextDecoder().decode(data)).forEach(addMesh);}else if(fileType==='stl'){parseStlUnified(ab).forEach(addMesh);}else if(fileType==='ply'){parsePlyUnified(new TextDecoder().decode(data)).forEach(addMesh);}else{await loadViaAssimp(ab);}if(group.children.length===0)throw Error('Görüntülenecek yüzey bulunamadı');"""
if oldseg not in h: raise SystemExit('v175 routing patch point not found')
h=h.replace(oldseg,newseg,1)
# Update empty workspace text.
...
# Fix markup/pen: add an always-visible floating pencil shortcut that toggles the existing markB.
pro=Path('modelviewer/src/main/assets/cadviewer/pro160.js')
# pro160.js is generated during build, so patch the generator source instead.
