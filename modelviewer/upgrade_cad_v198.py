...
if old not in s:
    raise SystemExit('v197 view menu hook not found')
...
if old not in s:
    raise SystemExit('v197 technical drawing button hook not found')
s=s.replace(old,new,1)
...
if old not in s:
    raise SystemExit('v197 feature marker not found')
s=s.replace(old,new,1)
p.write_text(s,encoding='utf-8')

# Native Android file picker: do not advertise PDF/doc/text as general files.
j=Path('modelviewer/src/main/java/com/muhammetgecgil/modelviewer/MainActivity.java')
t=j.read_text(encoding='utf-8')
old='void openFile(){Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType("*/*");i.putExtra(Intent.EXTRA_MIME_TYPES,new String[]{"model/step","model/iges","model/obj","model/stl","application/octet-stream","text/plain","application/x-ply"});startActivityForResult(i,OPEN_MODEL);}'
...
if old not in t:
    raise SystemExit('native file picker patch point not found')
t=t.replace(old,new,1)

# Reject accidental non-CAD selections even when a document provider ignores MIME filters.
...
if old not in t:
    raise SystemExit('loadModel CAD validation patch point not found')
t=t.replace(old,new,1)
j.write_text(t,encoding='utf-8')
print('v1.9.8 technical drawing menu + CAD-only picker patch applied')
