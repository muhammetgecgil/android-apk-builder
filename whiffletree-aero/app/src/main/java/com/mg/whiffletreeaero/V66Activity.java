...
public class V66Activity extends V65Activity {
  ...
  @Override void runPrimary(){super.runPrimary();update2DResult();}
  @Override void calculateAndShow(){super.calculateAndShow();update2DResult();}
  ...
  void update2DResult(){
    ...
    double maxR=0,maxM=0,maxD=0;String crit="-";
    for(CNode n:cnodes){double mm=Math.sqrt(n.mx*n.mx+n.my*n.my+n.mz*n.mz);if(n.r>maxR){maxR=n.r;crit=n.id;}maxM=Math.max(maxM,mm);maxD=Math.max(maxD,n.disp);}
  ...
  class Result2DView extends View{
    ...
    void label(Canvas c,String s,float x,float y,int col,float size){t.setColor(col);t.setTextSize(px(size));c.drawText(s,x,y,t);}
    @Override protected void onDraw(Canvas c){
      ...
      float left=dp(24),right=W-dp(24),tankY=dp(135);double len=Math.max(1,qd(qLength));int acts=Math.max(1,qi(qActs,1,12));
