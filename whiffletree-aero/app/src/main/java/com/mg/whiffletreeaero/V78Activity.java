...
public class V78Activity extends V76Activity {
  ...
  @Override public void onCreate(Bundle b){
    ...
    nav.addView(navBtn("GİRİŞ",v->showHome()),new LinearLayout.LayoutParams(0,dp(46),1));
    nav.addView(navBtn("2D RIG",v->showPanel(visualGuide)),new LinearLayout.LayoutParams(0,dp(46),1));
    nav.addView(navBtn("İSPAT",v->showPanel(matrixPanel)),new LinearLayout.LayoutParams(0,dp(46),1));
    nav.addView(navBtn("PARÇA",v->showPanel(equipPanel)),new LinearLayout.LayoutParams(0,dp(46),1));
    nav.addView(navBtn("TEST",v->showPanel(simPanel)),new LinearLayout.LayoutParams(0,dp(46),1));
    ...
    Button advanced=navBtn("GELİŞMİŞ GİRDİLER / TÜM MODÜLLER",v->showPanel(guided));proHome.addView(advanced,new LinearLayout.LayoutParams(-1,dp(50)));

    root.addView(proHome,0,lp());
    showHome();
  }

  Button navBtn(String s,View.OnClickListener l){Button b=new Button(this);b.setText(s);b.setTextSize(10);b.setAllCaps(false);b.setOnClickListener(l);return b;}
  ...
  void showHome(){hideAllInherited();if(proHome!=null)proHome.setVisibility(View.VISIBLE);}
  void showPanel(View target){hideAllInherited();if(proHome!=null)proHome.setVisibility(View.VISIBLE);if(target!=null)target.setVisibility(View.VISIBLE);}
  ...
  void calculateProfessional(){
    ...
    if(Double.isNaN(L)||Double.isNaN(D)||L<=0||D<=0||Double.isNaN(fx)||Double.isNaN(fy)||Double.isNaN(fz)||Double.isNaN(cap)||cap<=0){
      statusCard.setText("GİRİŞ KONTROLÜ: Kırmızı/boş alan bırakma. L, D ve kapasite > 0 olmalı; Fx/Fy/Fz pozitif veya negatif olabilir.");return;
    }
    ...
    if(solvedValid&&solved!=null&&!solved.isEmpty()){
      ...
      statusCard.setText("HESAP TAMAMLANDI — 2D RIG ile yük yolunu, İSPAT ile dengeyi, PARÇA ile ekipman seçimini incele.");
