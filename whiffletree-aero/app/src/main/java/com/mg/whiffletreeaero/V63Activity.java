...
public class V63Activity extends V62Activity {
  ...
  static class CNode { String id,type; int act,layer; double x,y,z,fx,fy,fz,r,mx,my,mz,disp,lc; }
  ...
  @Override void calculateAndShow(){ super.calculateAndShow(); if(solvedValid){buildConnections();connection2D.invalidate();connection3D.invalidate();} }

  void buildConnections(){
    cnodes.clear(); int layersN=Math.max(1,qi(qLayers,1,4)),acts=Math.max(1,qi(qActs,1,12)); double k=Math.max(1,qd(qStiffness));
    for(SNode s:solved){
      cnodes.add(make("Z"+(s.section+1),"PAD",s.act,0,s.x,s.y,s.z,s.fx,s.fy,s.fz,k));
      for(int l=1;l<=layersN;l++){ double blend=Math.pow(0.58,l); cnodes.add(make("L"+l+"-S"+(s.section+1),l==layersN?"PIVOT":"BEAM NODE",s.act,l,s.x*blend,s.y*blend,s.z-80*l,s.fx,s.fy,s.fz,k)); }
    }
    for(int a=0;a<acts;a++){
      ...
      cnodes.add(make("LC"+(a+1),"LOAD CELL",a,layersN,x*.25,y*.25,z,fx,fy,fz,k));
      cnodes.add(make("ACT"+(a+1),qActType.getSelectedItemPosition()==0?"HYDRAULIC ACTUATOR":"ELECTRIC ACTUATOR",a,layersN+1,x*.18,y*.18,z-140,fx,fy,fz,k));
    }
  ...
  CNode make(String id,String type,int act,int layer,double x,double y,double z,double fx,double fy,double fz,double k){ CNode n=new CNode();n.id=id;n.type=type;n.act=act;n.layer=layer;n.x=x;n.y=y;n.z=z;n.fx=fx;n.fy=fy;n.fz=fz;n.r=Math.sqrt(fx*fx+fy*fy+fz*fz);n.mx=y*fz-z*fy;n.my=z*fx-x*fz;n.mz=x*fy-y*fx;n.disp=n.r/k;n.lc=n.r;return n; }

  void inspect(CNode n){if(n==null)return;selectedConnection.setText(String.format(Locale.US,"%s — %s\nActuator path A%d • layer %d • XYZ %.1f / %.1f / %.1f mm\nFx %+,.1f N | Fy %+,.1f N | Fz %+,.1f N | R %.1f N\nMx %+,.1f | My %+,.1f | Mz %+,.1f Nmm\nDisplacement %.5f mm | load-cell basis %.1f N\nLoad path: EFT → pad → beam/pivot layers → load cell → actuator → strongback",n.id,n.type,n.act+1,n.layer,n.x,n.y,n.z,n.fx,n.fy,n.fz,n.r,n.mx,n.my,n.mz,n.disp,n.lc));}
  ...
  class Connection2DView extends View{
    ...
    @Override public boolean onTouchEvent(MotionEvent e){if(e.getAction()!=MotionEvent.ACTION_DOWN)return true;float best=1e9f;int bi=-1;for(int i=0;i<hp.size();i++){float dx=e.getX()-hp.get(i)[0],dy=e.getY()-hp.get(i)[1],dd=dx*dx+dy*dy;if(dd<best){best=dd;bi=i;}}if(bi>=0&&best<dp(40)*dp(40))inspect(hit.get(bi));return true;}
    @Override protected void onDraw(Canvas c){super.onDraw(c);hit.clear();hp.clear();t.setColor(Color.WHITE);t.setTextSize(dp(14));c.drawText("2D CONNECTION FORCE MAP",dp(14),dp(30),t);if(cnodes.isEmpty()){t.setTextSize(dp(9));c.drawText("Press HESAPLA VE GÖSTER.",dp(14),dp(65),t);return;}int W=getWidth();float left=dp(30),right=W-dp(30),tankY=dp(150);p.setColor(Color.rgb(68,80,92));c.drawRoundRect(new RectF(left,tankY-dp(34),right,tankY+dp(34)),dp(34),dp(34),p);double len=Math.max(1,qd(qLength));for(CNode n:cnodes){float x=(float)(left+(n.x+len/2)/len*(right-left));float y;if(n.type.equals("PAD"))y=tankY;else if(n.type.equals("LOAD CELL"))y=dp(620);else if(n.type.contains("ACTUATOR"))y=dp(740);else y=dp(260)+Math.max(0,n.layer-1)*dp(90);int col=n.type.equals("PAD")?Color.WHITE:n.type.equals("LOAD CELL")?Color.rgb(247,207,77):n.type.contains("ACTUATOR")?(qActType.getSelectedItemPosition()==0?Color.rgb(67,190,113):Color.rgb(160,110,230)):Color.rgb(51,205,220);p.setColor(col);c.drawCircle(x,y,dp(6),p);hit.add(n);hp.add(new float[]{x,y});t.setTextSize(dp(5));t.setColor(col);c.drawText(n.id,x-dp(10),y-dp(10),t);t.setTextSize(dp(5));c.drawText(String.format(Locale.US,"%+.0f/%+.0f/%+.0f N",n.fx,n.fy,n.fz),x-dp(24),y+dp(18),t);}t.setColor(Color.rgb(190,210,225));t.setTextSize(dp(7));c.drawText("Tap any node. Signed values are Fx/Fy/Fz [N].",dp(14),dp(850),t);}
  }
  ...
  class Connection3DView extends View{
    ...
    @Override public boolean onTouchEvent(MotionEvent e){if(e.getAction()!=MotionEvent.ACTION_DOWN)return true;float best=1e9f;int bi=-1;for(int i=0;i<hp.size();i++){float dx=e.getX()-hp.get(i)[0],dy=e.getY()-hp.get(i)[1],dd=dx*dx+dy*dy;if(dd<best){best=dd;bi=i;}}if(bi>=0&&best<dp(45)*dp(45))inspect(hit.get(bi));return true;}
    @Override protected void onDraw(Canvas c){super.onDraw(c);hit.clear();hp.clear();t.setColor(Color.WHITE);t.setTextSize(dp(14));c.drawText("3D XYZ CONNECTION / LOAD-PATH VIEW",dp(14),dp(30),t);if(cnodes.isEmpty()){t.setTextSize(dp(9));c.drawText("Press HESAPLA VE GÖSTER.",dp(14),dp(65),t);return;}int W=getWidth();float cx=W*.5f,cy=dp(250);double len=Math.max(1,qd(qLength)),sc=(W-dp(70))/len;CNode prev=null;for(CNode n:cnodes){float[] q=pr(n,cx,cy,sc);int col=n.type.equals("PAD")?Color.WHITE:n.type.equals("LOAD CELL")?Color.rgb(247,207,77):n.type.contains("ACTUATOR")?(qActType.getSelectedItemPosition()==0?Color.rgb(67,190,113):Color.rgb(160,110,230)):Color.rgb(51,205,220);p.setColor(col);c.drawCircle(q[0],q[1],dp(6),p);hit.add(n);hp.add(q);t.setColor(col);t.setTextSize(dp(5));c.drawText(n.id,q[0]+dp(6),q[1]-dp(5),t);if(prev!=null&&prev.act==n.act&&n.layer>=prev.layer){float[] a=pr(prev,cx,cy,sc);p.setStrokeWidth(dp(2));p.setColor(Color.rgb(130,150,165));c.drawLine(a[0],a[1],q[0],q[1],p);}prev=n;}t.setColor(Color.rgb(190,210,225));t.setTextSize(dp(7));c.drawText("White pad • cyan beam/pivot • yellow load-cell • green/purple actuator",dp(14),dp(920),t);}
  }
}
