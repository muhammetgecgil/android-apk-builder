...
public class V712Activity extends V711Activity {
  ...
  void designAutomaticWhiffletree(){
    ...
    // Direction compatibility: a single common lever tree is only physically meaningful when load vectors are approximately collinear.
    ...
    // Load-weighted contiguous actuator partition. It uses actual demand, not equal station count.
    ...
    for(int i=0;i<ss.size();i++){
      SNode s=ss.get(i);if(ai<na-1 && cum>0 && cum+s.r>target*(ai+1))ai++;
      s.act=ai;cum+=Math.max(1e-9,s.r);
      WtLeaf lf=new WtLeaf();lf.station=s.section;lf.act=ai;lf.x=s.x;lf.fx=s.fx;lf.fy=s.fy;lf.fz=s.fz;lf.r=s.r;lf.id="P"+(s.section+1);wtLeaves.add(lf);
    }

    // Build each actuator subtree by repeatedly merging adjacent demand nodes. Pivot arms are solved from child loads.
    for(int a=0;a<na;a++){
      ...
      while(nodes.size()>1 && level<=maxLayers){
        ...
        for(int i=0;i<nodes.size();i+=2){
          ...
          // For equilibrium FL*a = FR*b and a+b=span.
      ...
      // If requested layer count is insufficient, preserve remaining roots and flag architecture.
    ...
    // Closure against original station load set.
    ...
    autoWtSummary.setText(sb.toString());
    autoWtView.invalidate();
  }
  ...
  class AutoWtView extends View{
    ...
    void text(Canvas c,String s,float x,float y,int col,float sz){t.setColor(col);t.setTextSize(dp(sz));c.drawText(s,x,y,t);}void line(Canvas c,float x1,float y1,float x2,float y2,int col,float w){p.setColor(col);p.setStrokeWidth(dp(w));c.drawLine(x1,y1,x2,y2,p);}
    @Override protected void onDraw(Canvas c){super.onDraw(c);int W=getWidth();text(c,"AUTO UNEQUAL-LOAD WHIFFLETREE",dp(12),dp(28),Color.WHITE,13);if(wtLeaves.isEmpty()){text(c,"HESAPLA VE GÖSTER sonrası otomatik topoloji burada oluşur.",dp(12),dp(60),Color.LTGRAY,7);return;}double len=Math.max(1,val(hLen)*1000.0);float left=dp(28),right=W-dp(28),tankY=dp(110);p.setColor(Color.rgb(68,80,92));c.drawRoundRect(new RectF(left,tankY-dp(28),right,tankY+dp(28)),dp(26),dp(26),p);
      HashMap<String,float[]> pos=new HashMap<>();for(WtLeaf l:wtLeaves){float x=(float)(left+(l.x+len/2)/len*(right-left));pos.put(l.id,new float[]{x,tankY});p.setColor(Color.WHITE);c.drawCircle(x,tankY,dp(4),p);text(c,l.id,x-dp(7),tankY-dp(38),Color.WHITE,5.2f);text(c,String.format(Locale.US,"%.0fN",l.r),x-dp(10),tankY-dp(24),Color.rgb(205,220,230),4.8f);}
      ArrayList<WtBeam> bs=new ArrayList<>(wtBeams);Collections.sort(bs,(a,b)->Integer.compare(a.level,b.level));for(WtBeam b:bs){float[] pl=pos.get(b.leftId),pr=pos.get(b.rightId);if(pl==null||pr==null)continue;float x=(float)(left+(b.x+len/2)/len*(right-left));float y=tankY+dp(95*b.level);line(c,pl[0],pl[1],x,y,Color.rgb(145,165,180),1.5f);line(c,pr[0],pr[1],x,y,Color.rgb(145,165,180),1.5f);p.setColor(Color.rgb(220,145,35));c.drawRoundRect(new RectF(Math.min(pl[0],pr[0]),y-dp(5),Math.max(pl[0],pr[0]),y+dp(5)),dp(2),dp(2),p);p.setColor(Color.rgb(247,207,77));c.drawCircle(x,y,dp(5),p);text(c,String.format(Locale.US,"%.0f/%.0f",b.fl,b.fr),x-dp(16),y+dp(20),Color.WHITE,4.8f);text(c,String.format(Locale.US,"P %.0f%%",100*b.pivotRatio),x-dp(14),y+dp(34),Color.rgb(247,207,77),4.5f);pos.put(b.id,new float[]{x,y});}
      float base=dp(590);for(WtRoot r:wtRoots){float x=(float)(left+(r.x+len/2)/len*(right-left));p.setColor(Color.rgb(67,190,113));c.drawRoundRect(new RectF(x-dp(14),base-dp(38),x+dp(14),base+dp(20)),dp(5),dp(5),p);text(c,r.id,x-dp(13),base+dp(40),Color.WHITE,5.2f);text(c,String.format(Locale.US,"%.0fN",r.r),x-dp(13),base+dp(54),Color.rgb(247,207,77),4.8f);}
      text(c,wtArchitecture,dp(12),dp(685),wtDirectionSpread<=8?Color.rgb(90,220,120):(wtDirectionSpread<=25?Color.rgb(247,207,77):Color.rgb(235,90,80)),6.2f);
    }
  }
}
