...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b);buildWebView();}
    ...
    private void buildWebView(){
        ...
        webView.loadUrl(A+"index.html?v=217");
    }

    public final class Bridge {
        @JavascriptInterface public String getRecentTracks(){return getSharedPreferences("radio",MODE_PRIVATE).getString("tracks","[]");}
        @JavascriptInterface public String getTelemetry(){return getSharedPreferences("radio",MODE_PRIVATE).getString("telemetry","{}");}
    ...
    final class C extends WebViewClient {
        ...
        @Override public boolean onRenderProcessGone(WebView view,RenderProcessGoneDetail detail){try{view.destroy();}catch(Exception ignored){}new Handler(Looper.getMainLooper()).postDelayed(()->buildWebView(),250);return true;}
    }

    private void nativeCall(Uri u){
        String h=u.getHost()==null?"":u.getHost();Intent i;
        switch(h){
            case"play":String url=u.getQueryParameter("url"),n=u.getQueryParameter("name");if(url==null)return;i=new Intent(this,RadioService.class).setAction(RadioService.ACTION_PLAY).putExtra("url",url).putExtra("name",n);startFg(i);break;
            ...
            case"vol":startService(new Intent(this,RadioService.class).setAction(RadioService.ACTION_VOLUME).putExtra("volume",f(u.getQueryParameter("v"),1)));break;
            case"gain":startService(new Intent(this,RadioService.class).setAction(RadioService.ACTION_GAIN).putExtra("gain",q(u.getQueryParameter("mb"),0)));break;
            case"eq":startService(new Intent(this,RadioService.class).setAction(RadioService.ACTION_EQ).putExtra("band",q(u.getQueryParameter("band"),0)).putExtra("level",q(u.getQueryParameter("level"),0)));break;
            case"normalize":startService(new Intent(this,RadioService.class).setAction(RadioService.ACTION_NORMALIZE).putExtra("on","1".equals(u.getQueryParameter("on"))));break;
            case"smooth":startService(new Intent(this,RadioService.class).setAction(RadioService.ACTION_SMOOTH).putExtra("on","1".equals(u.getQueryParameter("on"))));break;
            case"shazam":try{Intent z=getPackageManager().getLaunchIntentForPackage("com.shazam.android");startActivity(z!=null?z:new Intent(Intent.ACTION_VIEW,Uri.parse("https://www.shazam.com/")));}catch(Exception ignored){}break;
            case"alarmsettings":if(Build.VERSION.SDK_INT>=31)try{startActivity(new Intent(Settings.ACTION_REQUEST_SCHEDULE_EXACT_ALARM,Uri.parse("package:"+getPackageName())));}catch(Exception ignored){}break;
            case"alarm":alarm(u,false);break;case"sleep":alarm(u,true);break;case"alarmclear":case"sleepclear":cancel(u);break;
        }
    }
    ...
    static int q(String s,int d){try{return Integer.parseInt(s);}catch(Exception e){return d;}}
    static long l(String s,long d){try{return Long.parseLong(s);}catch(Exception e){return d;}}
    static float f(String s,float d){try{return Float.parseFloat(s);}catch(Exception e){return d;}}
