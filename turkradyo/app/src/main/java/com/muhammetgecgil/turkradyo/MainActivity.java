...
public class MainActivity extends Activity {
    ...
    private void buildWebView(){
        ...
        webView.loadUrl(A+"index.html?v=288");
    }
    ...
    public final class Bridge {
        ...
        @JavascriptInterface public void setQueue(String json,int idx){
            String q=json==null?"[]":json;
            SharedPreferences p=getSharedPreferences("radio",MODE_PRIVATE);
            String old=p.getString("queue","[]");
            p.edit().putString("queue",q).putInt("queueIndex",Math.max(0,idx)).apply();
            if(!q.equals(old)){CatalogHealthManager.scanAsync(MainActivity.this,q);CatalogHealthManager.autoSweepIfDue(MainActivity.this,q);}
        }
