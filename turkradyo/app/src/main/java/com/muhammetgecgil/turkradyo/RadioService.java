...
public class RadioService extends Service {
    ...
    private static final long REBUFFER_TIMEOUT_MS=30_000L;
    ...
    private long playStartMs=0, startupMs=0, preparedAtMs=0, serviceStartMs=0, lastNetworkChangeMs=0, lastTrackMs=0, lastRecoveryMs=0, lastBufferStartMs=0;
    ...
    /** Do not tear down a healthy player merely because Wi-Fi/cellular changed. */
    private void onNetworkChanged(String next,String why){
        ...
        if("offline".equals(nextNet)){
            ...
            if(!userPaused&&!primaryUrl.isEmpty()){
                ...
                updateNotification("İnternet bekleniyor",true);
            }
            return;
        }
        ...
        networkRecoveryTask=()->{
            ...
            if(state==Player.STATE_IDLE||state==Player.STATE_ENDED||!playWhenReady){
                ...
                updateNotification("Ağ geçişi: "+oldNet+" → "+nextNet,true);
                saveTelemetry();
            }
        };
    ...
    private ExoPlayer buildPlayer(){
        ...
        DefaultLoadControl loadControl=new DefaultLoadControl.Builder()
                .setBufferDurationsMs(12_000,50_000,1_000,3_000)
    ...
    private void playResolved(String url){
        ...
        startForeground(NOTIF_ID,buildNotification("Bağlanıyor…",true));
        try{
            final ExoPlayer ep=buildPlayer();
            player=ep;
            playerListener=new Player.Listener(){
                @Override public void onPlaybackStateChanged(int state){
                    ...
                    if(state==Player.STATE_BUFFERING){
                        ...
                        if(startupMs<=0)startupMs=Math.max(1,System.currentTimeMillis()-playStartMs);
                        if(preparedAtMs<=0)preparedAtMs=System.currentTimeMillis();
                        StreamFallbackManager.markGood(RadioService.this,stationName,streamUrl,startupMs);
                        saveRecent();saveCurrent();saveTelemetry();
                        if(!userPaused)ep.play();
                    }else if(state==Player.STATE_ENDED&&!userPaused){
                        lastError=-43;saveTelemetry();handleFailure("Yayın sona erdi");
                    }
                }
    ...
    private void recordTrack(String title,String source){
        title=cleanTrackTitle(title);if(title.isEmpty())return;
        long now=System.currentTimeMillis();
        if(title.equalsIgnoreCase(lastTrackTitle)&&now-lastTrackMs<90_000L)return;
        lastTrackTitle=title;lastTrackMs=now;
        try{
            SharedPreferences p=getSharedPreferences("radio",MODE_PRIVATE);
            JSONArray old;try{old=new JSONArray(p.getString("tracks","[]"));}catch(Exception e){old=new JSONArray();}
            JSONArray out=new JSONArray();
            JSONObject n=new JSONObject();n.put("title",title);n.put("station",stationName);n.put("time",now);n.put("source",source);out.put(n);
            for(int i=0;i<old.length()&&out.length()<50;i++){JSONObject x=old.optJSONObject(i);if(x!=null)out.put(x);}
            p.edit().putString("tracks",out.toString()).putString("nowTitle",title).apply();
            updateMediaSession(player!=null&&player.isPlaying(),title);
        }catch(Exception ignored){}
    }
    ...
    private void handleFailure(String label){
        ...
        long now=System.currentTimeMillis();
        if(recoveryBusy&&now-lastRecoveryMs<1200L)return;
        recoveryBusy=true;lastRecoveryMs=now;
        cancelWatchdog();cancelStableReset();
        updateMediaSession(false,label);updateNotification(label,true);
        if("offline".equals(networkType)){recoveryBusy=false;return;}

        if(sameSourceRetries<MAX_SAME_SOURCE_RETRIES){
            sameSourceRetries++;engineRestarts++;
            long delay=sameSourceRetries==1?900L:2200L;
            schedulePlay(streamUrl.isEmpty()?primaryUrl:streamUrl,delay);
            return;
        }

        StreamFallbackManager.markBad(this,stationName,streamUrl,5*60_000L);
        if(reconnectAttempts<1){
            reconnectAttempts++;
            String next=StreamFallbackManager.getPreferred(this,stationName,primaryUrl);
            schedulePlay(next,1200L);
            return;
        }
        repairSameStation();
    }
    ...
    private void schedulePlay(String u,long delay){
        cancelReconnect();
        reconnectTask=()->{recoveryBusy=false;if(!userPaused)playResolved(u);};
        handler.postDelayed(reconnectTask,delay);
    }

    private void armStartupWatchdog(final String expected){
        cancelWatchdog();
        watchdogTask=()->{
            if(userPaused||player==null||!expected.equals(streamUrl))return;
            int state;try{state=player.getPlaybackState();}catch(Exception e){state=Player.STATE_IDLE;}
            if(state!=Player.STATE_READY){lastError=-31;saveTelemetry();handleFailure("Geç bağlantı");}
        };
        handler.postDelayed(watchdogTask,STARTUP_TIMEOUT_MS);
    }
    ...
    private void armBufferWatchdog(){
        ...
        handler.postDelayed(watchdogTask,REBUFFER_TIMEOUT_MS);
    }

    private void armStableReset(){
        cancelStableReset();
        stableTask=()->{
        ...
        handler.postDelayed(stableTask,STABLE_RESET_MS);
    }
    ...
    private void cancelTasks(){cancelReconnect();cancelWatchdog();cancelStableReset();if(networkRecoveryTask!=null){handler.removeCallbacks(networkRecoveryTask);networkRecoveryTask=null;}if(fadeTask!=null){handler.removeCallbacks(fadeTask);fadeTask=null;}}
    ...
    private void saveTelemetry(){
        try{
            ...
            getSharedPreferences("radio",MODE_PRIVATE).edit().putString("telemetry",o.toString()).apply();
        }catch(Exception ignored){}
    }

    private void updateMediaSession(boolean playing,String subtitle){
        if(mediaSession==null)return;
        long acts=PlaybackState.ACTION_PLAY|PlaybackState.ACTION_PAUSE|PlaybackState.ACTION_PLAY_PAUSE|PlaybackState.ACTION_STOP|PlaybackState.ACTION_SKIP_TO_NEXT|PlaybackState.ACTION_SKIP_TO_PREVIOUS|PlaybackState.ACTION_PLAY_FROM_MEDIA_ID|PlaybackState.ACTION_PLAY_FROM_SEARCH;
        int state=playing?PlaybackState.STATE_PLAYING:(userPaused?PlaybackState.STATE_PAUSED:PlaybackState.STATE_CONNECTING);
        mediaSession.setPlaybackState(new PlaybackState.Builder().setActions(acts).setState(state,PlaybackState.PLAYBACK_POSITION_UNKNOWN,playing?1f:0f).build());
        String now=getSharedPreferences("radio",MODE_PRIVATE).getString("nowTitle","");
        mediaSession.setMetadata(new MediaMetadata.Builder().putString(MediaMetadata.METADATA_KEY_TITLE,stationName).putString(MediaMetadata.METADATA_KEY_ARTIST,now.isEmpty()?subtitle:now).putString(MediaMetadata.METADATA_KEY_ALBUM,"Türk Radyo").build());
    }
    ...
    private void resume(){
        PlaybackGuardian.manualPlay(this);userPaused=false;
        if(player!=null){
            try{
                int state=player.getPlaybackState();
    ...
    private void releasePlayer(){
        ...
        if(player!=null){
            try{if(playerListener!=null)player.removeListener(playerListener);}catch(Exception ignored){}
            try{player.release();}catch(Exception ignored){}
            player=null;playerListener=null;
        }
    }
