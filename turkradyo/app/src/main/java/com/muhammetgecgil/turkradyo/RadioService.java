...
public class RadioService extends Service implements MediaPlayer.OnPreparedListener, MediaPlayer.OnErrorListener, MediaPlayer.OnInfoListener {
    ...
    @Override public int onStartCommand(Intent in,int flags,int id){
        ...
        if(ACTION_PLAY.equals(a)){
            ...
            if(u!=null&&!u.isEmpty()){PlaybackGuardian.manualPlay(this);primaryUrl=u;stationName=(n==null||n.isEmpty())?"Türk Radyo":n;reconnectAttempts=0;repairBusy=false;userPaused=false;resumeAfterFocus=false;syncQueueIndexForUrl(u);playResolved(StreamFallbackManager.getPreferred(this,stationName,primaryUrl));}
