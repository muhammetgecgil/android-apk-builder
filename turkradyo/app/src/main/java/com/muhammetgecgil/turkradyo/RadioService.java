...
public class RadioService extends Service implements MediaPlayer.OnPreparedListener, MediaPlayer.OnErrorListener, MediaPlayer.OnInfoListener, MediaPlayer.OnTimedMetaDataAvailableListener {
    ...
    private void onNetworkChanged(String next,String why){
        ...
        if("offline".equals(nextNet)){if(!userPaused&&!primaryUrl.isEmpty()){PlaybackGuardian.interrupted(this,"network");updateMediaSession(false,"İnternet bekleniyor");updateNotification("İnternet bekleniyor",true);}return;}
        if(userPaused||primaryUrl.isEmpty())return;
        if(networkRecoveryTask!=null)handler.removeCallbacks(networkRecoveryTask);
        networkRecoveryTask=()->{if(userPaused)return; boolean playing=false;try{playing=player!=null&&player.isPlaying()&&!buffering;}catch(Exception ignored){}if(!playing){reconnectAttempts=0;playResolved(StreamFallbackManager.getPreferred(this,stationName,primaryUrl));}else{if(PlaybackGuardian.mayAutoResume(this))PlaybackGuardian.recovered(this);updateNotification("Ağ geçişi: "+oldNet+" → "+nextNet,true);saveTelemetry();}};
    ...
    private String extractTimedTitle(byte[] raw){
        if(raw==null||raw.length==0)return"";String text;
        try{text=new String(raw,StandardCharsets.UTF_8).replace('\u0000',' ').trim();}catch(Exception e){return"";}
        Matcher m=STREAM_TITLE.matcher(text);if(m.find())return cleanTrackTitle(m.group(1));
        int i=text.indexOf("TIT2");if(i>=0){String t=text.substring(i+4).replaceAll("[\\x00-\\x1F]+"," ").trim();int cut=t.length();for(String marker:new String[]{"TPE1","TALB","TRCK","TCON","COMM","TYER","TDRC"}){int p=t.indexOf(marker);if(p>=0&&p<cut)cut=p;}if(cut>0)return cleanTrackTitle(t.substring(0,cut));}
        return"";
    }
    ...
    private void resume(){PlaybackGuardian.manualPlay(this);resumeAfterFocus=false;userPaused=false;if(player!=null)try{player.start();updateMediaSession(true,"Canlı yayın");updateNotification("Canlı yayın",true);armSilentWatchdog();}catch(Exception e){playResolved(StreamFallbackManager.getPreferred(this,stationName,primaryUrl));}else if(!primaryUrl.isEmpty())playResolved(StreamFallbackManager.getPreferred(this,stationName,primaryUrl));}
    private void autoResumeAfterFocus(){if(userPaused||!resumeAfterFocus||!PlaybackGuardian.mayAutoResume(this))return;resumeAfterFocus=false;if(player!=null)try{player.start();PlaybackGuardian.recovered(this);updateMediaSession(true,"Canlı yayın");updateNotification("Canlı yayın",true);armSilentWatchdog();}catch(Exception e){playResolved(StreamFallbackManager.getPreferred(this,stationName,primaryUrl));}else if(!primaryUrl.isEmpty())playResolved(StreamFallbackManager.getPreferred(this,stationName,primaryUrl));}
