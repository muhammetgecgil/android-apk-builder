...
public class RadioService extends Service implements MediaPlayer.OnPreparedListener, MediaPlayer.OnErrorListener, MediaPlayer.OnInfoListener {
    ...
    private Runnable reconnectTask, watchdogTask, fadeTask;
    ...
    private float volume=1f; private int gainMb=0, reconnectAttempts=0, bufferCount=0, lastError=0; private long playStartMs=0, startupMs=0;
    ...
    private void playResolved(String url){
        if(url==null||url.isEmpty())url=primaryUrl;streamUrl=url;playStartMs=System.currentTimeMillis();startupMs=0;buffering=false;lastError=0;cancelTasks();releasePlayer();saveTelemetry();saveCurrent();requestFocus();updateMediaSession(false,"Bağlanıyor…");startForeground(NOTIF_ID,buildNotification("Bağlanıyor…",true));
    ...
    @Override public void onPrepared(MediaPlayer mp){
        startupMs=Math.max(1,System.currentTimeMillis()-playStartMs);reconnectAttempts=0;repairBusy=false;buffering=false;cancelWatchdog();
        try{mp.start();applyGain();applyEq();if(smooth)fadeIn(mp);else mp.setVolume(volume,volume);StreamFallbackManager.markGood(this,stationName,streamUrl,startupMs);saveRecent();saveCurrent();saveTelemetry();updateMediaSession(true,"Canlı yayın");updateNotification("Canlı yayın",true);}catch(Exception e){lastError=-2;handleFailure("Başlatılamadı");}
    }
    ...
    @Override public boolean onInfo(MediaPlayer mp,int what,int extra){if(what==MediaPlayer.MEDIA_INFO_BUFFERING_START){buffering=true;bufferCount++;saveTelemetry();armBufferWatchdog();}else if(what==MediaPlayer.MEDIA_INFO_BUFFERING_END){buffering=false;cancelWatchdog();saveTelemetry();}return false;}

    private void handleFailure(String label){
        if(userPaused)return;updateMediaSession(false,label);updateNotification(label,true);StreamFallbackManager.markBad(this,stationName,streamUrl,reconnectAttempts<1?60_000L:30*60_000L);
    ...
    private void armStartupWatchdog(final String expected){cancelWatchdog();watchdogTask=()->{if(!userPaused&&player!=null&&startupMs==0&&expected.equals(streamUrl)){lastError=-31;handleFailure("Geç bağlantı");}};handler.postDelayed(watchdogTask,6500);}
    private void armBufferWatchdog(){cancelWatchdog();watchdogTask=()->{if(buffering&&!userPaused){lastError=-30;handleFailure("Uzun buffer");}};handler.postDelayed(watchdogTask,9000);}
    ...
    private void cancelTasks(){cancelReconnect();cancelWatchdog();if(fadeTask!=null){handler.removeCallbacks(fadeTask);fadeTask=null;}}
    ...
    private void resume(){userPaused=false;if(player!=null)try{player.start();updateMediaSession(true,"Canlı yayın");updateNotification("Canlı yayın",true);}catch(Exception e){playResolved(StreamFallbackManager.getPreferred(this,stationName,primaryUrl));}else if(!primaryUrl.isEmpty())playResolved(StreamFallbackManager.getPreferred(this,stationName,primaryUrl));}
