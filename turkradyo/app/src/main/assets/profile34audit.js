...
const H={s:[],b:[],k:[]};function push(a,v){a.push(v);if(a.length>30)a.shift()}function pts(a,w=400,h=120){let mx=Math.max(...a,1);return a.map((v,i)=>((i/Math.max(1,a.length-1))*w).toFixed(1)+','+(h-10-(v/mx)*(h-20)).toFixed(1)).join(' ')}
let lastSample='';function update(){if(document.hidden||!document.body.classList.contains('profile3'))return;let s=cur(),t=tele();if(!s)return;wire();let a=$('#p3Latency'),b=$('#p3Latency2');if(a)a.textContent=t.startupMs?t.startupMs+' ms':'ölçülüyor';if(b)b.textContent=String(t.bufferCount||0);let sample=[t.startupMs||0,t.bufferCount||0,s.bitrate||0,t.buffering?1:0,t.lastError||0].join('|');if(sample!==lastSample||H.s.length<2){push(H.s,+t.startupMs||0);push(H.b,+t.bufferCount||0);push(H.k,+s.bitrate||0);lastSample=sample}let svg=$('#p3Exact .graph svg');if(svg){let l=svg.querySelectorAll('polyline');if(l[0])l[0].setAttribute('points',pts(H.s));if(l[1])l[1].setAttribute('points',pts(H.b));if(l[2])l[2].setAttribute('points',pts(H.k));}}
window.refreshProfile3Telemetry=update;
window.addEventListener('load',()=>setTimeout(()=>{wire();update()},500),{once:true});
setInterval(()=>{if(!document.hidden&&document.body.classList.contains('profile3'))update()},1000);
})();