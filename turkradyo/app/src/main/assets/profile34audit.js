...
function update(){if(document.hidden||!document.body.classList.contains('profile3'))return;let s=cur(),t=tele();if(!s)return;wire();let a=$('#p3Latency'),b=$('#p3Latency2');if(a)a.textContent=t.startupMs?t.startupMs+' ms':'ölçülüyor';if(b)b.textContent=String(t.bufferCount||0);push(H.s,+t.startupMs||0);push(H.b,+t.bufferCount||0);push(H.k,+s.bitrate||0);let svg=$('#p3Exact .graph svg');if(svg){let l=svg.querySelectorAll('polyline');if(l[0])l[0].setAttribute('points',pts(H.s));if(l[1])l[1].setAttribute('points',pts(H.b));if(l[2])l[2].setAttribute('points',pts(H.k));}}
window.refreshProfile3Telemetry=update;
window.addEventListener('load',()=>setTimeout(()=>{wire();update()},900),{once:true});
setInterval(()=>{if(!document.hidden&&document.body.classList.contains('profile3'))update()},5000);
})();