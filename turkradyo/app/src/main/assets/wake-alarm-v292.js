...
function open(){
 const state=read();let current=null;try{current=stations[index]}catch{}
 ...
 $('#p2AlarmSet').onclick=()=>{
  ...
  try{const result=JSON.parse(window.RadioNative.setWakeAlarm(h,m,station.url,station.name,$('#p292AlarmDaily').checked));if(result.savedAt)localStorage.trWakeAlarm292=JSON.stringify(result);refresh();toast(result.error|| (result.scheduled?'Alarm '+value+' için kuruldu':'Saat kaydedildi; alarm iznini aç'))}catch{toast('Alarm kurulamadı; yeniden dene')}
 };
