...
async function api9(path){for(const a of API9){try{const d=await radioFetchJson(a+path);if(Array.isArray(d))return d}catch(e){}}return []}
...
function setHealth9(score,text,state=''){const el=healthEl9();if(!el)return;const cls='v9Health'+(state?' '+state:'');if(el.className!==cls)el.className=cls;const label=score===null?text:'BAĞLANTI '+Math.max(0,Math.min(100,Math.round(score)))+'/100 • '+text;const span=el.querySelector('span');if(span.textContent!==label)span.textContent=label}
...
async function heal9(reason){const s=cur9();if(!s)return;await refreshPool9(s.name);if(cur9()!==s||paused)return;if(playAlt9(reason))return;setHealth9(45,'KAYNAK ARANIYOR','warn');const d=await api9('/json/stations/byname/'+encodeURIComponent(s.name)+'?hidebroken=true&limit=200');if(cur9()!==s||paused)return;const cand=d.filter(x=>{const u=x.url_resolved||x.url;return u&&norm9(x.name)===norm9(s.name)&&!BAD9.test([x.name,x.tags,x.language].join(' '))&&!quarantined9(u)}).map(x=>({...x,_url:x.url_resolved||x.url})).sort((a,b)=>score9(b)-score9(a))[0];if(cand&&cand._url&&cand._url!==s._url){quarantine9(s._url);s._url=cand._url;s.bitrate=cand.bitrate||s.bitrate;lastSwitch=Date.now();try{play();setHealth9(92,'YENİ KAYNAK BULUNDU');try{toast('Self-healing yeni yayın kaynağı buldu')}catch(e){}}catch(e){}}else setHealth9(38,'ALTERNATİF BEKLENİYOR','warn')}
...
if(t.serviceActive===false){setHealth9(null,'YAYIN HAZIR','idle');return}
if(t.manualPause||paused){setHealth9(null,'YAYIN DURAKLATILDI','idle');return}
if(t.buffering){setHealth9(null,'YAYINA BAĞLANIYOR','warn');return}
if(!t.isPlaying){setHealth9(null,'BAĞLANTI BEKLENİYOR','idle');return}
