...
function boot(){[0,160,520,1100].forEach(ms=>setTimeout(patch,ms));document.addEventListener('click',e=>{const b=e.target.closest(SEL);if(!b||!isP2())return;e.preventDefault();e.stopImmediatePropagation();if(!fireZap())setTimeout(fireZap,180)},true);document.addEventListener('click',e=>{if(e.target.closest('[data-prof]')){setTimeout(patch,50);setTimeout(patch,300);setTimeout(patch,900)}},true);window.addEventListener('pageshow',()=>setTimeout(patch,0));document.addEventListener('visibilitychange',()=>{if(!document.hidden)setTimeout(patch,0)})}
