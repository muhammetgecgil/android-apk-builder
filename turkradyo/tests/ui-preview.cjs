...
(async()=>{
 ...
 await page.addInitScript(()=>{localStorage.setItem('p2Active','1');window.RadioNative={getWakeAlarm:()=>JSON.stringify({time:'06:45',name:'Power Türk',url:'https://stream.test/0',daily:true,enabled:true,scheduled:true,status:'scheduled',exactAllowed:true,savedAt:Date.now(),when:Date.now()+3600000}),getSleepTimer:()=>JSON.stringify({when:Date.now()+29*60000+42000,fade:true,exactAllowed:true}),getTelemetry:()=>JSON.stringify({nativeRecovery:true,serviceActive:true,isPlaying:false,manualPause:true,playerState:3,networkType:'wifi',volume:0.75,station:'Power Türk',primaryUrl:'https://stream.test/0'}),getNowTitle:()=>'',getRecentTracks:()=>'[]',getStreamHealth:()=>'{}',getProductGuard:()=>'{}',getCatalogHealth:()=>'{}',getNotaAnalysis:()=>'{"state":"IDLE"}',getNotaTextArchive:()=>'[]',setQueue:()=>{}}});
