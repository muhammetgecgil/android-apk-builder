...
public class Stage8Activity extends Activity {
 ...
 private void refresh(){if(status!=null)status.setText("● Katalog indiriliyor • kör geçmiş kayıtları korunuyor");new Thread(()->{try{TurkeyAnalyzer.Report rep=TurkeyAnalyzer.fetchAndAnalyze();String x=Stage8MetricsEngine.report(this,rep.eventsJson);runOnUiThread(()->{live.setText(x);status.setText("● "+rep.eventCount+" katalog olayı • Stage 8 kalibrasyon analizi güncel");});}catch(Exception e){String x=Stage8MetricsEngine.report(this,"[]");runOnUiThread(()->{live.setText(x);status.setText("● Canlı katalog alınamadı • yalnız yerel passport metrikleri gösteriliyor");});}}).start();}
