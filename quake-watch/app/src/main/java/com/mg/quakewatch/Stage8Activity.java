...
public class Stage8Activity extends Activity {
 ...
 private void build(){
  ...
  TextView s=t("CALIBRATION LAB • ERROR GEOMETRY • MAGNITUDE TIERS",11,C);s.setPadding(dp(16),0,dp(16),dp(5));root.addView(s);
  status=t("● Türkiye kataloğu ve passport kayıtları analiz ediliyor",11,G);status.setPadding(dp(16),0,dp(16),dp(9));root.addView(status);
 ...
 private void refresh(){if(status!=null)status.setText("● Katalog indiriliyor • kör geçmiş kayıtları korunuyor");new Thread(()->{try{TurkeyAnalyzer.Report rep=TurkeyAnalyzer.fetchAndAnalyze();String x=CalibrationMetrics.report(this,rep.eventsJson);runOnUiThread(()->{live.setText(x);status.setText("● "+rep.eventCount+" katalog olayı • Stage 8 kalibrasyon analizi güncel");});}catch(Exception e){String x=CalibrationMetrics.report(this,"[]");runOnUiThread(()->{live.setText(x);status.setText("● Canlı katalog alınamadı • yalnız yerel passport metrikleri gösteriliyor");});}}).start();}
