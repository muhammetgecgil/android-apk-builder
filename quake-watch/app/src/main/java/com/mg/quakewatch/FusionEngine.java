...
public final class FusionEngine {
    ...
    public static final class Context {
        ...
        public final double tidalProxy;
        public final double magneticUt;
        public final double infrastructureIndex;
        public final double fusionIndex;
    ...
    public static Context fetch(double seismicScore,double magneticUt) throws Exception {
        ...
        // Ayrı iki kavramı karıştırmıyoruz: deprem sinyali sismik modelden gelir;
        // çevresel/altyapı endeksi ise uzay hava durumu + gelgit + cihaz manyetometresi bağlamıdır.
        double infrastructure=clamp(100.0*(0.72*clamp(kp/9.0)+0.18*tide+0.10*magAnomaly));
        double fusion=clamp(0.78*seismicScore + 0.22*infrastructure);
        ...
        else if(kp>=5) s.append("• Orta-yüksek jeomanyetik aktivite: GNSS/HF doğruluğu ve bazı uzay-hava bağımlı sistemler izlenmeli.\n");
        else s.append("• Jeomanyetik koşullar düşük/orta seviyede.\n");
        if(tide>0.8) s.append("• Ay-Güneş gelgit geometrisi kuvvetli fazda; kıyı, liman ve deniz operasyonlarında yerel gelgit verisi ayrıca kontrol edilmeli.\n");
        s.append("• Tsunami riski yalnız deprem büyüklüğü, derinliği, mekanizması ve deniz tabanı yer değiştirmesiyle değerlendirilir; bu endeks tek başına tsunami tahmini değildir.\n");
    ...
    private static double tidalProxy(){
        // 2000-01-06 18:14 UTC yaklaşık yeni ay referansı, sinodik ay 29.530588 gün.
        double days=(System.currentTimeMillis()-947182440000L)/86400000.0;
        double phase=(days%29.530588)/29.530588;
        if(phase<0) phase+=1.0;
        // Yeni/dolunayda 1, dördünlerde 0.35 civarı: yalnız fiziksel gelgit potansiyeli bağlamı.
        return 0.35+0.65*Math.abs(Math.cos(2.0*Math.PI*phase));
    }

    private static double magneticContext(double uT){
        if(uT<=0) return 0;
        // Dünya yüzeyinde tipik alan yaklaşık 25–65 µT; telefon yakınındaki mıknatıs/metal alanı bozabilir.
        if(uT>=25 && uT<=65) return 0.1;
        double d=uT<25?(25-uT):(uT-65);
        return clamp(d/100.0);
    }
    ...
    private static double clampScore(double x){return clamp100(x);}
