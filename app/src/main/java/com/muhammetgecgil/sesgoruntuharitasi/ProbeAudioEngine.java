...
/** V8.7: S24 reference + Fyvadio probe with dedicated 96 Hz narrow-band measurement. */
public final class ProbeAudioEngine {
    public static final int MODE_USB_ABSOLUTE=0,MODE_USB_MINUS_REF=1,MODE_BAND_SCAN=2;
    public static final int BAND_ALL=0,BAND_LOW=1,BAND_VOICE=2,BAND_HIGH=3,BAND_96=4;
    public static final int SAMPLE_RATE=48000;
    private static final int FRAMES=1920;
    public interface Listener{void onProbe(Snapshot s);}
    public static final class Snapshot{
        public final long timestampNs; public final float usbDbfs,refDbfs,deltaDb,usbRms01,refRms01,bandEnergy01,bandDbfs,refNoiseFloorDb;
        public final boolean usbActive,phoneRefActive,dualLive,refStable;
        public final String usbName,refName,status;
        Snapshot(long t,float u,float r,float d,float ur,float rr,float be,float bd,float nf,boolean ua,boolean pa,boolean dl,boolean rs,String un,String rn,String st){timestampNs=t;usbDbfs=u;refDbfs=r;deltaDb=d;usbRms01=ur;refRms01=rr;bandEnergy01=be;bandDbfs=bd;refNoiseFloorDb=nf;usbActive=ua;phoneRefActive=pa;dualLive=dl;refStable=rs;usbName=un;refName=rn;status=st;}
    }
    ...
    public void setBand(int b){band=Math.max(0,Math.min(4,b));} public int getBand(){return band;}
    ...
    public void start(){if(running.getAndSet(true))return;thread=new Thread(this::loop,"ProbeFusionAudio87");thread.start();}
    ...
    private void loop(){
        ...
        if(usb==null){emit(-120,-120,0,0,0,0,-120,-120,false,false,false,false,usbName,refName,"USB mikrofon bağlı değil");running.set(false);return;}
        boolean refCal=false;if(builtIn!=null){Cal c=measureReference(builtIn);if(c.valid){calibratedRefDb=c.level;refNoiseFloorDb=c.floor;refCal=true;}}
        usbRec=build(usb);refRec=build(builtIn);boolean usbOk=initialized(usbRec),refOk=initialized(refRec);
        if(!usbOk){release();emit(-120,calibratedRefDb,0,0,0,0,-120,refNoiseFloorDb,false,refCal,false,refCal,usbName,refName,"USB AudioRecord açılamadı");running.set(false);return;}
        ...
        while(running.get()){
            ...
            float rawRef=dual&&rf!=null?rf.dbfs:calibratedRefDb;float jump=Math.abs(rawRef-smoothRef);float alpha=jump>6f?.035f:.12f;smoothRef=smoothRef*(1-alpha)+rawRef*alpha;boolean stable=jump<8f;
            ...
            String bandText=band==BAND_96?"96Hz "+String.format(Locale.US,"%.1f dB",uf.bandDb):"BAND";
            String st=(dual?"DUAL LIVE":"REF CAL")+" • "+bandText+" • S24 REF "+String.format(Locale.US,"%.1f dBFS",smoothRef)+" • FLOOR "+String.format(Locale.US,"%.1f",refNoiseFloorDb)+(stable?" • STABLE":" • GAIN BEKLE");
            emit(uf.dbfs,smoothRef,smoothDelta,uf.rms01,rf==null?0:rf.rms01,mapEnergy,uf.bandDb,refNoiseFloorDb,true,dual||refCal,dual,stable,usbName,refName,st);
        }
    ...
    private static final class Features{float dbfs,rms01,band01,bandDb;Features(float d,float r,float b,float bd){dbfs=d;rms01=r;band01=b;bandDb=bd;}}
    private static Features features(short[] x,int n,int band){double s=0;for(int i=0;i<n;i++){double v=x[i]/32768.0;s+=v*v;}double rms=Math.sqrt(s/Math.max(1,n));float db=(float)(20*Math.log10(Math.max(1e-7,rms)));
        if(band==BAND_96){double e=goertzel(x,n,96);double amp=2.0*Math.sqrt(Math.max(0,e))/Math.max(1,n)/32768.0;float bd=(float)(20*Math.log10(Math.max(1e-7,amp)));float bi=clamp01((bd+80f)/70f);return new Features(db,clamp01((float)rms*9f),bi,bd);}
        int[] f={100,200,400,800,1200,2000,3500,6000,10000};int lo=0,hi=f.length;if(band==BAND_LOW){hi=3;}else if(band==BAND_VOICE){lo=2;hi=7;}else if(band==BAND_HIGH){lo=6;}double e=0;for(int i=lo;i<hi;i++)e+=goertzel(x,n,f[i]);e/=Math.max(1,hi-lo);double amp=2.0*Math.sqrt(Math.max(0,e))/Math.max(1,n)/32768.0;float bd=(float)(20*Math.log10(Math.max(1e-7,amp)));return new Features(db,clamp01((float)rms*9f),clamp01((bd+80f)/70f),bd);}
    ...
    private void emit(float u,float r,float d,float ur,float rr,float be,float bd,float nf,boolean ua,boolean pa,boolean dl,boolean rs,String un,String rn,String st){if(listener!=null)listener.onProbe(new Snapshot(System.nanoTime(),u,r,d,ur,rr,be,bd,nf,ua,pa,dl,rs,un,rn,st));}
