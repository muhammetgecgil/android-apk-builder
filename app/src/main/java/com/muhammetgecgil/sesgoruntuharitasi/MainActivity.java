...
/** V9.1: robust source finder + persistent dynamic black-probe tracker. */
public final class MainActivity extends Activity {
    ...
    private float probeX=.5f,probeY=.65f,probeR=.08f,visionConf=0f;
    private boolean visionValid=false,probeFrozen=true,probeMoving=false,reacquiring=true,scanning=false,finished=false;
    ...
    private void buildUi(){
        ...
        overlay.setTargetListener((x,y)->{
            probeX=x;probeY=y;vision.seed(x,y);visionValid=false;probeFrozen=true;probeMoving=false;reacquiring=true;
            Toast.makeText(this,"İşaret alındı • prob dış çevresi ve merkezi otomatik oturtuluyor",Toast.LENGTH_SHORT).show();
        });
        ...
        TextView title=label("SES GÖRÜNTÜ HARİTASI V9.1 • PERSISTENT DYNAMIC TRACK",15,true);title.setTextColor(Color.WHITE);top.addView(title);
        statusText=label("Prob kendi bulunur • istersen bir kez dokun • merkez + dış çevre sürekli takip",10,false);statusText.setTextColor(Color.CYAN);top.addView(statusText);
        ...
        scanButton=button("TARAMAYI BAŞLAT");Button auto=button("PROBU OTOMATİK BUL");Button reset=button("YENİ TARAMA");
        ...
        scanButton.setOnClickListener(v->{
            if(!scanning&&!finished){
                if(!visionValid){Toast.makeText(this,"Prob otomatik aranıyor. İstersen ekranda probun merkezine bir kez dokun.",Toast.LENGTH_LONG).show();return;}
        ...
        auto.setOnClickListener(v->{vision.autoFind();visionValid=false;probeFrozen=true;reacquiring=true;Toast.makeText(this,"Tam görüntüde prob otomatik aranıyor",Toast.LENGTH_SHORT).show();});
    ...
    private final Runnable visionLoop=new Runnable(){@Override public void run(){
        if(cameraView!=null&&cameraView.isAvailable()){
            try{
                Bitmap b=cameraView.getBitmap(192,320);ProbeVisionTracker.Result r=vision.track(b);if(b!=null)b.recycle();
                probeX=r.x01;probeY=r.y01;probeR=r.radius01;visionConf=r.confidence;visionValid=r.valid;probeFrozen=r.frozen;probeMoving=r.moving;reacquiring=r.reacquiring;
                overlay.setTracker(probeX,probeY,probeR,visionConf,visionValid,probeFrozen,reacquiring);
            }catch(Exception ignored){}
        }
        // ~30-40 Hz request cadence; TextureView delivery/CPU load determines actual rate.
    ...
    private void onProbe(ProbeAudioEngine.Snapshot s){
        ...
        runOnUiThread(()->{
            ...
            String phase=finished?"SONUÇ HAZIR":scanning?"TARAMA AKTİF":visionValid?"PROB KİLİTLİ":"PROB ARANIYOR";
            ...
            String track=visionValid?String.format(Locale.US,"DİNAMİK TAKİP %d%% • R=%.3f",Math.round(visionConf*100),probeR):reacquiring?"TAKİP KAÇTI • DAİRE DONDU • OTOMATİK YENİDEN ARIYOR":"PROB ARANIYOR";
