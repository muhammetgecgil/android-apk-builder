...
public final class MainActivity extends Activity {
    ...
    // Variability history: prefer live S24 reference. USB history is only trusted while the probe is visually stationary.
    ...
    @Override protected void onCreate(Bundle b){
        ...
        getWindow().setStatusBarColor(Color.rgb(4,12,18));
        getWindow().setNavigationBarColor(Color.rgb(4,12,18));
        buildUi();
    ...
    private void buildUi(){
        ...
        overlay.setPark(PARK_X,PARK_Y,PARK_R);
        ...
        Button mode=button("AUTO KAYNAK");bandButton=button("BAND: TÜM");Button cal=button("REF KALİBRE");Button clear=button("HARİTA SİL");
        r1.addView(mode,weight());r1.addView(bandButton,weight());r1.addView(cal,weight());r1.addView(clear,weight());
        LinearLayout r2=new LinearLayout(this);r2.setOrientation(LinearLayout.HORIZONTAL);bottom.addView(r2);
        scanButton=button("TARAMAYI BAŞLAT");Button auto=button("PROB: DAİRE");Button reset=button("YENİ TARAMA");
        ...
        scanButton.setOnClickListener(v->{
            if(!scanning&&!finished){
                if(!parked){Toast.makeText(this,"Önce probu sol alttaki başlangıç dairesine getir",Toast.LENGTH_LONG).show();return;}
                resetHistories();overlay.clearMap();overlay.beginScan();scanning=true;finished=false;scanButton.setText("TARAMAYI BİTİR");
            }else if(scanning){
                scanning=false;finished=true;overlay.finishScan();scanButton.setText("YENİ TARAMA");
                Toast.makeText(this,overlay.isHomogeneous()?"Tarama bitti • alan büyük ölçüde homojen":"Tarama bitti • kaynak adayı gerçek ölçüm noktalarından hesaplandı",Toast.LENGTH_LONG).show();
            }else resetScan();
        });
        auto.setOnClickListener(v->{vision.seed(probeX,probeY);visionValid=true;probeFrozen=false;probeMoving=false;});
        reset.setOnClickListener(v->resetScan());
    }
    ...
    private void startAll(){
        ...
        audio.setMode(ProbeAudioEngine.MODE_USB_MINUS_REF);audio.setBand(ProbeAudioEngine.BAND_ALL);audio.start();
    ...
    private final Runnable visionLoop=new Runnable(){@Override public void run(){
        if(cameraView!=null&&cameraView.isAvailable()){
            try{
                ...
                probeX=r.x01;probeY=r.y01;probeR=r.radius01;visionConf=r.confidence;visionValid=r.valid;probeFrozen=r.frozen;probeMoving=r.moving;
                float dx=probeX-PARK_X,dy=probeY-PARK_Y;parked=visionValid&&!probeFrozen&&(dx*dx+dy*dy)<PARK_R*PARK_R;
                overlay.setTracker(probeX,probeY,probeR,visionConf,visionValid,probeFrozen);overlay.setParked(parked);
    ...
    private void onProbe(ProbeAudioEngine.Snapshot s){
        ...
        // Correct lesson from V8.7: do NOT classify source variability from a moving roaming probe.
        if(s.dualLive){
            push(refHistory,s.refDbfs,48);
            if(refHistory.size()>=16)variableSource=robustSpread(refHistory)>2.2f;
            variableUncertain=false;
        }else{
            if(!probeMoving&&visionValid&&!probeFrozen&&phoneStable){push(stationaryUsbHistory,s.usbDbfs,48);}
            if(stationaryUsbHistory.size()>=20){
                variableSource=robustSpread(stationaryUsbHistory)>2.8f;
                variableUncertain=variableSource;
            }
        }

        float measure;
        if(audio!=null&&audio.getBand()!=ProbeAudioEngine.BAND_ALL){
            measure=s.bandDbfs;
        }else if(variableSource&&s.dualLive){
            measure=s.deltaDb; // temporal loudness change largely cancels out here
        }else{
            measure=s.usbDbfs;
        }

        // Robust short median suppresses one-frame spikes. In variable/no-dual mode require longer persistence.
        push(mapMedianHistory,measure,variableSource&&!s.dualLive?15:5);
        float robustMeasure=median(mapMedianHistory);
        boolean canWrite=scanning&&visionValid&&!probeFrozen&&phoneStable&&s.refStable;
        if(variableSource&&!s.dualLive)canWrite=canWrite&&!probeMoving&&mapMedianHistory.size()>=9;
        if(canWrite)overlay.updateProbe(probeX,probeY,robustMeasure,s.usbActive);

        final float fm=robustMeasure;
        runOnUiThread(()->{
            usbText.setText(String.format(Locale.US,"USB PROBE\n%.1f dBFS",s.usbDbfs));
            refText.setText(String.format(Locale.US,"PHONE REF\n%.1f dBFS",s.refDbfs));
            deltaText.setText(String.format(Locale.US,"HARİTA\n%.1f dB",fm));
            imuText.setText(String.format(Locale.US,"IMU\n%.0f%%",motion*100f));
            String phase=finished?"SONUÇ HAZIR":scanning?"TARAMA AKTİF":parked?"PROB HAZIR":"PROBU SOL ALTA GETİR";
            String src=!variableSource?"SABİT/KARARLI SES":s.dualLive?"DEĞİŞKEN SES • S24 CANLI TELAFİ":"DEĞİŞKEN SES • DUAL YOK • YAVAŞ/BEKLEMELİ ÖLÇÜM";
            if(variableUncertain)src+=" • GÜVEN DÜŞÜK";
            ...
            String write=scanning?(canWrite?"ÖLÇÜM ✓":"ÖLÇÜM BEKLE"):"ÖLÇÜM KAPALI";
            statusText.setText(phase+" • "+src+" • "+track+" • "+write+"\n"+s.status);
        });
    }
    ...
    private void cycleBand(){
        if(audio==null)return;int n=(audio.getBand()+1)%5;audio.setBand(n);
        bandButton.setText(n==0?"BAND: TÜM":n==1?"BAND: DÜŞÜK":n==2?"BAND: KONUŞMA":n==3?"BAND: TİZ":"96 Hz TEST");
        audio.setMode(n==0?ProbeAudioEngine.MODE_USB_MINUS_REF:ProbeAudioEngine.MODE_BAND_SCAN);
    }
