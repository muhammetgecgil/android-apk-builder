...
/** V8.4: fixed lower-left park target, scan-gated sampling, and source reveal only in final mode. */
public final class HeatmapOverlayView extends View {
    ...
    public synchronized void updateProbe(float x01,float y01,float energy01,float deltaDb,boolean active){
        if(!active||finalMode||!scanActive)return;
        ...
        float ow=weight[i],add=.8f+.2f*conf;value[i]=ow<=0?level:(value[i]*ow+level*add)/(ow+add);weight[i]=Math.min(40f,ow+add);anchor[i]=true;samples++;lastDelta=deltaDb;postInvalidateOnAnimation();
    }
    ...
    public synchronized void finishScan(){
        ...
        finalMode=true;
        for(int i=0;i<N;i++)finalHeat[i]=anchor[i]?value[i]:.06f;
        float[] tmp=new float[N];
        for(int pass=0;pass<14;pass++){
            System.arraycopy(finalHeat,0,tmp,0,N);
            for(int y=0;y<ROWS;y++)for(int x=0;x<COLS;x++){
                int i=y*COLS+x;if(anchor[i])continue;float s=finalHeat[i]*3f,w=3f;
                if(x>0){s+=finalHeat[i-1];w++;}if(x<COLS-1){s+=finalHeat[i+1];w++;}if(y>0){s+=finalHeat[i-COLS];w++;}if(y<ROWS-1){s+=finalHeat[i+COLS];w++;}
                tmp[i]=s/w;
            }
    ...
    private void recomputeBest(){best=-1;bestValue=-1;for(int i=0;i<N;i++){float v=finalHeat[i];if(v>bestValue){bestValue=v;best=i;}}}
    ...
    @Override protected synchronized void onDraw(Canvas c){
        ...
        for(int y=0;y<ROWS;y++)for(int x=0;x<COLS;x++){
            ...
            fill.setColor(color(v,finalMode?165:185));c.drawRect(x*cw,y*ch,(x+1)*cw+1,(y+1)*ch+1,fill);
        }
    ...
    private void drawPark(Canvas c){
        float x=parkX*getWidth(),y=parkY*getHeight(),r=parkR*Math.min(getWidth(),getHeight());
        line.setStrokeWidth(dp(3));line.setColor(parked?Color.GREEN:Color.YELLOW);c.drawCircle(x,y,r,line);c.drawCircle(x,y,dp(5),line);
        text.setColor(line.getColor());c.drawText(parked?"PROB HAZIR":"BAŞLANGIÇ • PROBU BURAYA GETİR",Math.max(dp(8),x-r),Math.max(dp(20),y-r-dp(8)),text);
    }

    private void drawProbe(Canvas c){
        float x=targetX*getWidth(),y=targetY*getHeight(),r=targetR*Math.min(getWidth(),getHeight());
        line.setStrokeWidth(dp(2.6f));line.setColor(valid?Color.CYAN:Color.WHITE);c.drawCircle(x,y,r,line);c.drawCircle(x,y,dp(4),line);c.drawLine(x-dp(14),y,x+dp(14),y,line);c.drawLine(x,y-dp(14),x,y+dp(14),line);
        text.setColor(line.getColor());String s=scanActive?String.format(Locale.US,"PROB MERKEZ • Δ %+.1f dB",lastDelta):"PROB MERKEZ";c.drawText(s,Math.min(getWidth()-dp(155),x+dp(16)),Math.max(dp(45),y-dp(10)),text);
    }

    private void drawSource(Canvas c,float cw,float ch){
        int bx=best%COLS,by=best/COLS;RectF r=new RectF(Math.max(0,(bx-2)*cw),Math.max(0,(by-2)*ch),Math.min(getWidth(),(bx+3)*cw),Math.min(getHeight(),(by+3)*ch));
        line.setStrokeWidth(dp(bestValue>.86f?4:3));line.setColor(bestValue>.86f?Color.RED:Color.YELLOW);c.drawRoundRect(r,dp(8),dp(8),line);
        String s=(bestValue>.86f?"KAYNAK BULUNDU":"MUHTEMEL KAYNAK")+" • %"+Math.round(bestValue*100);
        fill.setColor(Color.argb(210,0,0,0));c.drawRoundRect(r.left,Math.max(0,r.top-dp(26)),Math.min(getWidth(),r.left+dp(190)),r.top,dp(6),dp(6),fill);text.setColor(line.getColor());c.drawText(s,r.left+dp(5),Math.max(dp(15),r.top-dp(7)),text);
    }

    private void drawState(Canvas c){
        String s=finalMode?"NİHAİ HARİTA • KAYNAK SONUCU":scanActive?String.format(Locale.US,"TARAMA AKTİF • %d örnek",samples):parked?"HAZIR • TARAMAYI BAŞLAT":"BEKLEME • PROB SOL ALTTA OLMALI";
        fill.setColor(Color.argb(175,0,0,0));c.drawRoundRect(dp(8),dp(8),Math.min(getWidth()-dp(8),dp(275)),dp(35),dp(7),dp(7),fill);text.setColor(Color.WHITE);c.drawText(s,dp(14),dp(27),text);
    }
