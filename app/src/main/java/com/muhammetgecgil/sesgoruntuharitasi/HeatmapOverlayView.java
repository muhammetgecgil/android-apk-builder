...
/** V8.8: robust P10-P90 colour range, minimum meaningful span, IDW display fill, measured-anchor-only source localization. */
public final class HeatmapOverlayView extends View {
    ...
    private boolean valid=false,parked=false,finalMode=false,scanActive=false,frozen=false,homogeneous=false;
    ...
    public synchronized void updateProbe(float x01,float y01,float measuredDb,boolean active){
        ...
        recomputeRobustRange(false);postInvalidateOnAnimation();
    }
    ...
    public synchronized void finishScan(){
        ...
        // Display-only IDW interpolation. Real anchors are preserved exactly.
        ...
        recomputeRobustRange(false); // colour range comes from REAL measured anchors only
        computeSourceFromMeasuredAnchors();
        invalidate();
    }

    private void recomputeRobustRange(boolean unused){
        ArrayList<Float> vals=new ArrayList<>();
        for(int i=0;i<N;i++)if(anchor[i])vals.add(raw[i]);
        ...
        int i10=Math.max(0,Math.min(n-1,(int)Math.floor(.10*(n-1))));
        int i90=Math.max(0,Math.min(n-1,(int)Math.ceil(.90*(n-1))));
        rangeLow=vals.get(i10);rangeHigh=vals.get(i90);
        homogeneous=(rangeHigh-rangeLow)<MIN_MEANINGFUL_SPAN_DB;
    }
    ...
    private void computeSourceFromMeasuredAnchors(){
        ...
        Collections.sort(vals);int n=vals.size();
        float threshold=vals.get(Math.max(0,(int)Math.floor(.90*(n-1))));
        double sx=0,sy=0,sw=0;float maxDb=-999f;int topCount=0;
        for(int i=0;i<N;i++){
            if(!anchor[i]||raw[i]<threshold)continue;
            int x=i%COLS,y=i/COLS;float excess=Math.max(.15f,raw[i]-threshold+.15f);float w=excess*(.5f+.5f*confidence[i]);
            sx+=(x+.5)*w;sy+=(y+.5)*w;sw+=w;topCount++;if(raw[i]>maxDb)maxDb=raw[i];
        }
        if(sw<=0){sourceX=sourceY=-1;sourceConfidence=0;return;}
        sourceX=(float)(sx/sw);sourceY=(float)(sy/sw);sourceDb=maxDb;
        float span=Math.max(0f,rangeHigh-rangeLow);
        float spanScore=clamp((span-1.2f)/5.0f);float countScore=clamp(observed/25f);float clusterScore=clamp(topCount/8f);
        sourceConfidence=clamp(.55f*spanScore+.30f*countScore+.15f*clusterScore);
        if(homogeneous)sourceConfidence=Math.min(sourceConfidence,.28f);
    }
    ...
    private float norm(float db){
        ...
        if(span<MIN_MEANINGFUL_SPAN_DB){
            // Do not manufacture a full rainbow from tiny/noisy differences.
            float center=(rangeLow+rangeHigh)*.5f;
            return clamp(.5f+(db-center)/MIN_MEANINGFUL_SPAN_DB*.18f);
        }
    ...
    private void drawSource(Canvas c,float cw,float ch){
        ...
        text.setColor(line.getColor());String s=String.format(Locale.US,"KAYNAK ADAYI • güven %d • %.1f dB",Math.round(sourceConfidence*100),sourceDb);c.drawText(s,r.left+dp(5),Math.max(dp(16),r.top-dp(7)),text);
    }
    private void drawState(Canvas c){
        ...
        String s=finalMode?(homogeneous?"NİHAİ • ALAN HOMOJEN"+range:"NİHAİ • ROBUST P10-P90"+range):scanActive?"TARAMA • "+samples+" örnek"+range:parked?"HAZIR • BAŞLAT":"PROB SOL ALTTA OLMALI";
        fill.setColor(Color.argb(175,0,0,0));c.drawRoundRect(dp(8),dp(8),Math.min(getWidth()-dp(8),dp(335)),dp(35),dp(7),dp(7),fill);text.setColor(Color.WHITE);c.drawText(s,dp(14),dp(27),text);
    }
    private void drawLegend(Canvas c){
        int left=(int)dp(10),right=Math.min(getWidth()-(int)dp(10),left+(int)dp(230)),top=getHeight()-(int)dp(16),h=(int)dp(7);for(int x=left;x<right;x++){float v=(x-left)/(float)Math.max(1,right-left-1);fill.setColor(rainbow(v,220));c.drawRect(x,top,x+1,top+h,fill);}
    }

    @Override public boolean onTouchEvent(MotionEvent e){
        if(e.getAction()==MotionEvent.ACTION_DOWN||e.getAction()==MotionEvent.ACTION_MOVE){targetX=clamp(e.getX()/Math.max(1f,getWidth()));targetY=clamp(e.getY()/Math.max(1f,getHeight()));valid=true;frozen=false;conf=.90f;if(listener!=null)listener.onTarget(targetX,targetY);invalidate();return true;}return true;
    }
