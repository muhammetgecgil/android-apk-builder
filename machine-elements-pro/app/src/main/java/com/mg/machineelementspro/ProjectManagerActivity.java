...
public class ProjectManagerActivity extends Activity {
    ...
    private View build(){
        ...
        r.addView(txt("PROJECT MANAGER",24,true));r.addView(txt("M80.2 • çoklu proje, revizyon, aktif proje ve bağlı eleman modeli",14,false));
    ...
    private View card(EngineeringProject p,boolean active){LinearLayout c=new LinearLayout(this);c.setOrientation(LinearLayout.VERTICAL);c.setPadding(dp(14),dp(12),dp(14),dp(12));c.setBackgroundColor(Color.WHITE);LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(-1,-2);cp.topMargin=dp(10);c.setLayoutParams(cp);c.addView(txt((active?"★ AKTİF • ":"")+p.name+" • Rev "+p.revision,17,true));c.addView(txt("ID: "+p.id+" • Eleman: "+p.elements.size(),13,false));StringBuilder b=new StringBuilder();for(EngineeringProject.Element e:p.elements)b.append(e.type).append(" • ").append(e.id).append('\n');c.addView(txt(b.length()==0?"Bağlı eleman yok":b.toString().trim(),13,false));
