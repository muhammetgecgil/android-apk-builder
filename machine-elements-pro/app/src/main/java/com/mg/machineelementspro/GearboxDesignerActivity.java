...
public class GearboxDesignerActivity extends Activity {
    ...
    private ScrollView build(){
        ...
        calc.setOnClickListener(v->runCalc());return s;
    }
    private void runCalc(){try{
        double T=d(0),rpm=d(1),module=d(4),face=d(5),pa=d(6),eff=d(7),sy=d(8),fos=d(9),allow=d(10),life=d(11);
        int z1=(int)Math.round(d(2)),z2=(int)Math.round(d(3));
        GearboxDesignEngine.GearboxResult x=GearboxDesignEngine.sizeSingleStage(T,rpm,z1,z2,module,face,pa,eff,sy,fos,allow,life);
