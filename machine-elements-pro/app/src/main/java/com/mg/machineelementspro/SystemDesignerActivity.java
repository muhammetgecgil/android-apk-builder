...
public class SystemDesignerActivity extends Activity {
    ...
    private Button productButton,saveButton;
    private int mode=0;
    private SystemSelectionEngine.ShaftBearingFitResult lastBearingResult;
    private SystemSelectionEngine.TighteningResult lastTighteningResult;
    ...
    private View build(){
        ...
        saveButton=new Button(this);saveButton.setText("AKTİF PROJEYE KAYDET");saveButton.setAllCaps(false);saveButton.setVisibility(View.GONE);root.addView(saveButton,lp(dp(54),8));
        ...
        calc.setOnClickListener(v->solve()); productButton.setOnClickListener(v->openCatalog());saveButton.setOnClickListener(v->saveToProject()); return s;
    }
    private void update(int m){for(int i=0;i<fields.length;i++){String x=labels[m][i]; fields[i].setHint(x); fields[i].setText(""); fields[i].setVisibility(x.isEmpty()?View.GONE:View.VISIBLE);} result.setText("Değerleri girin.");productButton.setVisibility(View.GONE);saveButton.setVisibility(View.GONE);lastBearingResult=null;lastTighteningResult=null;}
    ...
    private void solve(){try{
        if(mode==0){
            ...
            result.setText(b.toString()); productButton.setText("BU HESABA GÖRE RULMAN BUL →");productButton.setVisibility(View.VISIBLE);saveButton.setVisibility(View.VISIBLE);
        }else{
            lastBoltDiameter=val(0);SystemSelectionEngine.TighteningResult r=SystemSelectionEngine.tighteningScatter(lastBoltDiameter,val(1),val(2),val(3),val(4));lastTighteningResult=r;
            result.setText("Preload min/nom/max: "+f(r.preloadMinN)+" / "+f(r.nominalPreloadN)+" / "+f(r.preloadMaxN)+" N\nTork min/nom/max: "+f(r.torqueMinNm)+" / "+f(r.torqueNominalNm)+" / "+f(r.torqueMaxNm)+" Nm");productButton.setText("BU HESABA GÖRE CIVATA BUL →");productButton.setVisibility(View.VISIBLE);saveButton.setVisibility(View.VISIBLE);
        }
    }catch(Exception e){Toast.makeText(this,e.getMessage()==null?"Girişleri kontrol edin":e.getMessage(),Toast.LENGTH_LONG).show();}}
    private void saveToProject(){String name=null;if(mode==0&&lastBearingResult!=null)name=ProjectIntegration.saveShaftBearing(this,lastBearingResult,lastRadialN,lastAxialN,lastRpm,lastLifeH);else if(mode==1&&lastTighteningResult!=null)name=ProjectIntegration.saveBoltJoint(this,lastBoltDiameter,lastTighteningResult);if(name==null)Toast.makeText(this,"Önce Project Manager'dan aktif proje seçin.",Toast.LENGTH_LONG).show();else Toast.makeText(this,"Kaydedildi: "+name,Toast.LENGTH_SHORT).show();}
