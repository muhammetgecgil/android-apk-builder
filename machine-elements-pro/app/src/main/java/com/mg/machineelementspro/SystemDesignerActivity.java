...
public class SystemDesignerActivity extends Activity {
    ...
    private View build(){
        ...
        calc.setOnClickListener(v->solve()); return s;
    }
    private void update(int m){for(int i=0;i<fields.length;i++){String x=labels[m][i]; fields[i].setHint(x); fields[i].setText(""); fields[i].setVisibility(x.isEmpty()?View.GONE:View.VISIBLE);} result.setText("Değerleri girin.");}
    ...
    private void solve(){try{
        if(mode==0){
            SystemSelectionEngine.ShaftBearingFitResult r=SystemSelectionEngine.solveShaftBearingFit(val(0),val(1),val(2),val(3),val(4),val(5),val(6),val(7),val(8));
            ...
            SystemSelectionEngine.TighteningResult r=SystemSelectionEngine.tighteningScatter(val(0),val(1),val(2),val(3),val(4));
            result.setText("Preload min/nom/max: "+f(r.preloadMinN)+" / "+f(r.nominalPreloadN)+" / "+f(r.preloadMaxN)+" N\nTork min/nom/max: "+f(r.torqueMinNm)+" / "+f(r.torqueNominalNm)+" / "+f(r.torqueMaxNm)+" Nm");
        }
