...
public class DrivetrainActivity extends Activity {
    ...
    private Button bearingProducts,couplingProducts,saveProject;
    ...
    private ScrollView ui(){
        ...
        TextView sub=t("Dişli → mil → iki rulman bağlı çözüm → gerçek ürün adayı → aktif proje",14,false,Color.rgb(71,85,105));sub.setPadding(0,dp(4),0,dp(10));r.addView(sub);
        ...
        saveProject=new Button(this);saveProject.setText("AKTİF PROJEYE KAYDET");saveProject.setAllCaps(false);saveProject.setVisibility(View.GONE);r.addView(saveProject,lp(-1,dp(54),dp(10)));
        bearingProducts=new Button(this);bearingProducts.setText("BU HESABA GÖRE RULMAN BUL");bearingProducts.setAllCaps(false);bearingProducts.setVisibility(View.GONE);r.addView(bearingProducts,lp(-1,dp(54),dp(8)));
        ...
        calc.setOnClickListener(v->solve());saveProject.setOnClickListener(v->saveToProject());
        bearingProducts.setOnClickListener(v->openBearingProducts());couplingProducts.setOnClickListener(v->openCouplingProducts());
        return s;
    }
    ...
    private void solve(){try{
        ...
        saveProject.setVisibility(View.VISIBLE);bearingProducts.setVisibility(View.VISIBLE);couplingProducts.setVisibility(View.VISIBLE);
    }catch(Exception ex){saveProject.setVisibility(View.GONE);bearingProducts.setVisibility(View.GONE);couplingProducts.setVisibility(View.GONE);Toast.makeText(this,ex.getMessage()==null?"Girdileri kontrol edin.":ex.getMessage(),Toast.LENGTH_LONG).show();}}
    private void saveToProject(){if(lastInput==null||lastResult==null)return;String n=ProjectIntegration.saveDrivetrain(this,lastInput,lastResult);Toast.makeText(this,n==null?"Önce Project Manager'dan aktif proje seçin.":"Drivetrain → "+n+" projesine kaydedildi.",Toast.LENGTH_LONG).show();}
    private void openBearingProducts(){if(lastInput==null||lastResult==null)return;Intent i=new Intent(this,ProductCatalogActivity.class);i.putExtra(ProductCatalogActivity.EXTRA_TYPE,0);i.putExtra(ProductCatalogActivity.EXTRA_V0,String.valueOf(lastInput.shaftDiameterMm));i.putExtra(ProductCatalogActivity.EXTRA_V1,String.valueOf(Math.max(lastInput.bearingC1N,lastInput.bearingC2N)));startActivity(i);}
    private void openCouplingProducts(){if(lastInput==null)return;Intent i=new Intent(this,ProductCatalogActivity.class);i.putExtra(ProductCatalogActivity.EXTRA_TYPE,2);i.putExtra(ProductCatalogActivity.EXTRA_V0,String.valueOf(lastInput.torqueNm));i.putExtra(ProductCatalogActivity.EXTRA_V1,String.valueOf(lastInput.shaftDiameterMm));startActivity(i);}
}
