...
public class LocalWhisperCaptureActivity extends Activity {
    ...
    private void prepareAndStart(){
        ...
        }catch(Exception e){runOnUiThread(()->{action.setEnabled(true);status.setText("Whisper model hatası: "+e.getMessage());});}}).start();
    }
    ...
    private void startRecording(){
        ...
        }catch(Exception e){action.setEnabled(true);status.setText("Mikrofon başlatılamadı: "+e.getMessage());}
    }

    private void stopAndTranscribe(){
        action.setEnabled(false);status.setText("Ses telefonda Whisper ile yazıya çevriliyor…");recording=false;
        ...
        }catch(Exception e){runOnUiThread(()->{action.setEnabled(true);action.setText("Tekrar Dene");status.setText("Yerel ses tanıma hatası: "+e.getMessage());});}
    ...
    @Override protected void onDestroy(){if(recording&&recorder!=null){try{recorder.stopToWav(new File(getCacheDir(),"discard.wav"));}catch(Exception ignored){}}super.onDestroy();}
