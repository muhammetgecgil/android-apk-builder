...
public final class CapabilityRegistry {
    ...
    public static final class Capability {
        ...
        public final String requirementFamily;
        public final State state;
        public final String verification;

        Capability(String id, String name, String requirementFamily, State state, String verification) {
            this.id = id;
            this.name = name;
            this.requirementFamily = requirementFamily;
            this.state = state;
    ...
    static {
        ...
        x.add(new Capability("CAP-MEM", "Long-Term Memory", "MEM,WRK,EPI,SEM,PRC,USR,SKL,FLR,RMM,FOR,MCO,LCT,PRJ,DEC", State.INTEGRATING, "Local conversation memory active; semantic/project memory pending"));
        x.add(new Capability("CAP-TOOL", "Agents & Tools", "TOOL,CODE,AGT,DYN,CAS,CNS,DIV,PERM,ACT,EVT", State.PLANNED, "Not yet integrated"));
    ...
    public static int count(State state) {
        int n = 0;
        for (Capability c : ITEMS) if (c.state == state) n++;
        return n;
    }

    public static String summary() {
        return "Aktif " + count(State.ACTIVE) + " • Testli " + count(State.TESTED) +
                " • Entegrasyonda " + count(State.INTEGRATING) + " • Planlı " + count(State.PLANNED);
    }
}
