...
public class PhoneNativeActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        TextView desc=new TextView(this);desc.setText("v0.28 • Offline AI + hafıza + RAG + kamera OCR + sesli soru");desc.setTextSize(13);desc.setTextColor(Color.DKGRAY);root.addView(desc);
    ...
    private void startVoice(){
        ...
        if(i.resolveActivity(getPackageManager())!=null){output.setText("Dinliyorum… Cihazdaki offline Türkçe konuşma tanıma tercih edilecek.");startActivityForResult(i,SPEECH_PROMPT);}
        else output.setText("Bu cihazda konuşma tanıma servisi bulunamadı.");
    }
    ...
    @Override protected void onActivityResult(int r,int c,Intent data){
        ...
        if(r==SPEECH_PROMPT){
            ...
            if(results==null||results.isEmpty()){output.setText("Konuşma metne çevrilemedi.");return;}
            String text=results.get(0).trim();prompt.setText(text);output.setText("Duydum: "+text+"\nYerel modele gönderiliyor…");runLocal();return;
        }
        if(r==TAKE_PHOTO){
