...
public class PhoneNativeActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        TextView desc=new TextView(this);desc.setText("v0.40 • Phone-Native AI + canlı ses durum kartı");desc.setTextSize(13);desc.setTextColor(Color.DKGRAY);root.addView(desc);
        liveState=new TextView(this);liveState.setTextSize(17);liveState.setTextColor(Color.rgb(24,30,40));liveState.setPadding(dp(12),dp(12),dp(12),dp(12));liveState.setBackgroundColor(Color.WHITE);LinearLayout.LayoutParams lsp=new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,LinearLayout.LayoutParams.WRAP_CONTENT);lsp.setMargins(0,dp(10),0,dp(6));root.addView(liveState,lsp);renderLiveState(VoiceSessionStateManager.get());
    ...
    private void renderLiveState(VoiceSessionStateManager.State state){if(liveState==null)return;String label=VoiceSessionStateManager.label(state);String detail;switch(state){case LISTENING:detail="Mikrofon açık • Yerel Whisper";break;case TRANSCRIBING:detail="Ses cihazda yazıya çevriliyor";break;case THINKING:detail="Yerel LLM cevap üretiyor";break;case SPEAKING:detail="Cevap sesli okunuyor";break;case BARGE_IN:detail="Kullanıcı araya girdi • TTS kesiliyor";break;default:detail="Offline sohbet için hazır";}liveState.setText("MG-AI DURUMU: "+label+"\n"+detail);}
    ...
    @Override protected void onActivityResult(int r,int c,Intent data){
        ...
        if(r==SPEECH_PROMPT){ArrayList<String> results=data==null?null:data.getStringArrayListExtra(RecognizerIntent.EXTRA_RESULTS);if(results==null||results.isEmpty()){output.setText("Konuşma metne çevrilemedi.");return;}String text=results.get(0).trim();prompt.setText(text);output.setText("Duydum: "+text+"\nYerel modele gönderiliyor…");runLocal();return;}
        if(r==TAKE_PHOTO){Bitmap bmp=data!=null&&data.getExtras()!=null?(Bitmap)data.getExtras().get("data"):null;if(bmp==null){output.setText("Kamera görüntüsü alınamadı.");return;}output.setText("Kamera görüntüsü telefonda OCR ile okunuyor…");new Thread(()->{try{String text=LocalOcrEngine.recognizeBitmap(bmp).trim();String name="Kamera "+new SimpleDateFormat("yyyy-MM-dd HH:mm",Locale.getDefault()).format(new Date());String info=LocalDocumentStore.importCameraOcr(this,text,name);runOnUiThread(()->{output.setText("Kamera OCR yerel bilgi tabanına eklendi:\n"+info+"\nŞimdi bu görüntü hakkında soru sorabilirsin.");updateStatus();});}catch(Exception e){runOnUiThread(()->output.setText("Kamera OCR hatası: "+e.getMessage()));}}).start();return;}
    ...
    private void loadModel(){File f=LocalModelManager.activeModel(this);if(f==null){output.setText("Model hazırlanıyor. İlk kullanımda internet gerekir.");return;}if(!LocalInferenceBridge.nativeAvailable()){output.setText("Native llama.cpp runtime yüklenemedi.");updateStatus();return;}output.setText("MG-AI başlatılıyor…");askBtn.setEnabled(false);new Thread(()->{try{if(engine!=0)LocalInferenceBridge.destroyEngine(engine);engine=LocalInferenceBridge.createEngine(f.getAbsolutePath(),4096,Math.max(2,Runtime.getRuntime().availableProcessors()-2));runOnUiThread(()->{progress.setVisibility(ProgressBar.GONE);askBtn.setEnabled(true);output.setText(engine!=0?"Hazır. Offline sohbet, sesli soru, PDF, kamera OCR ve yerel RAG kullanılabilir.":"Model yüklenemedi.");updateStatus();if(engine!=0&&!pendingPrompt.isEmpty()){String q=pendingPrompt;pendingPrompt="";prompt.setText(q);runLocal();}});}catch(Throwable t){runOnUiThread(()->{askBtn.setEnabled(true);output.setText("Native yükleme hatası: "+t.getMessage());});}}).start();}
    private void runLocal(){String p=prompt.getText().toString().trim();if(p.isEmpty())return;if(engine==0){pendingPrompt=p;prompt.setText("");output.setText("Sorun kuyruğa alındı. Model hazır olduğunda otomatik cevaplanacak.");if(LocalModelManager.activeModel(this)==null&&!downloading)autoInstallDefault();return;}boolean remembered=LocalLongTermMemory.maybeRememberUserMessage(this,p);if(p.toLowerCase(new Locale("tr","TR")).startsWith("proje:"))LocalLongTermMemory.addProjectNote(this,p.substring(6).trim());LocalChatStore.add(this,"user",p);renderHistory();prompt.setText("");askBtn.setEnabled(false);output.setText(remembered?"Hafızaya kaydedildi. Düşünüyor…":"Düşünüyor…");String ctx=LocalChatStore.transcript(this,6);String mem=LocalLongTermMemory.relevantContext(this,p,6);String docs=LocalDocumentStore.retrieve(this,p,3);String full="Sen MG-AI adlı yardımcı yapay zekasın. Türkçe sorulara Türkçe cevap ver. Kullanıcıya ait uzun süreli hafıza ve yerel belge/OCR parçaları varsa yalnız ilgili olduğu ölçüde kullan. Belgelerde olmayan bir bilgiyi belge varmış gibi uydurma.\n\nUZUN SÜRELİ HAFIZA:\n"+(mem.isEmpty()?"(ilgili kayıt yok)":mem)+"\n\nYEREL BELGE/OCR BAĞLAMI:\n"+(docs.isEmpty()?"(ilgili belge parçası yok)":docs)+"\n\nSON KONUŞMA:\n"+ctx+"\nMG-AI:";new Thread(()->{try{String ans=LocalInferenceBridge.generate(engine,full,512,0.7f);LocalChatStore.add(this,"assistant",ans);runOnUiThread(()->{askBtn.setEnabled(true);output.setText(ans);renderHistory();updateStatus();});}catch(Throwable t){runOnUiThread(()->{askBtn.setEnabled(true);output.setText("Inference hatası: "+t.getMessage());});}}).start();}
