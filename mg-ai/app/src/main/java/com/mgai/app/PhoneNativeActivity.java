...
public class PhoneNativeActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        TextView desc=new TextView(this);desc.setText("v0.21 • Telefonun içinde çalışan offline AI");desc.setTextSize(13);desc.setTextColor(Color.DKGRAY);root.addView(desc);
        ...
        prompt=new EditText(this);prompt.setHint("Bir şey sor...");prompt.setMinLines(3);prompt.setPadding(dp(10),dp(14),dp(10),dp(14));root.addView(prompt);
        Button ask=new Button(this);ask.setText("Gönder");ask.setAllCaps(false);ask.setOnClickListener(v->runLocal());root.addView(ask);
        output=new TextView(this);output.setTextSize(16);output.setTextColor(Color.rgb(30,34,42));output.setPadding(0,dp(18),0,dp(8));root.addView(output);
    ...
    private void loadModel(){File f=LocalModelManager.activeModel(this);if(f==null){output.setText("Model hazırlanıyor. İlk kullanımda internet gerekir.");return;}if(!LocalInferenceBridge.nativeAvailable()){output.setText("Native llama.cpp runtime yüklenemedi.");updateStatus();return;}output.setText("MG-AI başlatılıyor…");new Thread(()->{try{if(engine!=0)LocalInferenceBridge.destroyEngine(engine);engine=LocalInferenceBridge.createEngine(f.getAbsolutePath(),4096,Math.max(2,Runtime.getRuntime().availableProcessors()-2));runOnUiThread(()->{progress.setVisibility(ProgressBar.GONE);output.setText(engine!=0?"Hazır. İnternet olmadan soru sorabilirsin.":"Model yüklenemedi.");updateStatus();});}catch(Throwable t){runOnUiThread(()->output.setText("Native yükleme hatası: "+t.getMessage()));}}).start();}
    private void runLocal(){String p=prompt.getText().toString().trim();if(p.isEmpty())return;if(engine==0){output.setText("MG-AI henüz hazır değil; model kurulumu/yüklemesi tamamlanmalı.");return;}output.setText("Düşünüyor…");new Thread(()->{try{String ans=LocalInferenceBridge.generate(engine,p,512,0.7f);runOnUiThread(()->{output.setText(ans);prompt.setText("");});}catch(Throwable t){runOnUiThread(()->output.setText("Inference hatası: "+t.getMessage()));}}).start();}
