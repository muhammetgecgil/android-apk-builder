...
public final class LocalVoiceOutput {
    ...
    public static void setEnabled(Context c,boolean enabled){c.getSharedPreferences(PREFS,Context.MODE_PRIVATE).edit().putBoolean(KEY_ENABLED,enabled).apply();if(!enabled){ContinuousDialogManager.suppressNextAutoListen();stop();}}
    ...
    public static boolean handleCommand(Context c,String text){
        ...
        if(n.equals("sesi durdur")||n.equals("okumayı durdur")){ContinuousDialogManager.suppressNextAutoListen();stop();return true;}
    ...
    public static synchronized void speak(Context context,String text){
        ...
        tts=new TextToSpeech(app,status->{synchronized(LocalVoiceOutput.class){initStarted=false;if(status!=TextToSpeech.SUCCESS){ready=false;return;}Locale tr=new Locale("tr","TR");int result=tts.setLanguage(tr);ready=result!=TextToSpeech.LANG_MISSING_DATA&&result!=TextToSpeech.LANG_NOT_SUPPORTED;tts.setSpeechRate(currentRate);tts.setPitch(1.0f);tts.setOnUtteranceProgressListener(new UtteranceProgressListener(){@Override public void onStart(String id){}@Override public void onDone(String id){ContinuousDialogManager.onSpeechDone(appContext);}@Override public void onError(String id){}});if(ready&&!pending.isEmpty()){doSpeak(pending);pending="";}}});
    }
    ...
    public static synchronized void shutdown(){try{if(tts!=null){tts.stop();tts.shutdown();}}catch(Throwable ignored){}tts=null;ready=false;initStarted=false;pending="";appContext=null;}
