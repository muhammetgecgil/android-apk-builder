...
public final class AdaptivePerformanceManager {
    public static final class Profile {
        ...
        public String summary(){return name+" • "+threads+" thread • ctx "+contextSize+" • max "+maxTokens+" • "+String.format(java.util.Locale.US,"%.1f°C",temperatureC)+" • "+reason;}
    }
    ...
    public static Profile choose(Context c){
        ...
        if(temp>=43f){
            return new Profile("SERİN",2048,Math.max(2,cores/3),256,temp,"yüksek sıcaklık");
        }
        if(temp>=39f){
            return new Profile("DENGELİ",3072,Math.max(2,cores/2),384,temp,"ısı kontrolü");
        }
        if(ttft>2500 || (tps>0 && tps<6.0)){
            return new Profile("HIZLI",3072,Math.max(3,cores-2),384,temp,"yüksek TTFT / düşük token hızı");
        }
        if(tps>=12.0 && (temp<0 || temp<37f)){
            return new Profile("KALİTE",4096,Math.max(3,cores-2),512,temp,"performans payı mevcut");
        }
