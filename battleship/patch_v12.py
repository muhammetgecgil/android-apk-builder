...
# Sonar selection intercepts enemy-grid tap without consuming the turn.
rep('if(playing&&myTurn&&!gameOver){int c=(int)((e.getX()-left)/cell),r=(int)((e.getY()-t2)/cell);if(valid(r,c)){myTurn=false;if(solo)playerShot(r,c);else if(connected){link.send("SHOT|"+r+"|"+c);status.setText("Atış gönderildi — sonuç bekleniyor…");}}}return true;',
'''if(playing&&myTurn&&!gameOver){int c=(int)((e.getX()-left)/cell),r=(int)((e.getY()-t2)/cell);if(valid(r,c)){if(tacticalMode&&solo&&sonarArmed){useSonar(r,c);return true;}myTurn=false;if(solo)playerShot(r,c);else if(connected){link.send("SHOT|"+r+"|"+c);status.setText("Atış gönderildi — sonuç bekleniyor…");}}}return true;''','sonar-touch')
