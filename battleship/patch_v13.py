...
# Combat state: threat direction for the live HUD.
...
# HUD no longer uses a cramped fixed glass box. Two adaptive rows fit small screens.
...
# Split telemetry across two rows, including threat direction.
...
'''int dmg=board.ownDamagePercent();String alarm=dmg>=70?"KIRMIZI ALARM":(dmg>=40?"HASAR UYARISI":"SİSTEM NORMAL");battleHud.setText(turn+"  •  GEMİ "+mine+"  •  DÜŞMAN "+enemy+"\nHASAR %"+dmg+"  •  İSABET %"+String.format(java.util.Locale.US,"%.0f",acc)+"  •  SERİ ×"+hitStreak+"  •  "+threatText+"  •  "+alarm);''','hud-two-lines')

# Enemy grid title sits above A-J coordinates instead of colliding with them.
...
# Critical combat mechanic: every third consecutive hit earns one immediate extra shot.
...
# AI fire now reports the quadrant of the latest threat contact.
...
# Make high-level AI less repetitive by mixing parity hunt with pressure around the last threat sector.
...
# v13 identity in the main dashboard footer.
