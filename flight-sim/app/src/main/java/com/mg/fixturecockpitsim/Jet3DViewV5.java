...
/** V5 procedural fighter: smoother fuselage loft, softened IMU response and eased camera moves. */
public final class Jet3DViewV5 extends GLSurfaceView {
    ...
    private static final class R implements Renderer{
        ...
        @Override public void onDrawFrame(GL10 gl){long n=System.nanoTime();float dt=Math.min(.04f,Math.max(.001f,(n-last)/1e9f));last=n;
            // critically damped feel: smooth but responsive, avoids jerky Bluetooth steps
        ...
        Mesh buildJet(){B b=new B();float[] top={.42f,.46f,.50f,1},mid={.30f,.34f,.38f,1},side={.21f,.24f,.28f,1},dark={.07f,.09f,.11f,1},glass={.025f,.13f,.19f,.94f},noz={.09f,.09f,.10f,1};
            // smooth lofted fuselage with 9 cross-sections
            ...
            b.loft(z,wx,hy,10,top,mid,dark);
            wing(b,-1,top,side,dark);wing(b,1,top,side,dark);stab(b,-1,mid,dark);stab(b,1,mid,dark);fin(b,-.68f,-1,mid,dark);fin(b,.68f,1,mid,dark);
            // canopy as elongated faceted bubble
            b.loft(new float[]{-1.75f,-1.35f,-.90f,-.45f},new float[]{.05f,.22f,.27f,.12f},new float[]{.04f,.20f,.24f,.10f},10,glass,glass,glass,.34f);
            // engine shoulders and 24-segment nozzles
        ...
        static final String FS="precision mediump float;uniform vec3 uLight;uniform float uEmissive;varying vec3 n;varying vec4 c;void main(){float d=max(0.0,dot(normalize(n),normalize(uLight)));float rim=pow(1.0-max(0.0,n.z),2.0)*0.10;float l=0.30+0.64*d+rim+uEmissive;gl_FragColor=vec4(c.rgb*l,c.a);}";
    }
