...
public final class RunwayHudView extends View {
    ...
    private void drawHangar(Canvas c,int w,int h,float hz){float q=phase.contains("PARK")?1-groundProgress:groundProgress,cx=w*.5f;float doorHalf=w*(.22f+.16f*q),top=h*.16f,bottom=h*.78f;fill.setShader(new LinearGradient(0,0,0,h,new int[]{0xff0b1014,0xff182126,0xff353d41},null,Shader.TileMode.CLAMP));c.drawRect(0,0,w,h,fill);fill.setShader(null);drawOutside(c,cx-doorHalf,top,cx+doorHalf,bottom,w,h);quad(c,cx-doorHalf,bottom,cx+doorHalf,bottom,w,h,0,h,0xff363c3f);stroke.setColor(0xff646e74);stroke.setStrokeWidth(2);for(int i=-9;i<=9;i++){float x=cx+i*w*.064f;c.drawLine(x,h,cx+(x-cx)*.29f,bottom,stroke);}for(int i=1;i<8;i++){float t=i/8f,y=lerp(bottom,h,t*t);c.drawLine(0,y,w,y,stroke);}stroke.setColor(0xffe6c52e);stroke.setStrokeWidth(5);c.drawLine(cx,bottom,cx,h,stroke);stroke.setColor(0xff5e6970);for(int i=0;i<7;i++){float t=i/6f,y=lerp(h*.06f,bottom,t),half=lerp(w*.48f,doorHalf,t);stroke.setStrokeWidth(7-3*t);c.drawLine(cx-half,y,cx-half,bottom,stroke);c.drawLine(cx+half,y,cx+half,bottom,stroke);c.drawLine(cx-half,y,cx+half,y,stroke);c.drawLine(cx-half,y,cx,y-h*.055f,stroke);c.drawLine(cx+half,y,cx,y-h*.055f,stroke);}for(int i=0;i<9;i++){float x=w*(.07f+i*.105f);fill.setColor(0x55fff0ba);c.drawRoundRect(x,h*.072f,x+w*.060f,h*.092f,5,5,fill);fill.setColor(0xffffe7a4);c.drawRoundRect(x+w*.008f,h*.078f,x+w*.052f,h*.087f,4,4,fill);}for(int side=-1;side<=1;side+=2){float bx=side<0?w*.02f:w*.86f;fill.setColor(0xff444d52);c.drawRect(bx,h*.49f,bx+w*.115f,h*.775f,fill);fill.setColor(0xff20272b);for(int k=0;k<4;k++)c.drawRect(bx+w*.012f,h*(.525f+k*.055f),bx+w*.102f,h*(.544f+k*.055f),fill);fill.setColor(0xffc93631);c.drawRect(bx+w*.014f,h*.70f,bx+w*.038f,h*.765f,fill);fill.setColor(0xffd9a72c);c.drawCircle(bx+w*.078f,h*.635f,w*.014f,fill);}fill.setColor(0xe8ffffff);fill.setTextSize(Math.max(15,w*.016f));c.drawText("MIL HANGAR 01  •  TAXI A  •  RWY 27",w*.035f,h*.93f,fill);}

    private void drawOutside(Canvas c,float x0,float y0,float x1,float y1,int w,int h){fill.setShader(new LinearGradient(0,y0,0,y1,new int[]{0xff9dd2e5,0xff759fac,0xff465a3e},null,Shader.TileMode.CLAMP));c.drawRect(x0,y0,x1,y1,fill);fill.setShader(null);float cx=(x0+x1)/2;quad(c,cx-(x1-x0)*.055f,y0+(y1-y0)*.10f,cx+(x1-x0)*.055f,y0+(y1-y0)*.10f,x1,y1,x0,y1,0xff555b5e);stroke.setColor(0xffe6c42d);stroke.setStrokeWidth(4);c.drawLine(cx,y0+(y1-y0)*.10f,cx,y1,stroke);drawMilitaryTowers(c,(int)x0,(int)x1,(int)(y0+(y1-y0)*.42f),(int)(y1-y0),.70f);}

    private void drawTaxi(Canvas c,int w,int h,float hz){fill.setColor(0xff3f543b);c.drawRect(0,hz,w,h,fill);drawMountains(c,w,h,hz,.17f,false);drawMilitaryTowers(c,0,w,(int)hz,h,.90f);float cx=w*.5f,far=w*.043f,near=w*.30f;quad(c,cx-far,hz+h*.014f,cx+far,hz+h*.014f,cx+near,h,cx-near,h,0xff51575a);quad(c,cx-far*.78f,hz+h*.014f,cx+far*.78f,hz+h*.014f,cx+near*.78f,h,cx-near*.78f,h,0xff3b4043);stroke.setColor(0xffefca22);stroke.setStrokeWidth(5);c.drawLine(cx,hz+h*.015f,cx,h,stroke);for(int i=0;i<26;i++){float q=(i/26f+scroll*.56f)%1f,z=q*q,y=lerp(hz+h*.025f,h,z),hh=lerp(far,near,z);fill.setColor(0xff429cff);float r=1.2f+4.2f*z;c.drawCircle(cx-hh*.95f,y,r,fill);c.drawCircle(cx+hh*.95f,y,r,fill);if(i%3==0){stroke.setColor(0x355f6465);stroke.setStrokeWidth(1+2*z);c.drawLine(cx-hh*.75f,y,cx+hh*.75f,y,stroke);}}if(groundProgress>.55f){float y=h*.66f,hh=w*.145f;stroke.setColor(0xffffd43e);stroke.setStrokeWidth(5);c.drawLine(cx-hh,y,cx+hh,y,stroke);stroke.setStrokeWidth(2);for(int i=0;i<8;i++){float x=cx-hh+i*(2*hh/7f);c.drawLine(x,y+7,x+10,y+18,stroke);}fill.setColor(0xeeffffff);fill.setTextSize(Math.max(14,w*.015f));c.drawText("MIL RWY 27 HOLD SHORT",w*.64f,h*.72f,fill);}fill.setColor(0xe8ffffff);fill.setTextSize(Math.max(14,w*.015f));c.drawText("MIL AIR BASE  •  HDG "+String.format(Locale.US,"%03.0f",norm(headingDeg))+"  •  TAXI",w*.04f,h*.92f,fill);}

    private void drawRunway(Canvas c,int w,int h,float hz){float near=1-Math.min(1,altitudeM/1050f),err=angleError(headingDeg,270f),cx=w*.5f+clamp(err/22f,-1,1)*w*.095f;float fh=w*(.018f+.012f*near),nh=w*(onGround?.45f:(.12f+.30f*near)),fy=hz+h*.014f,ny=h*(onGround?.999f:(.72f+.26f*near));drawTerrain(c,w,h,hz);quad(c,cx-fh*1.28f,fy,cx+fh*1.28f,fy,cx+nh*1.10f,ny,cx-nh*1.10f,ny,0xff666a63);quad(c,cx-fh,fy,cx+fh,fy,cx+nh,ny,cx-nh,ny,0xff24282a);for(int i=-8;i<=8;i++){float a=i/8f;stroke.setColor(i%3==0?0x33484a4b:0x1d101112);stroke.setStrokeWidth(1+Math.abs(a)*1.2f);c.drawLine(cx+fh*a*.82f,fy,cx+nh*a*.82f,ny,stroke);}stroke.setColor(0xfff4f3ed);stroke.setStrokeWidth(Math.max(2,w*.0025f));c.drawLine(cx-fh,fy,cx-nh,ny,stroke);c.drawLine(cx+fh,fy,cx+nh,ny,stroke);for(int i=0;i<36;i++){float q=(i/36f+scroll)%1f,z=q*q,y=lerp(fy,ny,z),y2=Math.min(ny,y+4+46*z);stroke.setColor(0xfff5f5f0);stroke.setStrokeWidth(2+12*z);c.drawLine(cx,y,cx,y2,stroke);}for(int i=0;i<30;i++){float q=(i/30f+scroll*.62f)%1f,z=q*q,y=lerp(fy,ny,z),hh=lerp(fh,nh,z);fill.setColor(i>25?0xffffd56b:0xfff3f0d8);float r=1+4*z;c.drawCircle(cx-hh,y,r,fill);c.drawCircle(cx+hh,y,r,fill);}for(int set=0;set<3;set++){float q=(.31f+set*.17f+scroll*.14f)%1f,z=q*q,y=lerp(fy,ny,z),hh=lerp(fh,nh,z),bw=hh*.10f,bh=5+20*z;fill.setColor(0xffeeeeea);for(int s=-1;s<=1;s+=2){c.drawRect(cx+s*hh*.43f-bw,y,cx+s*hh*.43f+bw,y+bh,fill);c.drawRect(cx+s*hh*.66f-bw*.72f,y,cx+s*hh*.66f+bw*.72f,y+bh,fill);}}float nz=.48f+(phase.contains("TAKEOFF")?runwayMeters/310f:0);if(nz<=1.04f){float z=clamp(nz,0,1),y=lerp(fy,ny,z*z),hh=lerp(fh,nh,z);fill.setColor(0xfffafaf5);for(int i=-5;i<=5;i++){if(i==0)continue;float x=cx+i*hh*.095f;c.drawRect(x-hh*.030f,y,x+hh*.030f,y+13+38*z,fill);}fill.setTextAlign(Paint.Align.CENTER);fill.setTextSize(Math.max(27,w*(.033f+.030f*z)));c.drawText("27",cx,y+58+42*z,fill);fill.setTextAlign(Paint.Align.LEFT);}if(!onGround||phase.contains("APPROACH")||phase.contains("FLARE")){float py=lerp(fy,ny,.36f),ph=lerp(fh,nh,.36f);for(int i=0;i<4;i++){fill.setColor(i<2?0xffffffff:0xffff3b30);c.drawCircle(cx-ph*1.28f+i*6,py,3.2f,fill);}}if(phase.contains("APPROACH")||phase.contains("FLARE")){fill.setColor(Math.abs(err)<2.5?0xff7fe889:0xffffd35a);fill.setTextSize(Math.max(14,w*.015f));c.drawText(Math.abs(err)<2.5?"MIL RWY 27 • CENTERLINE CAPTURED":"MIL RWY 27 • ALIGNING",w*.62f,h*.92f,fill);}}
