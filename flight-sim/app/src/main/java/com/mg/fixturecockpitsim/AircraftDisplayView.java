...
/** Aircraft display V5: refined 3D fighter + pilot-helmet HMD style side display. */
public final class AircraftDisplayView extends LinearLayout {
    private final Jet3DViewV5 jet;
    ...
    public AircraftDisplayView(Context c){
        ...
        jet=new Jet3DViewV5(c);panel=new HelmetPanel(c);
        addView(jet,new LayoutParams(0,LayoutParams.MATCH_PARENT,.74f));
        addView(panel,new LayoutParams(0,LayoutParams.MATCH_PARENT,.26f));
        ui.post(tick);
    }
    ...
    private final Runnable tick=new Runnable(){@Override public void run(){
        ...
        if(!live&&demoEnabled){float t=(now-demoStart)/1000f;float rr=26f*(float)Math.sin(t*.46f)+6f*(float)Math.sin(t*1.1f);float pp=8f*(float)Math.sin(t*.31f)+2.5f*(float)Math.cos(t*.74f);float yy=(t*12f)%360f;float tt=.73f+.13f*(float)Math.sin(t*.18f);
            dr+=angle(rr-dr)*.035f;dp+=(pp-dp)*.035f;dy+=angle(yy-dy)*.025f;dt+=(tt-dt)*.025f;roll=dr;pitch=dp;yaw=dy;throttle=dt;linkHz=50;drops=0;
    ...
    private static final class HelmetPanel extends View{
        ...
        @Override protected void onDraw(Canvas c){int w=getWidth(),h=getHeight();
            // tinted visor background
            p.setShader(new LinearGradient(0,0,0,h,new int[]{Color.rgb(18,38,43),Color.rgb(4,12,15)},null,Shader.TileMode.CLAMP));c.drawRect(0,0,w,h,p);p.setShader(null);
            // curved helmet visor vignette
            ...
            // pitch ladder rotates opposite bank
