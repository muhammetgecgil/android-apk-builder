...
public final class CinematicTerrainMesh {
    ...
    private static float h(int kind,float x,float z){
        ...
        if(kind==0){ // Toros: steep folded ridges with multiple frequencies.
        ...
        if(kind==1){ // Aegean coast: low sea shelf on left, rocky coastal hills on right.
        ...
        if(kind==2){ // Patara: coastal dune trains.
        ...
        if(kind==3){ // Karapinar-style arid interior, rolling dry relief rather than a fantasy desert.
    ...
    private static void tri(ArrayList<Float> o,int kind,float ax,float ay,float az,float bx,float by,float bz,float cx,float cy,float cz){
        ...
        float part=60f+kind;v(o,ax,ay,az,nx,ny,nz,part);v(o,bx,by,bz,nx,ny,nz,part);v(o,cx,cy,cz,nx,ny,nz,part);
    }
