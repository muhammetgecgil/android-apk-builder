...
public class MainActivity extends Activity implements SensorEventListener {
    ...
    private PilotHmdSceneView pilotView; private AircraftDisplayView aircraftDisplay;
    ...
    private void showRoleScreen(){
        ...
        root.addView(label("FIXTURE COCKPIT SIM • HMD 3D PRO",30));TextView sub=label("Tek APK • iki telefon • Bluetooth IMU • 3D stealth fighter • pilot HMD • offline demo",15);sub.setTextColor(Color.LTGRAY);root.addView(sub);
        Button p=button("PİLOT / KASK HMD GÖRÜŞÜ");Button d=button("UÇAK + KOKPİT EKRANI");LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(dp(500),dp(70));lp.setMargins(0,14,0,0);root.addView(p,lp);root.addView(d,lp);
        TextView info=label("Pilot ekranında gökyüzü, dağlar ve HMD bilgileri. Uçak tarafı Bluetooth olmadan DEMO ile çalışır.",14);info.setTextColor(Color.GRAY);root.addView(info);
    ...
    private void startPilot(){
        ...
        FrameLayout root=new FrameLayout(this);pilotView=new PilotHmdSceneView(this);pilotView.setData(roll,pitch,yaw,throttle,connected,rttMs);root.addView(pilotView,new FrameLayout.LayoutParams(-1,-1));
        LinearLayout bar=new LinearLayout(this);bar.setGravity(Gravity.CENTER);bar.setBackgroundColor(0xB8000000);Button conn=button("DATA LINK");Button center=button("IMU MERKEZLE");Button minus=button("THR −");Button plus=button("THR +");Button back=button("MOD");bar.addView(conn);bar.addView(center);bar.addView(minus);bar.addView(plus);bar.addView(back);root.addView(bar,new FrameLayout.LayoutParams(-1,dp(56),Gravity.BOTTOM));setContentView(root);
        if(rotationSensor!=null)sensors.registerListener(this,rotationSensor,SensorManager.SENSOR_DELAY_GAME);
        conn.setOnClickListener(v->chooseDevice());center.setOnClickListener(v->{zeroRoll=rawRoll;zeroPitch=rawPitch;zeroYaw=rawYaw;centered=true;roll=pitch=yaw=0;pilotView.setData(roll,pitch,yaw,throttle,connected,rttMs);toast("IMU merkezlendi");});minus.setOnClickListener(v->{throttle=Math.max(0,throttle-.05f);pilotView.setData(roll,pitch,yaw,throttle,connected,rttMs);});plus.setOnClickListener(v->{throttle=Math.min(1,throttle+.05f);pilotView.setData(roll,pitch,yaw,throttle,connected,rttMs);});back.setOnClickListener(v->showRoleScreen());
    }
