...
/** AVM-7.4 volumetric fighter mesh with flush upper-surface fairings and integrated exhaust shoulders. */
public final class ProceduralFighterMesh {
    ...
    public static final class Mesh {
        ...
        public Mesh(float[] data){
            if(data==null||data.length%7!=0) throw new IllegalStateException("Mesh vertex stride corrupted");
            this.data=data;
        }
    ...
    private void fuselage(){
        float[] z=AirframeShapeProfile.Z,rx=AirframeShapeProfile.HALF_WIDTH,ry=AirframeShapeProfile.HALF_HEIGHT,cy=AirframeShapeProfile.CENTER_Y;
        int stations=Math.min(Math.min(z.length,rx.length),Math.min(ry.length,cy.length)),sides=28;
        for(int s=0;s<stations-1;s++)for(int i=0;i<sides;i++){
            double a0=2*Math.PI*i/sides,a1=2*Math.PI*(i+1)/sides;
            float sy0=sectionY((float)Math.sin(a0)),sy1=sectionY((float)Math.sin(a1));
            quad(new float[]{rx[s]*(float)Math.cos(a0),cy[s]+ry[s]*sy0,z[s]},new float[]{rx[s+1]*(float)Math.cos(a0),cy[s+1]+ry[s+1]*sy0,z[s+1]},new float[]{rx[s+1]*(float)Math.cos(a1),cy[s+1]+ry[s+1]*sy1,z[s+1]},new float[]{rx[s]*(float)Math.cos(a1),cy[s]+ry[s]*sy1,z[s]});
        }
    }
    ...
    private void noseCanopyBlend(){
        prism(new float[][]{{-.46f,.605f,-2.86f},{-.20f,.65f,-2.74f},{-.28f,.73f,-2.36f},{-.43f,.785f,-2.08f},{-.59f,.725f,-2.16f},{-.56f,.63f,-2.62f}},.014f);
        prism(new float[][]{{.20f,.65f,-2.74f},{.46f,.605f,-2.86f},{.56f,.63f,-2.62f},{.59f,.725f,-2.16f},{.43f,.785f,-2.08f},{.28f,.73f,-2.36f}},.014f);
        prism(new float[][]{{-.18f,.665f,-2.76f},{.18f,.665f,-2.76f},{.27f,.74f,-2.34f},{.22f,.79f,-2.10f},{-.22f,.79f,-2.10f},{-.27f,.74f,-2.34f}},.008f);
    }
    private void upperSpinePanel(){
        prism(new float[][]{{-.31f,.855f,.72f},{.31f,.855f,.72f},{.36f,.85f,1.15f},{.34f,.81f,1.68f},{.28f,.74f,2.18f},{.20f,.65f,2.62f},{-.20f,.65f,2.62f},{-.28f,.74f,2.18f},{-.34f,.81f,1.68f},{-.36f,.85f,1.15f}},.020f);
        prism(new float[][]{{-.10f,.875f,.84f},{.10f,.875f,.84f},{.12f,.86f,1.54f},{.10f,.785f,2.20f},{.06f,.69f,2.58f},{-.06f,.69f,2.58f},{-.10f,.785f,2.20f},{-.12f,.86f,1.54f}},.010f);
    }
    private void centerlineDorsalRidge(){
        prism(new float[][]{{-.075f,.885f,.82f},{.075f,.885f,.82f},{.070f,.87f,1.22f},{.062f,.835f,1.66f},{.052f,.785f,2.06f},{.040f,.73f,2.42f},{.026f,.685f,2.70f},{-.026f,.685f,2.70f},{-.040f,.73f,2.42f},{-.052f,.785f,2.06f},{-.062f,.835f,1.66f},{-.070f,.87f,1.22f}},.006f);
    }
    private void canopyShoulderPanels(){
        prism(new float[][]{{-.63f,.79f,-1.92f},{-.50f,.865f,-1.78f},{-.48f,.88f,-.40f},{-.42f,.845f,.48f},{-.54f,.80f,.70f},{-.61f,.775f,-.62f}},.012f);
        prism(new float[][]{{.50f,.865f,-1.78f},{.63f,.79f,-1.92f},{.61f,.775f,-.62f},{.54f,.80f,.70f},{.42f,.845f,.48f},{.48f,.88f,-.40f}},.012f);
    }
    ...
    private void wingRootUpperFairing(float side){
        float[] z={-2.62f,-2.20f,-1.76f,-1.32f,-.86f,-.38f,.08f,.50f};
        float[] ix={.74f,.82f,.90f,.98f,1.04f,1.08f,1.08f,1.02f};
        float[] ox={1.14f,1.56f,2.02f,2.42f,2.26f,1.92f,1.54f,1.22f};
        float[] iy={.40f,.44f,.48f,.51f,.52f,.51f,.47f,.42f};
        float[] oy={.37f,.34f,.31f,.28f,.28f,.30f,.32f,.34f};
        surfaceRibbon(side,z,ix,ox,iy,oy,.018f);
    }

    private void engineDeckPanels(){
        prism(new float[][]{{-1.22f,.46f,.72f},{-.48f,.68f,.90f},{-.45f,.65f,2.18f},{-.60f,.54f,2.84f},{-1.00f,.39f,3.08f},{-1.18f,.44f,1.72f}},.028f);
        prism(new float[][]{{.48f,.68f,.90f},{1.22f,.46f,.72f},{1.18f,.44f,1.72f},{1.00f,.39f,3.08f},{.60f,.54f,2.84f},{.45f,.65f,2.18f}},.028f);
        prism(new float[][]{{-.98f,.50f,1.05f},{-.48f,.69f,1.10f},{-.44f,.64f,2.02f},{-.57f,.54f,2.64f},{-.89f,.43f,2.78f}},.012f);
        prism(new float[][]{{.48f,.69f,1.10f},{.98f,.50f,1.05f},{.89f,.43f,2.78f},{.57f,.54f,2.64f},{.44f,.64f,2.02f}},.012f);
    }
    ...
    private void engineDeckBulge(float side){
        ...
        int n=minLen(z,inner,outer,yi,yo);
        for(int i=0;i<n-1;i++){
    ...
    private void twinEngineCenterValley(){
        ...
        float[] half={.36f,.38f,.40f,.42f,.43f,.40f,.32f};
        int n=Math.min(Math.min(z.length,y.length),half.length);
        for(int i=0;i<n-1;i++){
            float[] a={-half[i],y[i]-.035f,z[i]},b={half[i],y[i]-.035f,z[i]},c={half[i+1],y[i+1]-.035f,z[i+1]},d={-half[i+1],y[i+1]-.035f,z[i+1]};
            quad(a,b,c,d);
            float[] ai=offset(a,0,-.012f,0),bi=offset(b,0,-.012f,0),ci=offset(c,0,-.012f,0),di=offset(d,0,-.012f,0);
    ...
    private void nozzleShoulderShroud(float side){
        ...
        int n=Math.min(z.length,Math.min(w.length,y.length));
        for(int i=0;i<n-1;i++){
            float cx=.70f*side,inner=Math.max(.30f,w[i]-.72f),outer=Math.min(.62f,w[i]-.46f),innerN=Math.max(.28f,w[i+1]-.72f),outerN=Math.min(.58f,w[i+1]-.46f);
            float[] a={cx-inner*side,y[i],z[i]},b={cx+outer*side,y[i]-.065f,z[i]},c={cx+outerN*side,y[i+1]-.065f,z[i+1]},d={cx-innerN*side,y[i+1],z[i+1]};
            if(side<0f) quad(b,a,d,c); else quad(a,b,c,d);
            float[] ai=offset(a,0,-.024f,0),bi=offset(b,0,-.024f,0),ci=offset(c,0,-.024f,0),di=offset(d,0,-.024f,0);
            if(side<0f) quad(ai,bi,ci,di); else quad(bi,ai,di,ci);
            quad(a,ai,di,d);quad(b,c,ci,bi);
        }
    }
    private void tailRootFairing(){tailRootFairing(-1f);tailRootFairing(1f);}
    private void tailRootFairing(float side){
        float[] z={1.44f,1.82f,2.22f,2.62f,2.96f,3.18f};
        float[] ix={.54f,.56f,.60f,.66f,.74f,.82f};
        float[] ox={.74f,.86f,.96f,1.02f,1.03f,.96f};
        float[] iy={.63f,.65f,.62f,.56f,.47f,.38f};
        float[] oy={.52f,.53f,.51f,.46f,.39f,.32f};
        surfaceRibbon(side,z,ix,ox,iy,oy,.018f);
    }
    ...
    private void verticalTailRootShoulder(float side){
        float[] z={1.52f,1.88f,2.26f,2.62f,2.94f,3.16f};
        float[] ix={.58f,.60f,.64f,.70f,.78f,.84f};
        float[] ox={.76f,.86f,.96f,1.04f,1.05f,.98f};
        float[] iy={.61f,.61f,.58f,.53f,.45f,.36f};
        float[] oy={.53f,.53f,.50f,.45f,.38f,.31f};
        surfaceRibbon(side,z,ix,ox,iy,oy,.014f);
    }

    private void surfaceRibbon(float side,float[] z,float[] innerX,float[] outerX,float[] innerY,float[] outerY,float thickness){
        int n=minLen(z,innerX,outerX,innerY,outerY);
        for(int i=0;i<n-1;i++){
            float[] a={innerX[i]*side,innerY[i],z[i]},b={outerX[i]*side,outerY[i],z[i]},c={outerX[i+1]*side,outerY[i+1],z[i+1]},d={innerX[i+1]*side,innerY[i+1],z[i+1]};
            if(side<0f) quad(b,a,d,c); else quad(a,b,c,d);
            float[] ai=offset(a,0,-thickness,0),bi=offset(b,0,-thickness,0),ci=offset(c,0,-thickness,0),di=offset(d,0,-thickness,0);
            if(side<0f) quad(ai,bi,ci,di); else quad(bi,ai,di,ci);
            quad(a,ai,di,d);quad(b,c,ci,bi);
        }
    }
    private static int minLen(float[]... arrays){int n=Integer.MAX_VALUE;for(float[] a:arrays)n=Math.min(n,a.length);return n;}
    ...
    private void verticalTail(float side){
        float[][] top={{.62f*side,.56f,1.56f},{.82f*side,1.40f,1.78f},{1.22f*side,2.38f,2.16f},{1.10f*side,1.42f,2.64f},{.96f*side,.50f,3.12f}};
        prism(top,.075f);
    }
    ...
    private void enginePod(float x){
        float[] z=AirframeShapeProfile.ENGINE_Z,r=AirframeShapeProfile.ENGINE_R;int stations=Math.min(z.length,r.length),sides=22;
        for(int s=0;s<stations-1;s++)for(int i=0;i<sides;i++){
            double a0=2*Math.PI*i/sides,a1=2*Math.PI*(i+1)/sides;
            quad(new float[]{x+r[s]*(float)Math.cos(a0),-.11f+r[s]*.58f*(float)Math.sin(a0),z[s]},new float[]{x+r[s+1]*(float)Math.cos(a0),-.11f+r[s+1]*.58f*(float)Math.sin(a0),z[s+1]},new float[]{x+r[s+1]*(float)Math.cos(a1),-.11f+r[s+1]*.58f*(float)Math.sin(a1),z[s+1]},new float[]{x+r[s]*(float)Math.cos(a1),-.11f+r[s]*.58f*(float)Math.sin(a1),z[s]});
        }
    }
    ...
    private void prism(float[][] top,float thickness){if(top==null||top.length<3)return;int n=top.length;float[][] bot=new float[n][3];for(int i=0;i<n;i++)bot[i]=offset(top[i],0,-thickness,0);for(int i=1;i<n-1;i++){tri(top[0],top[i],top[i+1]);tri(bot[0],bot[i+1],bot[i]);}for(int i=0;i<n;i++)quad(top[i],bot[i],bot[(i+1)%n],top[(i+1)%n]);}
