...
public final class WeatherEffectsView extends CinematicEnvironmentView {
    ...
    @Override protected void onDraw(Canvas c){
        ...
        drawTimeTint(c,w,h,now,day01);drawWeather(c,w,h,now);postInvalidateOnAnimation();
    }
    ...
    private void drawTimeTint(Canvas c,int w,int h,long now,float day01){
        switch(dayPhase){
            case DAWN:tint(c,w,h,0x48201c43,0x31ff9a59);break;
            case MORNING:tint(c,w,h,0x109bd8ef,0x08ffe4ad);break;
            case NOON:tint(c,w,h,0x067fc7e9,0x03ffffff);break;
            case SUNSET:tint(c,w,h,0x4f422362,0x42ff713d);break;
            case EVENING:tint(c,w,h,0x78161b35,0x632b263a);drawStars(c,w,h,now,28,.40f);break;
    ...
    private void drawWeather(Canvas c,int w,int h,long now){
        ...
        else if(weather==CLOUDY){tint(c,w,h,0x24394852,0x124b5962);drawClouds(c,w,h,now,0x8caeb9bf,cloudCount);}
        else if(weather==RAIN){tint(c,w,h,0x4c172836,0x383a4d5b);drawClouds(c,w,h,now,0xa84a5862,cloudCount);drawRain(c,w,h,now);drawRainFlash(c,w,h,now);}
        else{tint(c,w,h,0x2e404f5c,0x2291a5b3);drawClouds(c,w,h,now,0x92c7d0d5,cloudCount);drawSnow(c,w,h,now);}
