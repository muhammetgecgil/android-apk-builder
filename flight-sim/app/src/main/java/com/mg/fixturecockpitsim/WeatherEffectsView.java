...
/** AVM-12.9: continuous day/night cycle plus independent weather with distributed rain. */
public final class WeatherEffectsView extends View {
    ...
    private void drawSun(Canvas c,float x,float y,float r,int color,float visibility){
    ...
    private void drawMoon(Canvas c,float x,float y,float r,float visibility){
        int glow=(int)(0x55*visibility),solid=(int)(255*visibility);p.setColor((glow<<24)|0x00d9e8ff);c.drawCircle(x,y,r*1.8f,p);p.setColor((solid<<24)|0x00e6efff);c.drawCircle(x,y,r,p);p.setColor(((int)(230*visibility)<<24)|0x00172238);c.drawCircle(x+r*.34f,y-r*.08f,r*.88f,p);
    }
    ...
    private void drawStars(Canvas c,int w,int h,long now,int count,float visibility){
        ...
        for(int i=0;i<n;i++){int alpha=(int)(255*starA[i]*(i%5==0?pulse:1)*visibility);alpha=Math.max(12,Math.min(255,alpha));p.setColor((alpha<<24)|0x00e9f4ff);float r=i%9==0?2.7f:i%3==0?1.8f:1.1f;c.drawCircle(starX[i]*w,starY[i]*h,r,p);}
    }

    private void drawClouds(Canvas c,int w,int h,long now,int color,int count){
        float speed=(windy?.000030f:.000015f)*(1f+windStrength*.75f),travel=(now-weatherEpochMs)*speed*w*windSign;
        for(int i=0;i<count;i++){
            float period=w*1.30f,base=(i*.173f*w+travel*(.62f+(i%3)*.15f))%period;if(base<0)base+=period;base-=w*.15f;
            float y=h*(.07f+(i%4)*.073f),cw=w*(.075f+.018f*(i%3)),ch=h*(.018f+.004f*(i%2));
    ...
    private void drawRain(Canvas c,int w,int h,long now){
        stroke.setColor(0x68aed8ef);stroke.setStrokeWidth(Math.max(1.0f,w*.0009f));
        long elapsed=Math.max(0L,now-weatherEpochMs);
        float fall=(elapsed%1700L)/1700f;
        float travel=(elapsed%12000L)/12000f;
        float period=w+140f;
        float slant=windSign*w*(.004f+.011f*windStrength);
        // Use bounded elapsed time, not epoch-sized floats. This keeps drops spread across the whole screen
        // and prevents the former bright vertical/diagonal "rain curtain" at one edge.
        for(int i=0;i<64;i++){
            float raw=i*83.73f+travel*period*4.2f*windSign;
            float x=((raw%period)+period)%period-70f;
            float y=((i*.071f+fall*(.82f+(i%5)*.035f))%1f)*h;
            float len=h*(.018f+.006f*(i%4));
            c.drawLine(x,y,x+slant,y+len,stroke);
        }
    }
    ...
    private void drawSnow(Canvas c,int w,int h,long now){
        float fall=(now%7000L)/7000f,drift=windSign*w*.025f*windStrength;
        for(int i=0;i<76;i++){float x=(i*83.2f+(float)Math.sin(now*.001+i)*24f+drift*(i%5))%w;if(x<0)x+=w;float y=((i*.083f+fall*(.70f+(i%4)*.09f))%1f)*h,r=1.8f+(i%5)*.55f;p.setColor(i%6==0?0xe8ffffff:0xb8edf7ff);c.drawCircle(x,y,r,p);}
    }

    private void drawWind(Canvas c,int w,int h,long now){
        stroke.setColor(weather==RAIN?0x245fc2dd:0x22d9eff8);stroke.setStrokeWidth(Math.max(1f,w*.0007f));float phase=(now%2800L)/2800f;
        for(int i=0;i<16;i++){float y=h*(.10f+(i%8)*.09f),x=((i*149f+phase*w*1.2f*windSign)%w+w)%w,len=w*(.015f+.014f*windStrength);c.drawLine(x,y,x+windSign*len,y,stroke);}
    }
