...
/** V10 visual wrapper: high-detail fighter over a moving mountain world,
 * refined wing cues and symmetric under-wing simulator stores. */
public final class JetWingStoreView extends FrameLayout {
    ...
    private static final class StoresOverlay extends View {
        ...
        @Override protected void onDraw(Canvas c){
            ...
            p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(Math.max(2,w*.0024f));p.setColor(0x889EB0BB);
            ...
            p.setColor(0x665C6870);Path te=new Path();te.moveTo(cx-span*.16f,cy+span*.10f);te.lineTo(cx-span*.62f,cy+span*.24f);te.moveTo(cx+span*.16f,cy+span*.10f);te.lineTo(cx+span*.62f,cy+span*.24f);c.drawPath(te,p);
