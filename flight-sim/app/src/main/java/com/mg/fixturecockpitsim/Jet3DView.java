...
public final class Jet3DView extends GLSurfaceView {
    ...
    private static final class JetRenderer implements GLSurfaceView.Renderer{
        ...
        private volatile float targetRoll,targetPitch,targetYaw,throttle=.62f,targetGear=1f;private volatile boolean live;
        private float roll,pitch,yaw,gear=1f;private int cameraMode;
        private int program,aPos,aNormal,aPart,uMvp,uModel,uColor,uLightDir,uThrottle,uSurfPitch,uSurfRoll,uSurfYaw,uGear;
        ...
        void setTelemetry(float r,float p,float y,float t,boolean l){
            targetRoll=r;targetPitch=p;targetYaw=y;throttle=Math.max(0f,Math.min(1f,t));live=l;
            if(!l)targetGear=1f;else if(throttle>.70f)targetGear=0f;else if(throttle<.48f)targetGear=1f;
        }
        ...
        @Override public void onDrawFrame(GL10 gl){
            long now=System.nanoTime();float dt=Math.min(.05f,Math.max(.001f,(now-lastNs)/1_000_000_000f));lastNs=now;
            float k=1f-(float)Math.exp(-dt*8f);roll+=shortest(targetRoll-roll)*k;pitch+=(targetPitch-pitch)*k;yaw+=shortest(targetYaw-yaw)*k*.65f;
            float kg=1f-(float)Math.exp(-dt*2.15f);gear+=(targetGear-gear)*kg;
