...
public final class Jet3DView extends GLSurfaceView {
    ...
    public void setTelemetry(float a,float b,float c,float d,float e,int x,boolean live){
        r.tele(a,b,c,d,live);
        st=Math.max(0,Math.min(1,d));
        sound.update(st,st*230,sg,sb,ground);
    }
    public void setSimulationState(float g,float m,float n,float b,boolean gr){
        r.sim(g,m,n);sg=g;sb=b;ground=gr;sound.update(st,st*230,sg,sb,ground);
    }
    public void setFlightMotion(float speed,float vertical,boolean gr){r.speed=Math.max(0,speed);r.vertical=vertical;r.onGround=gr;}
    ...
    public boolean onTouchEvent(MotionEvent e){if(e.getAction()==1)r.cam=(r.cam+1)%4;return true;}
    public void onResume(){super.onResume();sound.start();}
    public void onPause(){sound.stop();super.onPause();}
    ...
    private static final class R implements Renderer {
        ...
        final AircraftControlSurfaces surfaceMix=new AircraftControlSurfaces();
        ...
        float stabL,stabR,rudL,rudR,flapL,flapR;
        int pg,ap,an,apart,umvp,umodel,uc,ul,ut,ug,um,un,uws;
        int usl,usr,url,urr,ufl,ufr;
        FloatBuffer vb;
        int count;
        long last;
        ...
        public void onSurfaceCreated(GL10 g,EGLConfig e){
            ...
            url=GLES20.glGetUniformLocation(pg,"uRudL");
            urr=GLES20.glGetUniformLocation(pg,"uRudR");
            ...
            vb=ByteBuffer.allocateDirect(m.data.length*4).order(ByteOrder.nativeOrder()).asFloatBuffer();
            vb.put(m.data).position(0);
            count=m.vertexCount();
            last=System.nanoTime();
        }

        public void onSurfaceChanged(GL10 g,int w,int h){GLES20.glViewport(0,0,w,h);aspect=(float)w/Math.max(1,h);}

        public void onDrawFrame(GL10 g){
            long n=System.nanoTime();
            float dt=Math.min(.05f,Math.max(.001f,(n-last)/1e9f));
            last=n;t+=dt;
            float oldRoll=roll,oldPitch=pitch,oldYaw=yaw;
            float k=1-(float)Math.exp(-dt*8),kg=1-(float)Math.exp(-dt*2.2);
            roll+=(tr-roll)*k;
            pitch+=(tp-pitch)*k;
            yaw+=shortest(ty-yaw)*k*.65f;
            gear+=(tg-gear)*kg;mc+=(tm-mc)*kg;nc+=(tn-nc)*kg;
            if(gear>.72f)spin=(spin+ws*dt*2.55f)%6.2831855f;

            // Infer the active maneuver demand from aircraft attitude motion. This makes the
            // surfaces respond in every control mode without changing the Bluetooth protocol.
            float rollRate=(roll-oldRoll)/Math.max(.001f,dt);
            float pitchRate=(pitch-oldPitch)/Math.max(.001f,dt);
            float yawRate=shortest(yaw-oldYaw)/Math.max(.001f,dt);
            float rollCmd=cl(shortest(tr-roll)/19f+rollRate/145f,-1,1);
            float pitchCmd=cl((tp-pitch)/11f+pitchRate/82f,-1,1);
            float yawCmd=cl(shortest(ty-yaw)/27f+yawRate/78f,-1,1);
            surfaceMix.update(pitchCmd,rollCmd,yawCmd,thr,gear,speed,onGround);
            float ks=1-(float)Math.exp(-dt*(onGround?7.5f:10.5f));
            stabL+=(surfaceMix.leftStabilatorDeg-stabL)*ks;
            stabR+=(surfaceMix.rightStabilatorDeg-stabR)*ks;
            rudL+=(surfaceMix.leftRudderDeg-rudL)*ks;
            rudR+=(surfaceMix.rightRudderDeg-rudR)*ks;
            flapL+=(surfaceMix.leftFlaperonDeg-flapL)*ks;
            flapR+=(surfaceMix.rightFlaperonDeg-flapR)*ks;

            float sp=cl(speed/270f,0,1),fov=32f+sp*6f;
            ...
            final float d2r=.01745329252f;
            GLES20.glUniform1f(usl,stabL*d2r);
            GLES20.glUniform1f(usr,stabR*d2r);
            GLES20.glUniform1f(url,rudL*d2r);
            GLES20.glUniform1f(urr,rudR*d2r);
            GLES20.glUniform1f(ufl,flapL*d2r);
            GLES20.glUniform1f(ufr,flapR*d2r);

            vb.position(0);GLES20.glEnableVertexAttribArray(ap);GLES20.glVertexAttribPointer(ap,3,5126,false,28,vb);
            vb.position(3);GLES20.glEnableVertexAttribArray(an);GLES20.glVertexAttribPointer(an,3,5126,false,28,vb);
            vb.position(6);GLES20.glEnableVertexAttribArray(apart);GLES20.glVertexAttribPointer(apart,1,5126,false,28,vb);
            GLES20.glDrawArrays(4,0,count);
        }

        void camera(float sp){
            float lag=sp*2.1f,bob=(float)Math.sin(t*1.5f)*.035f*(1-sp);
            float climbLook=cl(vertical/30f,-.18f,.42f);
            if(cam==1)Matrix.setLookAtM(vw,0,0,1.7f,12.8f+lag,0,-.05f-climbLook*.2f,1.6f,0,1,0);
            else if(cam==2)Matrix.setLookAtM(vw,0,12.4f+sp*.8f,5.0f,13.2f+lag,0,-.10f-climbLook,.15f,0,1,0);
            else if(cam==3)Matrix.setLookAtM(vw,0,-12.4f-sp*.8f,5.0f,13.2f+lag,0,-.10f-climbLook,.15f,0,1,0);
            else Matrix.setLookAtM(vw,0,0,5.1f+bob+Math.max(0,climbLook)*1.4f,19.5f+lag,0,.05f-climbLook*1.8f,-.75f-sp*.28f,0,1,0);
        }
        ...
        static int sh(int t,String s){int x=GLES20.glCreateShader(t);GLES20.glShaderSource(x,s);GLES20.glCompileShader(x);int[]o=new int[1];GLES20.glGetShaderiv(x,GLES20.GL_COMPILE_STATUS,o,0);if(o[0]==0)throw new IllegalStateException(GLES20.glGetShaderInfoLog(x));return x;}
        static int prog(String v,String f){int p=GLES20.glCreateProgram();GLES20.glAttachShader(p,sh(35633,v));GLES20.glAttachShader(p,sh(35632,f));GLES20.glLinkProgram(p);int[]o=new int[1];GLES20.glGetProgramiv(p,GLES20.GL_LINK_STATUS,o,0);if(o[0]==0)throw new IllegalStateException(GLES20.glGetProgramInfoLog(p));return p;}

        static final String VS=
            "uniform mat4 uMvp,uModel;"+
            "uniform float uThrottle,uGear,uMainComp,uNoseComp,uWheelSpin;"+
            "uniform float uStabL,uStabR,uRudL,uRudR,uFlapL,uFlapR;"+
            ...
            "void main(){vec3 p=aPos,n=aNormal;vR=0.;"+
            // Stabilators: rotate y/z around an approximate all-moving-tail hinge.
            "if(aPart>3.5&&aPart<5.5){float a=aPart<4.5?uStabL:uStabR;vec2 piv=vec2(.29,1.88);p.yz=rr(a)*(p.yz-piv)+piv;n.yz=rr(a)*n.yz;}"+
            // Flaperons: rotate y/z around the leading hinge line of each surface.
            "else if(aPart>8.5&&aPart<10.5){float a=aPart<9.5?uFlapL:uFlapR;vec2 piv=vec2(.20,.76);p.yz=rr(a)*(p.yz-piv)+piv;n.yz=rr(a)*n.yz;}"+
            // Twin rudders: yaw around an approximate vertical hinge; left/right use the same command.
            "else if(aPart>5.5&&aPart<7.5){float a=aPart<6.5?uRudL:uRudR;float sx=p.x<0.?-1.:1.;vec2 piv=vec2(.82*sx,2.02);p.xz=rr(a)*(p.xz-piv)+piv;n.xz=rr(a)*n.xz;}"+
