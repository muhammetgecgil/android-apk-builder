...
public final class Jet3DView extends GLSurfaceView {
    ...
    private static final class R implements Renderer {
        ...
        @Override public void onSurfaceCreated(GL10 g,EGLConfig e){
            ...
            float[] es=EngineDynamicsOverlay.buildSolid();engineSolidBuffer=buffer(es);engineSolidCount=es.length/7;
            float[] et=EngineDynamicsOverlay.buildTransparent();engineTransparentBuffer=buffer(et);engineTransparentCount=et.length/7;
            ...
            float[] vortex=WingtipVortexMesh.build();vortexBuffer=buffer(vortex);vortexCount=vortex.length/7;float[] bird=buildBirdMesh();birdBuffer=buffer(bird);birdCount=bird.length/7;last=System.nanoTime();
        }
        ...
        static final String VS=
                ...
                "if(aPart>20.5&&aPart<21.5){float op=mix(.92,1.115,smoothstep(.08,1.,uThrottle));float cx=p.x<0.?-.72:.72;float aft=smoothstep(3.28,3.88,p.z);float sc=mix(1.,op,aft);p.x=cx+(p.x-cx)*sc;p.y=-.10+(p.y+.10)*sc;}"+
                ...
                "if(aPart>45.5&&aPart<46.5){float cx=p.x<0.?-.94:.94;float a=uTime*(5.+uThrottle*74.);vec2 q=vec2(p.x-cx,(p.y+.09)/.74);q=rr(a)*q;p.x=cx+q.x;p.y=-.09+q.y*.74;vec2 nq=vec2(n.x,n.y/.74);nq=rr(a)*nq;n.x=nq.x;n.y=nq.y*.74;}"+
                "if(aPart>46.5&&aPart<47.5){float th=smoothstep(.16,1.,uThrottle),q=clamp((p.z-3.85)/2.85,0.,1.);p.x+=sin(uTime*18.+p.z*7.1+p.y*11.)*.030*th*(.45+q);p.y+=cos(uTime*15.7+p.z*8.3+p.x*9.)*.018*th*(.45+q);}"+
                "if(aPart>47.5&&aPart<48.5){float ab=smoothstep(.72,.89,uThrottle);float cx=p.x<0.?-.72:.72;float pulse=1.+.035*sin(uTime*24.+p.z*14.);p.x=cx+(p.x-cx)*mix(1.,pulse,ab);p.y=-.10+(p.y+.10)*mix(1.,pulse,ab);p.z+=.022*sin(uTime*27.+p.z*17.)*ab;}"+
                ...
                "if((aPart>1.5&&aPart<2.5)||(aPart>7.5&&aPart<8.5)||(aPart>11.5&&aPart<12.5)||(aPart>20.5&&aPart<22.5)||(aPart>27.5&&aPart<28.5)||(aPart>46.5&&aPart<48.5)){float a=uVector*d;vec2 piv=vec2(-.10,3.18);p.yz=rr(a)*(p.yz-piv)+piv;n.yz=rr(a)*n.yz;}"+
        ...
        static final String FS=
                ...
                "if(vP>46.5&&vP<47.5){float th=smoothstep(.12,1.,uThrottle),q=sat((vPos.z-3.82)/2.9),wave=.50+.50*sin(vPos.z*11.+uTime*13.+vPos.x*17.);vec3 R=reflect(normalize(vN),normalize(uCameraPos-vPos));vec3 haze=mix(vec3(.28,.32,.35),envc(R),.58);float a=th*(.012+.045*q)*(.55+.45*wave);gl_FragColor=vec4(haze,a);return;}"+
                ...
                "if(vP>48.5&&vP<49.5){float rpm=smoothstep(.18,.92,uThrottle),pulse=.70+.30*sin(uTime*(36.+uThrottle*90.));gl_FragColor=vec4(vec3(.33,.38,.42)*(.72+.18*pulse),rpm*(.025+.105*rpm));return;}"+
