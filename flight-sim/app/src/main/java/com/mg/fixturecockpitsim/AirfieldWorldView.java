...
/** AVM-12.8 world view: post-hangar airfield, manual approach perspective and runway growth with descent. */
public final class AirfieldWorldView extends View {
    ...
    @Override protected void onDraw(Canvas c){
        ...
        // Nose down shows more ground; nose up shows more sky, without making the runway jump abruptly.
        if(!onGround)horizon+=h*clamp(pitchDeg/32f,-.095f,.095f);
        if(approach){float a=approach01();horizon=lerp(h*.215f,h*.405f,a)+h*clamp(pitchDeg/35f,-.065f,.065f);}
        drawSky(c,w,h,horizon);
        if(phase.contains("ORBIT")&&altitudeM>520f&&!approach)drawHighTerrain(c,w,h,horizon);else drawRunwayWorld(c,w,h,horizon);
        if(crashed)drawCrash(c,w,h);if(speedMps>.3f||crashed)postInvalidateOnAnimation();
    }

    private boolean isApproachScene(){
        if(onGround)return false;
        if(phase.contains("APPROACH")||phase.contains("RWY_CAPTURE_AIR"))return true;
        float hdgErr=Math.abs(angleError(headingDeg,270f));
        return altitudeM<950f&&hdgErr<88f;
    }
    private float approach01(){return 1f-clamp((altitudeM-4f)/920f,0f,1f);}
    ...
    private void drawHighTerrain(Canvas c,int w,int h,float hz){
        ...
        for(int i=0;i<38;i++){float depth=.05f+((i*19)%37)/40f,y=hz+gh*depth*depth,x=(i*131f+alongTrackM*.45f)%Math.max(1,w),ww=w*(.018f+.052f*depth),hh=h*(.003f+.009f*depth);p.setColor(i%3==0?0x22334d2d:i%3==1?0x1f49603a:0x1c263c2b);c.drawOval(x,y,x+ww,y+hh,p);}drawRunway(c,w,h,hz,false);
    }
    ...
    private void drawRunway(Canvas c,int w,int h,float hz,boolean ground){
        ...
        if(ground){
            ...
            float a=approach01();
            // High approach: compact runway seen from above. As altitude falls it expands continuously
            // into a normal landing perspective and reaches the bottom of the display near touchdown.
            nearHalf=w*lerp(.052f,.405f,(float)Math.pow(a,.78));
            farHalf=w*lerp(.025f,.040f,a);
            fy=lerp(hz+h*.026f,hz+h*.050f,a);
            ny=lerp(hz+h*.28f,h*.985f,(float)Math.pow(a,.72));
            if(!isApproachScene()){nearHalf*=.78f;ny=lerp(hz+h*.24f,h*.77f,clamp(1-altitudeM/1400f,0,1));}
        }

        quad(c,cx-farHalf*1.52f,fy,cx+farHalf*1.52f,fy,cx+nearHalf*1.20f,ny,cx-nearHalf*1.20f,ny,0xff777a72);
        path.reset();path.moveTo(cx-farHalf,fy);path.lineTo(cx+farHalf,fy);path.lineTo(cx+nearHalf,ny);path.lineTo(cx-nearHalf,ny);path.close();p.setShader(new LinearGradient(cx,fy,cx,ny,new int[]{0xff3a3e40,0xff292d2f,0xff1d2224},null,Shader.TileMode.CLAMP));c.drawPath(path,p);p.setShader(null);
        ...
        for(int i=0;i<42;i++){float q=(i/42f+scroll)%1f,z=q*q,y=lerp(fy,ny,z),y2=Math.min(ny,y+3+46*z);stroke.setStrokeWidth(1.5f+10*z);c.drawLine(cx,y,cx,y2,stroke);}
        for(int i=0;i<34;i++){float q=(i/34f+scroll*.62f)%1f,z=q*q,y=lerp(fy,ny,z),hh=lerp(farHalf,nearHalf,z),r=1+4*z;p.setColor(i>29?0xffffd465:0xfff5f0d9);c.drawCircle(cx-hh,y,r,p);c.drawCircle(cx+hh,y,r,p);}

        // Threshold bars and aiming blocks get larger as the runway approaches.
    ...
    private void drawApproachGuidance(Canvas c,int w,int h,float cx,float fy,float ny,float farHalf,float nearHalf,float headingErr){
        float a=approach01();
        // PAPI-style visual reference: two white/two red around a nominal glide path, without auto-flying the aircraft.
        float py=lerp(fy,ny,.46f),ph=lerp(farHalf,nearHalf,.46f),r=Math.max(2,w*(.0023f+.0015f*a));
