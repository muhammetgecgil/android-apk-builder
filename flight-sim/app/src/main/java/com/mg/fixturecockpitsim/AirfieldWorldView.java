...
/** AVM-13.1 world: varied scenic cruise plus runway hidden above 1000 m. */
public final class AirfieldWorldView extends View {
    ...
    private long lastNs;
    private float scroll,scenicClock;
    ...
    private void drawRunwayWorld(Canvas c,int w,int h,float hz){
        p.setShader(new LinearGradient(0,hz,0,h,new int[]{0xff6f8d5f,0xff587650,0xff416246},null,Shader.TileMode.CLAMP));c.drawRect(0,hz,w,h,p);p.setShader(null);drawMountains(c,w,h,hz,.18f);if(runwayVisible())drawRunway(c,w,h,hz,onGround);
    ...
    private void drawRunway(Canvas c,int w,int h,float hz,boolean ground){
        ...
        for(int i=0;i<42;i++){float q=(i/42f+scroll)%1f,z=q*q,y=lerp(fy,ny,z),y2=Math.min(ny,y+3+46*z);stroke.setStrokeWidth(1.5f+10*z);c.drawLine(cx,y,cx,y2,stroke);}for(int i=0;i<34;i++){float q=(i/34f+scroll*.62f)%1f,z=q*q,y=lerp(fy,ny,z),hh=lerp(farHalf,nearHalf,z),r=1+4*z;p.setColor(i>29?0xffffd465:0xfff5f0d9);c.drawCircle(cx-hh,y,r,p);c.drawCircle(cx+hh,y,r,p);}for(int set=0;set<4;set++){float z=.30f+set*.085f,y=lerp(fy,ny,z*z),hh=lerp(farHalf,nearHalf,z),bw=Math.max(2,hh*.075f),bh=4+19*z;p.setColor(0xfff6f5ef);c.drawRect(cx-hh*.54f-bw,y,cx-hh*.54f+bw,y+bh,p);c.drawRect(cx+hh*.54f-bw,y,cx+hh*.54f+bw,y+bh,p);}
        if(!ground&&isApproachScene())drawApproachGuidance(c,w,h,cx,fy,ny,farHalf,nearHalf,headingErr);if(ground&&alongTrackM<115f){float z=clamp(.48f+alongTrackM/260f,0,1),y=lerp(fy,ny,z*z);p.setColor(0xfffaf9f4);p.setTextAlign(Paint.Align.CENTER);p.setTextSize(Math.max(30,w*(.038f+.02f*z)));c.drawText("27",cx,Math.min(ny-6,y+54),p);p.setTextAlign(Paint.Align.LEFT);}
    }
