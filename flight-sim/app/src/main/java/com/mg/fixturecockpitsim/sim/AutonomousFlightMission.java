...
/** Continuous hangar -> taxi -> runway -> takeoff -> 5 min scenic flight -> return/landing. */
public final class AutonomousFlightMission {
    ...
    public double getPhaseProgress01(){
        double duration;
        switch(phase){case HANGAR_START:duration=4.0;break;case TAXI_OUT:duration=14.0;break;case RUNWAY_HOLD:duration=2.0;break;case TAKEOFF_ROLL:duration=12.0;break;case ROTATE_CLIMB:duration=16.0;break;case ORBIT:duration=SCENIC_DURATION_SEC;break;case APPROACH:duration=30.0;break;case FLARE:duration=8.0;break;case ROLLOUT:duration=15.0;break;default:duration=1.0;}
    ...
    public void update(FlightState s,FlightControls c,double dt){phaseTime+=dt;c.pitch=c.roll=c.yaw=c.brake=0;
        switch(phase){
            case HANGAR_START:
                c.throttle=.08;c.brake=1;c.gearDown=true;c.yaw=headingError(s.headingDeg,RUNWAY_HEADING_DEG)*.05;
                if(phaseTime>=4.0)next(Phase.TAXI_OUT);break;
            case TAXI_OUT:
                c.throttle=.16;c.brake=0;c.gearDown=true;
                // Hangar, apron and taxiway are laid out parallel to runway. Keep nose aligned throughout.
                c.yaw=headingError(s.headingDeg,RUNWAY_HEADING_DEG)*.055;
                if(s.trueAirspeedMps>12)c.brake=.20;
                if(phaseTime>=14.0)next(Phase.RUNWAY_HOLD);break;
            case RUNWAY_HOLD:c.throttle=.10;c.brake=1;c.gearDown=true;c.yaw=headingError(s.headingDeg,RUNWAY_HEADING_DEG)*.05;if(phaseTime>=2.0)next(Phase.TAKEOFF_ROLL);break;
            ...
            case ROLLOUT:c.gearDown=true;c.throttle=0;c.brake=.88;c.yaw=headingError(s.headingDeg,RUNWAY_HEADING_DEG)*.05;if(s.trueAirspeedMps<2&&phaseTime>2)next(Phase.COMPLETE);break;
            case COMPLETE:c.gearDown=true;c.throttle=0;c.brake=1;c.yaw=headingError(s.headingDeg,RUNWAY_HEADING_DEG)*.05;break;
        }c.clamp();}
