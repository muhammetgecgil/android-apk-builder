...
/** Continuous demo loop: realistic hangar -> taxi -> RWY27 -> takeoff -> scenic flight -> RWY27 stabilized landing -> hangar. */
public final class AutonomousFlightMission {
 ...
 public void update(FlightState s,FlightControls c,double dt){phaseTime+=dt;c.pitch=c.roll=c.yaw=c.brake=0;switch(phase){
  case HANGAR_START:c.throttle=.07;c.brake=1;c.gearDown=true;align(c,s,.07);if(phaseTime>=5)next(Phase.TAXI_OUT);break;
  case TAXI_OUT:c.throttle=.15;c.gearDown=true;align(c,s,.075);if(s.trueAirspeedMps>11)c.brake=.18;if(phaseTime>=18)next(Phase.RUNWAY_HOLD);break;
  case RUNWAY_HOLD:c.throttle=.11;c.brake=1;c.gearDown=true;align(c,s,.09);if(phaseTime>=3)next(Phase.TAKEOFF_ROLL);break;
  case TAKEOFF_ROLL:c.throttle=1;c.gearDown=true;align(c,s,.095);if(s.trueAirspeedMps>=86||phaseTime>=14)next(Phase.ROTATE_CLIMB);break;
  ...
  case ROLLOUT:c.gearDown=true;c.throttle=0;c.brake=.86;align(c,s,.10);if(s.trueAirspeedMps<2&&phaseTime>2)next(Phase.TAXI_IN);break;
  case TAXI_IN:c.gearDown=true;c.throttle=.12;align(c,s,.075);if(s.trueAirspeedMps>9)c.brake=.18;if(phaseTime>=18)next(Phase.HANGAR_PARK);break;
  case HANGAR_PARK:c.gearDown=true;c.throttle=.04;c.brake=.75;align(c,s,.075);if(phaseTime>=6)reset(s);break;
  case COMPLETE:c.gearDown=true;c.throttle=0;c.brake=1;break;}c.clamp();}
 private static void align(FlightControls c,FlightState s,double k){c.yaw=clamp(headingError(s.headingDeg,RUNWAY_HEADING_DEG)*k,-.45,.45);}
