...
/** Continuous demo loop with stabilized approach monitoring and one automatic go-around opportunity. */
public final class AutonomousFlightMission {
 public enum Phase { HANGAR_START,TAXI_OUT,RUNWAY_HOLD,TAKEOFF_ROLL,ROTATE_CLIMB,ORBIT,APPROACH,GO_AROUND,FLARE,ROLLOUT,TAXI_IN,HANGAR_PARK,COMPLETE }
 ...
 private Phase phase=Phase.HANGAR_START;private double phaseTime,orbitTime;private int goAroundCount;
 public void reset(FlightState s){phase=Phase.HANGAR_START;phaseTime=orbitTime=0;goAroundCount=0;s.timeSec=0;s.altitudeM=0;s.trueAirspeedMps=0;s.indicatedAirspeedMps=0;s.verticalSpeedMps=0;s.headingDeg=RUNWAY_HEADING_DEG;s.pitchDeg=s.rollDeg=0;s.throttle=0;s.gearPosition=1;s.brake01=1;s.onGround=true;}
 public Phase getPhase(){return phase;}public double getOrbitTimeSec(){return orbitTime;}public double getPhaseTimeSec(){return phaseTime;}public int getGoAroundCount(){return goAroundCount;}
 public double getPhaseProgress01(){double d;switch(phase){case HANGAR_START:d=5;break;case TAXI_OUT:d=18;break;case RUNWAY_HOLD:d=3;break;case TAKEOFF_ROLL:d=14;break;case ROTATE_CLIMB:d=18;break;case ORBIT:d=300;break;case APPROACH:d=32;break;case GO_AROUND:d=18;break;case FLARE:d=6;break;case ROLLOUT:d=17;break;case TAXI_IN:d=18;break;case HANGAR_PARK:d=6;break;default:d=1;}return clamp(phaseTime/d,0,1);}
 public void update(FlightState s,FlightControls c,double dt){phaseTime+=dt;c.pitch=c.roll=c.yaw=c.brake=0;switch(phase){
  ...
  case APPROACH:{c.gearDown=true;double q=clamp(phaseTime/32.0,0,1),desiredAlt=360.0*(1.0-q)+5.5*q;c.throttle=s.altitudeM>180?.38:s.altitudeM>65?.31:.27;double err=headingError(s.headingDeg,RUNWAY_HEADING_DEG);c.roll=clamp(err/55.0,-.18,.18);c.yaw=clamp(err*.040,-.58,.58);double altErr=desiredAlt-s.altitudeM;c.pitch=clamp(altErr/180.0,-.24,.028);
   boolean unstable=s.altitudeM<95&&phaseTime>9&&(Math.abs(err)>16.0||s.verticalSpeedMps<-14.0||s.indicatedAirspeedMps<54.0||s.stallWarning);
   if(unstable&&goAroundCount<1){goAroundCount++;next(Phase.GO_AROUND);}else if(s.altitudeM<=6.5||phaseTime>=36)next(Phase.FLARE);break;}
  case GO_AROUND:{c.throttle=1.0;c.pitch=.34;double err=headingError(s.headingDeg,RUNWAY_HEADING_DEG);c.roll=clamp(err/60.0,-.22,.22);c.yaw=clamp(err*.025,-.35,.35);c.gearDown=s.altitudeM<70;if(s.altitudeM>240&&phaseTime>8)next(Phase.APPROACH);break;}
  case FLARE:{c.gearDown=true;c.throttle=.13;double ferr=headingError(s.headingDeg,RUNWAY_HEADING_DEG);c.roll=clamp(ferr/95.0,-.08,.08);c.yaw=clamp(ferr*.055,-.65,.65);c.pitch=s.altitudeM>2.2?-.018:s.altitudeM>.45?.055:.025;if(s.onGround||s.altitudeM<=.05||phaseTime>7)next(Phase.ROLLOUT);break;}
