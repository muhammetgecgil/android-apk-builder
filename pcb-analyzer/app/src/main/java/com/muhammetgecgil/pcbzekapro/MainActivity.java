...
public class MainActivity extends AppCompatActivity {
    ...
    private void buildUi(){
        ...
        top.addView(label("PCB ZEKÂ PRO v3 • MUHAMMET GEÇGİL",17,Color.WHITE));status=label("Kamera hazırlanıyor…",12,0xFF65FFF5);top.addView(status);summary=label("BOM: 0 • SÜPER OCR hazır • R/C/L/D/Q/U",12,0xFFFFD166);top.addView(summary);root.addView(top,new FrameLayout.LayoutParams(-1,-2,Gravity.TOP));
        ...
        Button board=button("8 KARE KART TARA"),bands=button("DİRENÇ RENK"),ocr=button("SÜPER OCR"),flash=button("FLAŞ"),bomBtn=button("BOM"),data=button("DATASHEET"),fault=button("ARIZA"),meter=button("MULTİMETRE"),reference=button("REFERANS KART"),compare=button("KARŞILAŞTIR"),pdf=button("PDF RAPOR"),share=button("CSV PAYLAŞ"),clear=button("TEMİZLE");
        for(Button x:new Button[]{board,bands,ocr,flash,bomBtn,data,fault,meter,reference,compare,pdf,share,clear})buttons.addView(x,new LinearLayout.LayoutParams(dp(138),dp(48)));hs.addView(buttons);bottom.addView(hs);
        ...
        board.setOnClickListener(v->requestMulti(Mode.BOARD,"8 kareli kart OCR + BOM"));bands.setOnClickListener(v->request(Mode.BANDS,"Renk/ışık kalibrasyonu ve bant analizi"));ocr.setOnClickListener(v->requestMulti(Mode.OCR,"8 kareli süper OCR"));
        ...
        bomBtn.setOnClickListener(v->showBom());data.setOnClickListener(v->datasheet());fault.setOnClickListener(v->faultReport());meter.setOnClickListener(v->meterGuide());reference.setOnClickListener(v->saveReference());compare.setOnClickListener(v->compareReference());pdf.setOnClickListener(v->createPdf());share.setOnClickListener(v->shareCsv());clear.setOnClickListener(v->{bom.clear();scanNo=1;lastCode="";multiText.setLength(0);overlay.clear();refresh();result.setText("Sonuçlar temizlendi.");});
    ...
    private void createPdf(){try{File dir=new File(getCacheDir(),"reports");dir.mkdirs();File f=new File(dir,"PCB_Zeka_Pro_Servis_Raporu.pdf");PdfDocument doc=new PdfDocument();Paint p=new Paint();p.setColor(Color.BLACK);p.setTextSize(13);int pageNo=1,index=0;while(index<Math.max(1,bom.size())){PdfDocument.Page page=doc.startPage(new PdfDocument.PageInfo.Builder(595,842,pageNo++).create());page.getCanvas().drawText("PCB ZEKÂ PRO v3 - SERVİS RAPORU",40,45,p);page.getCanvas().drawText("Muhammet Geçgil • BOM: "+bom.size(),40,70,p);int y=105;while(index<bom.size()&&y<800){page.getCanvas().drawText(bom.get(index++).line(),30,y,p);y+=22;}if(bom.isEmpty()){page.getCanvas().drawText("Henüz tarama sonucu yok.",40,y,p);index++;}doc.finishPage(page);}try(FileOutputStream o=new FileOutputStream(f)){doc.writeTo(o);}doc.close();Uri u=FileProvider.getUriForFile(this,getPackageName()+".files",f);Intent i=new Intent(Intent.ACTION_SEND);i.setType("application/pdf");i.putExtra(Intent.EXTRA_STREAM,u);i.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);startActivity(Intent.createChooser(i,"PDF servis raporunu kaydet/paylaş"));}catch(Exception e){dialog("PDF HATASI",e.getMessage()==null?"Rapor oluşturulamadı":e.getMessage());}}
