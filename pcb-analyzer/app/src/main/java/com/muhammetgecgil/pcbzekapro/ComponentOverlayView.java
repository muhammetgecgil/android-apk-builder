...
final class ComponentOverlayView extends View {
    ...
    void showProbes(boolean on){probes=on;postInvalidate();}
    @Override protected void onDraw(Canvas c){super.onDraw(c);float sx=getWidth()/(float)imageW,sy=getHeight()/(float)imageH;for(Text.TextBlock b:blocks){Rect r=b.getBoundingBox();if(r==null)continue;float l=r.left*sx,t=r.top*sy,rr=r.right*sx,bb=r.bottom*sy;box.setColor(colorFor(b.getText()));c.drawRoundRect(l,t,rr,bb,10,10,box);String s=b.getText().replace('\n',' ');if(s.length()>16)s=s.substring(0,16);c.drawText(s,l,Math.max(28,t-6),label);}if(probes){Paint p=new Paint(Paint.ANTI_ALIAS_FLAG);p.setStyle(Paint.Style.FILL);p.setColor(Color.BLACK);c.drawCircle(getWidth()*.30f,getHeight()*.48f,28,p);p.setColor(Color.RED);c.drawCircle(getWidth()*.70f,getHeight()*.48f,28,p);label.setTextSize(32);c.drawText("COM / GND",getWidth()*.20f,getHeight()*.44f,label);c.drawText("KIRMIZI PROB",getWidth()*.58f,getHeight()*.44f,label);}}
